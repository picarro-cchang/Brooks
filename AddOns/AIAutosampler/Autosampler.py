...
def _getAppVersion():
    try:
        ...
        try:
            ...
            # TODO: use version.json to get the version, if it exists

            verStr = "1.1.0.x"
            print "Release or setup version not found, using: %s" % verStr
            buildTypeStr = "UNKNOWN"

    # if running from source code, the build type is DEVEL
...
class AutosamplerFrame(AutosamplerGUI):
    def __init__(self, configFile, stateFile, tdFile, *a, **k):
        ...
        # if running from sources, it may be under the Vendor folder
        if not hasattr(sys, "frozen") and not os.path.isfile(dllPath):
                # try a path under the Vendor folder (assume running from sources)
                dllPath = os.path.normpath(os.path.join(os.getcwd(),
                                                        "..", "..",
                                                        "Vendor",
                                                        "Applied Instruments",
                                                        "ALS-G",
        ...
        # look for the icon in the current dir
        iconPath = "cogwheel.ico"

        # if running from sources, it may be under the Assets folder
        if not hasattr(sys, "frozen") and not os.path.isfile(iconPath):
            iconPath = os.path.normpath(os.path.join(os.getcwd(),
