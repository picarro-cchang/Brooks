...
# for building the Autosampler from the command line.
#
# This executes python autosamplerSetup.py py2exe and ensures PYTHONPATH is properly
# set to pull local libs from its own folder tree instead of what is
# in Picarro.pth.
#
# This script can be run from a cmd window. autosamplerSetup.py should NOT be run
...
def buildExes():
    ...
    if curDir.lower() != "aiautosampler":
        print "Not running in expected folder 'AIAutosampler'!"
        sys.exit(1)
    ...
    # run "python autosamplerSetup.py py2exe"
    retCode = subprocess.call(['python.exe', 'autosamplerSetup.py', 'py2exe'], env=buildEnv)
