...
Driver for building the ChemCorrect installer package.
"""
...
def makeInstaller(opts):
    """
    Build the ChemCorrect installer from a clean checkout.
    """
    ...
    # productFamily incorporates the OS type (e.g., 'ChemCorrect_win7')
    productFamily = "ChemCorrect_%s" % osType
    ...
    print "ChemCorrect build script start: %s" % time.strftime("%Y/%m/%d %H:%M:%S %p", startTime)
    ...
    if opts.buildExes:
        ...
        exeDir = os.path.join(SANDBOX_DIR, "host", "AddOns", "ChemCorrect", "dist")
...
def _compileInstaller(product, osType, ver):
    ...
    setupFilePath = "%s\\setup_ChemCorrect.iss" % installScriptDir

    print "building from '%s'" % setupFilePath

    args = [ISCC,
            "/dchemCorrectVersion=%s" % _verAsString(product, ver),
...
def _generateReleaseVersion(product, ver):
    ...
    with OS.chdir(os.path.join(SANDBOX_DIR, 'host')):
        filename = os.path.normpath(os.path.join(os.getcwd(), 'AddOns', 'ChemCorrect',
                                                 'release_version.py'))

        with open(filename, 'w') as fp:
            fp.writelines(
                ["# autogenerated by ChemCorrect release.py, %s\n" % time.asctime(),
                 "\n",
                 "def versionString():\n",
...
Builds a new release of the ChemCorrect installer.
"""
