...
# for building PumpSequencer.exe from the command line.
#
# This executes python sequencerSetup.py py2exe and ensures PYTHONPATH is properly
# set to pull local libs from its own folder tree instead of what is
# in Picarro.pth.
#
# This script can be run from a cmd window. sequencerSetup.py should NOT be run
...
buildScriptName = "buildSequencer.py"   # this script name
setupScriptName = "sequencerSetup.py"   # setup script for running py2exe
productName = "PumpSequencer"           # for the internal name string shown for non-release builds
...
def buildExes():
    ...
    # run "python sequencerSetup.py py2exe"
