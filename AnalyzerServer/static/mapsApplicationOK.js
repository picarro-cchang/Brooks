        ...
        if ("timeStrings" in data.result) {
            ...
            if (n>0) {
                ...
                if (n>1) {
                    ...
                    if (prime_view) {
                        $('#concentrationSparkline').sparkline(methaneHistory,{"chartRangeMin":1.8, "width":"150px", "height":"50px"});
                        showWind(135,5);
                    } else {
                        $('#concentrationSparkline').html("");
                    }
...
function showWind(meanBearing,halfWidth) {
    var wMin = meanBearing - halfWidth;
    $('#windRose').sparkline([2*halfWidth,360.0-2*halfWidth],{"type":"pie","height":"50px","offset":-90+wMin,
                                                              "sliceColors":["#00f","#fff"]});
};
...
/*                peakMarker = new google.maps.Marker({position:peakCoords,title:"Amp: "+amp[i].toFixed(2)+" HalfWidth: "+sigma[i].toFixed(1),
                    ...
                    "http://chart.apis.google.com/chart?chst=d_map_spin&chld="+size+"|0|40FFFF|"+fontsize+"|b|" + ch4[i].toFixed(1),
...
function getData() {
    ...
    var params = {'startPos': startPos, 'alog': alog, 'gmtOffset': gmt_offset, 'varList':'["GPS_ABS_LAT","GPS_ABS_LONG","GPS_FIT","CH4","ValveMask","INST_STATUS"]'};
