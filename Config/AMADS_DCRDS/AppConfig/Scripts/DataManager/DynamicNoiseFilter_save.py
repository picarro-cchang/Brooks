...
minNoise[0] = Paras.getfloat("Dynamic filter","MinNoiseHF")      # for HF
minNoise[1] = Paras.getfloat("Dynamic filter","MinNoiseNH3")     # for NH3
minNoise[2] = Paras.getfloat("Dynamic filter","MinNoiseHCl")     # for HCl
...
def variableExpAverage(buffer,y, t, length, MinNoiseID, previousS,previousNoise):
    ...
    sigma = max(MinNoise, current_noise_est)*1.25*np.sqrt(2) #0.01 should be species specific (HF 0.002, HCl 0.01, NH3 0.05) put in an ini file
    ...
    #if k % 250 == 0 and True:
    print k
    print "%.3f  %.2f  %.3f, %.3f" % (sigma, tau,previousNoise, zz)
