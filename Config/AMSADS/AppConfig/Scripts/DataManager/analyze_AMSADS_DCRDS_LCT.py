...
here = os.path.split(os.path.abspath(inspect.getfile(
...
if _PERSISTENT_["init"]:
    ...
    _PERSISTENT_["pzt1_offset"] = _DRIVER_.rdDasReg(
        "PZT_OFFSET_VIRTUAL_LASER1")
    _PERSISTENT_["pzt2_offset"] = _DRIVER_.rdDasReg(
    ...
    _PERSISTENT_["pzt3_offset"] = _DRIVER_.rdDasReg(
...
HF_CONC = (_INSTR_["concentration_hf_slope"],
           _INSTR_["concentration_hf_intercept"])
H2O_CONC_60 = (_INSTR_["concentration_h2o_60_slope"],
               _INSTR_["concentration_h2o_60_intercept"])
O2_CONC = (_INSTR_["concentration_o2_slope"],
           _INSTR_["concentration_o2_intercept"])
NH3_CONC = (_INSTR_["concentration_nh3_slope"],
            _INSTR_["concentration_nh3_intercept"])
CO2_CONC = (_INSTR_["concentration_co2_slope"],
            _INSTR_["concentration_co2_intercept"])
H2O_CONC = (_INSTR_["concentration_h2o_slope"],
            _INSTR_["concentration_h2o_intercept"])

# Define linear transformtions for post-processing
HCl_CONC = (_INSTR_["concentration_hcl_slope"],
            _INSTR_["concentration_hcl_intercept"])
H2O_a_CONC = (_INSTR_["concentration_h2o_a_slope"],
              _INSTR_["concentration_h2o_a_intercept"])
CH4_CONC = (_INSTR_["concentration_ch4_slope"],
            _INSTR_["concentration_ch4_intercept"])
...
HCl_P_correction = (_INSTR_["pressure_hcl_slope"],
                    _INSTR_["pressure_hcl_intercept"])
H2O_a_P_correction = (_INSTR_["pressure_h2o_a_slope"],
                      _INSTR_["pressure_h2o_a_intercept"])
CH4_P_correction = (_INSTR_["pressure_ch4_slope"],
                    _INSTR_["pressure_ch4_intercept"])
C2H4_P_correction = (_INSTR_["pressure_c2h4_slope"],
                     _INSTR_["pressure_c2h4_intercept"])

# Define linear temp correction for HCl cavity
HCl_T_correction = (_INSTR_["temp_hcl_slope"], _INSTR_["temp_hcl_intercept"])
H2O_a_T_correction = (_INSTR_["temp_h2o_a_slope"],
                      _INSTR_["temp_h2o_a_intercept"])
#CH4_T_correction= (_INSTR_["temp_ch4_slope"],_INSTR_["temp_ch4_intercept"])
C2H4_T_correction = (_INSTR_["temp_c2h4_slope"],
                     _INSTR_["temp_c2h4_intercept"])
...
good = pressureLocked and cavityTempLocked and warmboxTempLocked and (
    not warmingUp) and (not systemError)
...
try:
    ...
    if "nh3_conc_ave" in _DATA_:
        ...
        corrected = (temp + H1 * _DATA_["peak15"]) / (1.0 + A1H1 * str15 +
                                                      A1H2 * str15**2)
        ...
        PF_nh3_corrected = _DATA_["PF_nh3_conc_ave"] + C2H2toNH3 * _DATA_[
            "PF_c2h2_conc"] + C3H4toNH3 * _DATA_["PF_c3h4_conc"]
        ...
        h2o_HCcorrected = _DATA_["PF_a_h2o_conc"] + C2H2toH2O * _DATA_[
            "PF_c2h2_conc"] + C3H4toH2O * _DATA_["PF_c3h4_conc"]
        ...
        pf_corrected = (nh3_cal_HCcorrected_wet + H1 * pf_peak15) / (
            1.0 + A1H1 * pf_str15 + A1H2 * pf_str15**2)
        ...
        if ((_DATA_["species"] == 4) and
            (abs(_DATA_["CavityPressure"] - _PERSISTENT_["Pressure_save"]) <
             2.0)):
            _PERSISTENT_[
                "NH3_filter"] = nh3_cal_HCcorrected  #  "nh3_cal_HCcorrected" replaced "dry" on 19 July 2018 (5 places)
            _PERSISTENT_["previousS_NH3"], _PERSISTENT_[
                "previousNoise_NH3"], _PERSISTENT_[
                    "tau_NH3"] = variableExpAverage(
                        _PERSISTENT_["bufferZZ_NH3"],
                        _PERSISTENT_["bufferExpAvg_NH3"], nh3_cal_HCcorrected,
                        now, 1100, 1, _PERSISTENT_["previousS_NH3"],
                        _PERSISTENT_["previousNoise_NH3"])
        _PERSISTENT_["Pressure_save"] = _DATA_["CavityPressure"]
        _NEW_DATA_[
            "NH3_sigma"] = _PERSISTENT_["previousNoise_NH3"] * 1.5 * sqrt(2)
        ...
        _NEW_DATA_["NH3raw-NH3expavg"] = _NEW_DATA_["NH3_filter"] - _NEW_DATA_[
            "NH3_ExpAvg"]
        _NEW_DATA_["NH3_ExpAvg_NZ"] = negative_number_filter(
            "NH3", _NEW_DATA_["NH3_ExpAvg"])
        ...
        NH330s = boxAverage(_PERSISTENT_["bufferNH330"], nh3_cal_HCcorrected,
                            now, 30)
        _NEW_DATA_["NH3_30s"] = NH330s
        _NEW_DATA_["NH3_2min"] = boxAverage(_PERSISTENT_["bufferNH3120"],
                                            nh3_cal_HCcorrected, now, 120)
        _NEW_DATA_["NH3_5min"] = boxAverage(_PERSISTENT_["bufferNH3300"],
                                            nh3_cal_HCcorrected, now, 300)
...
try:
    ...
    _NEW_DATA_[
        "CO2_HCcorrected"] = _DATA_["PF_a_co2_conc"] + C2H2toCO2 * _DATA_[
            "PF_c2h2_conc"] + C3H4toCO2 * _DATA_["PF_c3h4_conc"]
...
try:
    ...
    if "peak_82" in _DATA_:
        temp = applyLinear(_DATA_["hf_ppbv"],
                           HF_CONC) + H1onHF * _DATA_["peak_82"]
        ...
        if "HF_raw" in _OLD_DATA_ and len(_OLD_DATA_["HF_raw"]) > 1:
            ...
            _NEW_DATA_["HF_2min"] = boxAverage(_PERSISTENT_["bufferHF120"],
                                               temp, now, 120)
            _NEW_DATA_["HF_5min"] = boxAverage(_PERSISTENT_["bufferHF300"],
                                               temp, now, 300)
            if ((_DATA_["species"] == 60) and
                (abs(_DATA_["CavityPressure"] - _PERSISTENT_["Pressure_save"])
                 < 2.0)):
                _PERSISTENT_["HF_filter"] = temp
                _PERSISTENT_["previousS_HF"], _PERSISTENT_[
                    "previousNoise_HF"], _PERSISTENT_[
                        "tau_HF"] = variableExpAverage(
                            _PERSISTENT_["bufferZZ_HF"],
                            _PERSISTENT_["bufferExpAvg_HF"], temp, now, 1100,
                            0, _PERSISTENT_["previousS_HF"],
                            _PERSISTENT_["previousNoise_HF"])
            _NEW_DATA_[
                "HF_sigma"] = _PERSISTENT_["previousNoise_HF"] * 1.5 * sqrt(2)
            ...
            _NEW_DATA_["HFraw-HFexpavg"] = _NEW_DATA_[
                "HF_filter"] - _PERSISTENT_["previousS_HF"]
            _NEW_DATA_["HF_ExpAvg_NZ"] = negative_number_filter(
                "HF", _NEW_DATA_["HF_ExpAvg"])
...
try:
    if "hcl_conc" in _DATA_:
        ...
        CH4_T_corr = tempDiff * tempDiff * _INSTR_[
            "temp_ch4_c2"] + tempDiff * _INSTR_["temp_ch4_c1"] + _INSTR_[
                "temp_ch4_c0"]
        C2H4_T_corr = applyLinear(tempDiff, C2H4_T_correction)

        hcl_conc = _DATA_["hcl_conc"] * (1 + HCl_P_corr + HCl_T_corr)
        h2o_a_conc = _DATA_["h2o_a_conc_raw"] * (1 + H2O_a_P_corr +
                                                 H2O_a_T_corr)
        ...
        temp = applyLinear(
            hcl_conc + C2H4_to_HCl * c2h4_conc +
            C3H4_to_HCl * _DATA_["PF_c3h4_conc"], HCl_CONC)
        _NEW_DATA_["HCl_raw"] = temp
        now = _OLD_DATA_["HCl_raw"][-2].time
        _NEW_DATA_["HCl_30sec"] = boxAverage(_PERSISTENT_["bufferHCl30"], temp,
                                             now, 30)
        _NEW_DATA_["HCl_2min"] = boxAverage(_PERSISTENT_["bufferHCl120"], temp,
                                            now, 120)
        _NEW_DATA_["HCl_5min"] = boxAverage(_PERSISTENT_["bufferHCl300"], temp,
                                            now, 300)
        # remove the species filt#er (_DATA_["species"]==63) and
        if ((abs(_DATA_["CavityPressure"] - _PERSISTENT_["Pressure_save"]) <
             2.0)):
            _PERSISTENT_["HCl_filter"] = temp
            _PERSISTENT_["previousS_HCl"], _PERSISTENT_[
                "previousNoise_HCl"], _PERSISTENT_[
                    "tau_HCl"] = variableExpAverage(
                        _PERSISTENT_["bufferZZ_HCl"],
                        _PERSISTENT_["bufferExpAvg_HCl"], temp, now, 1100, 2,
                        _PERSISTENT_["previousS_HCl"],
                        _PERSISTENT_["previousNoise_HCl"])
        _PERSISTENT_["Pressure_save"] = _DATA_["CavityPressure"]
        _NEW_DATA_[
            "HCl_sigma"] = _PERSISTENT_["previousNoise_HCl"] * 1.5 * sqrt(2)
        ...
        _NEW_DATA_["HCl_ExpAvg_NZ"] = negative_number_filter(
            "HCl", _NEW_DATA_["HCl_ExpAvg"])
        ...
        _NEW_DATA_["H2O_a_30sec"] = boxAverage(_PERSISTENT_["bufferHCl30_H2O"],
                                               _NEW_DATA_["H2O_a"], now, 30)
...
#  PZT voltage control goes here.  For the time being I am not locking the pzt offsets
        ...
        _DRIVER_.wrDasReg("PZT_OFFSET_VIRTUAL_LASER2",
                          _PERSISTENT_["pzt2_offset"])
...
#  PZT voltage control goes here.  For the time being I am not locking the pzt offsets
        ...
        _DRIVER_.wrDasReg("PZT_OFFSET_VIRTUAL_LASER1",
                          _PERSISTENT_["pzt1_offset"])
...
#  PZT voltage control goes here.
        ...
        _DRIVER_.wrDasReg("PZT_OFFSET_VIRTUAL_LASER3",
                          _PERSISTENT_["pzt3_offset"])
