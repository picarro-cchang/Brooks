...
#  2016 0523:  modification to use "big water" for WLM calibration in dry nitrogen
...
if not good:
    ...
    if _DATA_["species"] == 63: # Update the offset for virtual laser 2
        try:
            hcl_adjust = _DATA_["hcl_adjust"]*hcl_gain
            hcl_adjust = min(max_adjust,max(-max_adjust,hcl_adjust))
            newOffset0 = _FREQ_CONV_.getWlmOffset(2) + hcl_adjust
            _PERSISTENT_["wlm2_offset"] = newOffset0
            _FREQ_CONV_.setWlmOffset(2,float(newOffset0))
            # print "New HCl (virtual laser 2) offset: %.5f" % newOffset0 
        except:
            pass
            # print "No new HCl (virtual laser 2) offset"
