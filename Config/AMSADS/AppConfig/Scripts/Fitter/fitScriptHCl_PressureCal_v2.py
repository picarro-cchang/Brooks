...
#  changed to use cavity2pressure and temp to account for use in dual cavity
...
if INIT:
    ...
    anH2O = []
    anH2O.append(
        Analysis(os.path.join(BASEPATH, r"./HCl/HCl_PressureCal_v1.ini")))
    lastShift = None
...
ANALYSIS = []
d = DATA
d.cavityPressure += (d.sensorDict["Cavity2Pressure"] -
                     d.sensorDict["CavityPressure"])
...
if d["spectrumId"] == 0:
    ...
    RESULT = {
        "residuals": residuals,
        "y_parameter": abs(y_60),
        "h2o_peak": peak_60,
        "baseline": base,
        "baseline_slope": slope,
        "perturber_peak": peak_61,
        "cm_shift": cm_shift,
        "freq_offset": cm_adjust
    }

    RESULT.update({
        "species": 0,
        "fittime": time.clock() - tstart,
        "cavity_pressure": P,
