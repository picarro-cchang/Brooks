...
#  2019-09-04: edited for use in parrallel with other fitters to only report when spectrumID is correct
...
if INIT:
    ...
    Driver = CmdFIFO.CmdFIFOServerProxy("http://localhost:%d" % RPC_PORT_DRIVER,
                                        "FitScript1", IsDontCareConnection=False)
    anHF = []
...
# Make it so that fitter does no computation when spectrumID is not correct.
if d["spectrumId"] not in [60,61]:
    RESULT = {}
else:
