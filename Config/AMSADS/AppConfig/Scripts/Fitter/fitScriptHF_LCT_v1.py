...
if d["spectrumId"]==60 and d["ngroups"]>8:
    ...
    if goodLCT:
        d.waveNumber = 7823.84945 + fsr*round_((d.waveNumber - 7823.84945)/fsr)
        d.badRingdownFilter("uncorrectedAbsorbance",minVal=0.20,maxVal=20.0)
        d.sparse(maxPoints=1000,width=0.003,height=100000.0,xColumn="waveNumber",yColumn="uncorrectedAbsorbance",outlierThreshold=4.0)
        d.evaluateGroups(["waveNumber","uncorrectedAbsorbance"])
        d.defineFitData(freq=d.groupMeans["waveNumber"],loss=1000*d.groupMeans["uncorrectedAbsorbance"],sdev=1/sqrt(d.groupSizes)) 
    ...
    f = d.waveNumber    
    topper = (f >= 7823.84) & (f <= 7823.86)  #  Check number of peak points
    ftop = f[topper]
    ntopper = len(f[topper])
    
    # if goodLCT and peak_77 > 5:
        # pzt1_adjust = -shift_77*pzt_per_fsr/fsr
    # else:
        # pzt1_adjust = 0.0
...
if d["spectrumId"]==61 and d["ngroups"]>9:
    ...
    if goodLCT and peak_81 > 10:
        pzt1_adjust = -shift_81*pzt_per_fsr/fsr
    else:
        pzt1_adjust = 0.0
...
if (ignore_count == 0) and (not IgnoreThis):      
    RESULT = {"base0":base0,"base1":base1,"base77_ave":base77_ave,"y_77":y_77,"y_79":y_79,
          ...
          "h2o_conc_61":h2o_conc_61,"o2_conc":o2_conc,"ntopper":ntopper,
