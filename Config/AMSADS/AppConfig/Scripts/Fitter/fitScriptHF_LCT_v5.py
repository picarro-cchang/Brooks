...
#  2018-02-13:  Changed pzt shift calculation to accomodate case where mode at HF peak is missing
#  2018-02-15:  Added correction for effect of missing peak points -- for ultra-high range
...
if d["spectrumId"]==60 and d["ngroups"]>6:
    ...
    # HF Region
    base_start = (f >= 7823.735) & (f <= 7823.755) & (d.extra1 == 1)
    base_start_loss = l[base_start]
    base_finish = (f >= 7823.735) & (f <= 7823.755) & (d.extra1 == 2)
    base_finish_loss = l[base_finish]
    if len(base_start_loss)*len(base_finish_loss) == 0:
        baselineFlagHF = setBitFlag(baselineFlagHF, 0)  # Trip first bitFlag ( No RD )
    elif len(base_start_loss)*len(base_finish_loss) != 0:
        baselineFlagHF = resetBitFlag(baselineFlagHF, 0)  # Reset first bitFlag
        delta_loss_hf = mean(base_finish_loss)-mean(base_start_loss)   # Compute baseline shift in HF region
        if abs(delta_loss_hf) > 1.5:                        #  Value set to be 10 std dev of typical room air measurement
            baselineFlagHF = setBitFlag(baselineFlagHF, 1)  # Trip second bitFlag ( Baseline Change )
        elif abs(delta_loss_hf) <= 1.5:
            baselineFlagHF = resetBitFlag(baselineFlagHF, 1)  # Reset second bitFlag
    if baselineFlagHF == 0:
        bad_hf_baseline = 0
    else:
        bad_hf_baseline = 1
...
if (ignore_count == 0) and (not IgnoreThis):      
    RESULT = {"base0":base0,"base1":base1,"base77_ave":base77_ave,"y_77":y_77,"y_79":y_79,
          ...
          "h2o_conc_61":h2o_conc_61,"o2_conc":o2_conc,"delta_loss_hf":delta_loss_hf,
          ...
          "bad_hf_baseline":bad_hf_baseline,"incomplete_hf_spectrum":incomplete_hf_spectrum,"baselineFlagHF":baselineFlagHF,          
