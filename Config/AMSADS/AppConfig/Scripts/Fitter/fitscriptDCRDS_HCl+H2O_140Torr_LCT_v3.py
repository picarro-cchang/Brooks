...
#  2019-09-04:  Changed sstructure to only enter fitter when correct species are involved
...
if INIT:
    ...
    #  the threshold is set to "min_rd_threshold_factor". Once all reported
    #  losses are less than "safe_loss_level", the threshold is 
    #  reset to "original_rd_threshold_factor"

    if "original_rd_threshold_factor" in instrParams:
        original_rd_threshold_factor = instrParams['original_rd_threshold_factor']
    else:
        original_rd_threshold_factor = Driver.rdDasReg("THRESHOLD_FACTOR_VIRTUAL_LASER3")
    #HCl laser is set in Virtual laser 3
    if "min_rd_threshold_factor" in instrParams:
        min_rd_threshold_factor = instrParams['min_rd_threshold_factor']
    else:
        min_rd_threshold_factor = 0.5
    ...
    Log("Setting dynamic ringdown threshold parameters", 
        Data=dict(original_rd_threshold_factor=original_rd_threshold_factor,
                  min_rd_threshold_factor=min_rd_threshold_factor,
...
if not(disable_dynamic_threshold) and (rd_rate != -1) and (rd_rate < min_rd_rate) and (current_threshold_factor > min_rd_threshold_factor):
    ...
    if (ignore_count ==0) and not IgnoreThis:   # and not bad_baseline:
        RESULT = {"res":res,"degraded_hcl_performance":degraded_hcl_performance,"baseline_level":baseline_level,"baseline_slope":baseline_slope,"base70":base70,
                ...
                "ngroups":d["ngroups"],"numRDs":d["datapoints"],"hcl_interval":hcl_interval,
