...
#  2017-03-30:  Include the reference points into fitter, if baseStdev*1000 < 1 
from numpy import any, mean, std, sqrt,abs
import numpy as np
import os.path
import time
...
baseMean = np.mean(d.uncorrectedAbsorbance[d.extra2 ==1])
baseStdev = np.std(d.uncorrectedAbsorbance[d.extra2 ==1])
#print baseStdev
N = len(d.extra2)
tWindow =180 #  Time window for baseline measurement in ms. We have tested the different time windows 20, 40, 60, 80, 100 for the schemes in different intervals
goodPoints = []
for i, ext2 in enumerate(d.extra2):
    #if ext2 == 1 and baseStdev*1000 < 1 : 
        #print abs(baseMean-loss[i])
    #    goodPoints.append(i)    
    if ext2 == 0:
        win = slice(max(0,i-10), min(N,i+11))
        lwin = loss[win]
        twin = ts[win]
        select = (d.extra2[win] == 1) & (abs(ts[win] - ts[i]) < tWindow)
        if sum(select) > 1:
...
d.badRingdownFilter("uncorrectedAbsorbance",minVal=-10.0,maxVal=10.0)
...
if d["spectrumId"]==63 and d["ngroups"]>25:
    ...
    if (r[62,"peak"] < 4 and r[70,"peak"] < 4 and r[1002,2] < 0.02) or abs(r["base",3]) > 0.05:
        ...
        ANALYSIS.append(r)
    elif r[62,"peak"] < 4 or abs(r[62,"y"]-y62lib) > 0.5:
...
if (ignore_count ==0) and not IgnoreThis:   # and not bad_baseline:     
    RESULT = {"res":res,"baseline_level":baseline_level,"baseline_slope":baseline_slope,"base70":base70,
              ...
              "baseMean":baseMean,"baseStdev":baseStdev,              
