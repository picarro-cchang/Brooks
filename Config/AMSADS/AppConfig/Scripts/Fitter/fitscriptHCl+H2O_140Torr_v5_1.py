...
if INIT:
    ...
    anHCl.append(Analysis(os.path.join(BASEPATH,r"./HCl/HCl only 140 Torr FC FY v1_1.ini")))
...
if d["spectrumId"]==63 and d["ngroups"]>25:
    ...
    if len(base3_loss)*len(base4_loss) == 0:
        bad_baseline = 1
    delta_loss_h2o = mean(base4_loss)-mean(base3_loss)   # Reject spectrum with large baseline shift in water region
    if abs(delta_loss_h2o) > 1:
        bad_baseline = 1

if d["ngroups"]>6: #d["spectrumId"]==64 and d["ngroups"]>6:
    initialize_Baseline()
    init[63,"strength"] = 0.0459*str62
    init[1002,2] = ch4_ampl
    r = anHCl[3](d,init,deps)          #  Fit HCl line only with water and methane preset  
    ANALYSIS.append(r)
    res = r["std_dev_res"]
    baseline_level = r["base",0]
    baseline_slope = r["base",1]
    hcl_shifta = r["base",3]
    peak70a = r[70,"peak"]  
    str70a = r[70,"strength"]
    base70a = r[70,"base"]
    
    hcl_conc_raw = hcl_lin*str70a
    hcl_conc = hcl_conc_raw + ch4_hcl_lin*ch4_ampl + h2o_hcl_lin*peak62
...
if (ignore_count ==0) and not IgnoreThis:   # and not bad_baseline:     
    RESULT = {"res":res,"baseline_level":baseline_level,"baseline_slope":baseline_slope,"base70":base70,
              ...
              "hcl_shifta":hcl_shifta,"peak70a":peak70a,"str70a":str70a,
              ...
              "bad_baseline":bad_baseline,"pzt_mean":pzt_mean,"pzt_stdev":pzt_stdev}
