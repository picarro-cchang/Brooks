...
# Get this argument from .mode file
try:
    scanningMode = _ARGS_[0]
except:
    SYNC_LIST = ["h2s_conc", "H2S_30s", "H2S_2min", "H2S_5min", "ch4_conc", "ch4_conc_dry", "h2o_conc"]

if scanningMode == "CH4_Only":
    SYNC_LIST = ["ch4_conc"]
elif scanningMode == "H2S_H2O":
    SYNC_LIST = ["h2s_conc", "H2S_30s", "H2S_2min", "H2S_5min", "h2o_conc"]
else:
    SYNC_LIST = ["h2s_conc", "H2S_30s", "H2S_2min", "H2S_5min", "ch4_conc", "ch4_conc_dry", "h2o_conc"]
...
if allGood:
    ...
    # Get tagalong data
    for dataLabel in ["GPS_TIME", "GPS_ABS_LAT", "GPS_REL_LAT", "GPS_ABS_LONG", "GPS_REL_LONG", "GPS_FIT", "WS_WIND_SPEED", "WS_WIND_DIRECTION", "WS_TEMP", "WS_REL_HUMIDITY", "WS_PRESSURE"]:
        try:
            _REPORT_[dataLabel] = _OLD_DATA_[dataLabel][index].value
        except:
            pass
