...
PCO2 = _INSTR_["co2_pressure_slope"]
PCH4 = _INSTR_["ch4_pressure_slope"]
Pnom = _INSTR_["nominal_pressure"]
...
if _PERSISTENT_["ignore_count"] > 0:
    ...
    Pdiff = _DATA_['cavity_pressure'] - Pnom

    for k in _DATA_.keys():
        if k in newname:
            new_k = newname[k]
            if new_k.startswith('CO2'):
                _REPORT_[new_k] = _DATA_[k]/(1+PCO2*Pdiff)
            elif new_k.startswith('CH4'):
                _REPORT_[new_k] = _DATA_[k]/(1+PCH4*Pdiff)
            else:
                _REPORT_[new_k] = _DATA_[k]
        else:
            _REPORT_[k] = _DATA_[k]
    ...
    for k in _NEW_DATA_.keys():
        if k in newname:
            new_k = newname[k]
            if new_k.startswith('CO2'):
                _REPORT_[new_k] = _NEW_DATA_[k]/(1+PCO2*Pdiff)
            elif new_k.startswith('CH4'):
                _REPORT_[new_k] = _NEW_DATA_[k]/(1+PCH4*Pdiff)
            else:
                _REPORT_[new_k] = _NEW_DATA_[k]
        else:
