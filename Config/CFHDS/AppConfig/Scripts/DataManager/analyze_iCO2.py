...
#  2013 0521 - added wet/dry correction to methane concentration.  Copied from CFADS but w/o bookending of water measurements
#              added nonlinearity correction to water to generate "actual" water concentration
#  2013 0524 - differentiate W/D correction for high precision and high range modes
#  2013 1031 - change reporting of isotopic concentrations for consistency of [12CO2], [13CO2], and delta
...
try:
    if _OLD_DATA_["species"][-1].value == 106 and _OLD_DATA_["species"][-2].value == 105 and _OLD_DATA_["species"][-3].value == 106:
        ...
        ratio = R13*(1.0 + 0.001*_OLD_DATA_["Delta_Raw"][-1].value)
...
try:
    ...
    _NEW_DATA_["13CO2"] = temp*ratio
    _NEW_DATA_["CO2"] = temp*(1.0 + ratio + Rrest)
except:
    pass
...
try:
    ...
    _NEW_DATA_["13CO2_dry"] = temp*ratio
    _NEW_DATA_["CO2_dry"] = temp*(1.0 + ratio + Rrest)
except:
    pass

try:
    h2o_reported = applyLinear(_DATA_["h2o_conc"],H2O)  # Uses 6057.8 water peak (CFADS line)
...
try:
    ...
    h2o_reported = applyLinear(_DATA_["h2o_conc"],H2O)
    h2o_precorr = h2o_reported                                            # drop h2o bookending for now
    ch4_conc_dry = temp/(1.0+h2o_precorr*(_INSTR_["ch4_hp_watercorrection_linear"]+h2o_precorr*_INSTR_["ch4_hp_watercorrection_quadratic"]))
    _NEW_DATA_["CH4_High_Precision_dry"] = ch4_conc_dry
...
try:
    ...
    h2o_reported = applyLinear(_DATA_["h2o_conc"],H2O)
    h2o_precorr = h2o_reported                                            # drop h2o bookending for now
    ch4_conc_dry = temp/(1.0+h2o_precorr*(_INSTR_["ch4_watercorrection_linear"]+h2o_precorr*_INSTR_["ch4_watercorrection_quadratic"]))
    _NEW_DATA_["CH4_dry"] = ch4_conc_dry
...
if not good:
    ...
    if _DATA_["species"] == 105: # Update the offset for virtual laser 1
        ...
        try:
            ...
            # print "New CH4 high precision(virtual laser 3) offset: %.5f" % newOffset0
        except:
            pass
            # print "No new CH4 high precision(virtual laser 3) offset"
        ...
        try:
            ...
            # print "New CH4 low precision(virtual laser 4) offset: %.5f" % newOffset0
        except:
            pass
            # print "No new CH4 low precision(virtual laser 4) offset"
