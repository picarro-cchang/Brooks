#  Data analysis script for the experimental instrument combining isotopic CO2 and isotopic methane
#  2011 1118 - start by merging FCDS (iso-CH4) and CFFDS (iso-CO2) analysis scripts
#  2013 0918- All 'print' statements were commented out, added 'Log()' statement in order to get rid of data latency issues inside the Data manager.
...
if _PERSISTENT_["init"]:
    ...
    _PERSISTENT_["last_delta_time_iCO2"] = 0
    ...
    _PERSISTENT_["delta_interval_iCO2"] = 0
    _PERSISTENT_["Delta_iCH4_Raw"] = 0.0
    _PERSISTENT_["meas_mode"] = 0
    # For ChemDetect
    _PERSISTENT_["chemdetect_inst"] = InstructionProcess()
    #configFile = os.path.join(here,"ChemDetect\ChemDetect.ini")
    ...
    #ChemDetect_FileName=r"C:\Picarro\G2000\AppConfig\Scripts\DataManager\ChemDetect\ChemDetect_iCH4.csv" #this works but needs "r"
...
# Cannot execute this here or end up with extra rows of zeroes at instrument start.
...
TARGET_SPECIES = [11, 25, 105, 106, 150, 151, 152]
...
ch4 = optDict.get("ch4", True)
co2 = optDict.get("co2", True)
#bck = optDict.get("background", "N2").lower()
...
try:
    _12_ch4_raw_dry = _DATA_["12CH4_raw_dry"]
    temp = applyLinear(_12_ch4_raw_dry,C12_iCH4)
    _NEW_DATA_["HR_12CH4_dry"] = temp
except:
    pass
...
#try:
    #temp = applyLinear(_DATA_["13CH4_raw_dry"],C13_iCH4)  # Uses high concentration methane
    #_NEW_DATA_["13CH4_dry"] = temp
#except:
    #pass
...
if co2 == True:
    if ch4 == True:
        if conc == "high":
            ...
            if _PERSISTENT_["state"] != "low_conc":
                ...
                try:
                    _SPEC_COLL_.setSequence("FBDS_mode_DUAL_LOW_CH4")
        ...
        if _PERSISTENT_["state"] != "low_conc":
            _PERSISTENT_["state"] = "low_conc"
            try:
                _SPEC_COLL_.setSequence("FBDS_mode_iCO2")
                # _DRIVER_.setMultipleNoRepeatScan()
            except: # For Virtual analyzer
                pass
else:
    if conc == "high":
        if _PERSISTENT_["state"] != "low_conc":
            _PERSISTENT_["state"] = "low_conc"
            try:
                _SPEC_COLL_.setSequence("FBDS_mode_iCH4_HIGH_CH4")
                # _DRIVER_.setMultipleNoRepeatScan()
            except: # For Virtual analyzer
                pass
    elif conc == "low":
        if _PERSISTENT_["state"] == "high_conc":
            _PERSISTENT_["state"] = "low_conc"
            try:
                _SPEC_COLL_.setSequence("FBDS_mode_iCH4_LOW_CH4")
                # _DRIVER_.setMultipleNoRepeatScan()
                # print "iCH4 HP mode"
            except: # For Virtual analyzer
                pass
...
# print _PERSISTENT_["state"]

# Calculate  methane delta without CFADS laser
try:
    if _DATA_["species"] == 150:
        ...
        # use VPDP std reference
        _NEW_DATA_["HR_Ratio_iCH4_Raw"] = ((delta_high_conc_iCH4)/1000+1)*0.0112372
        HR_ratio_i13CH4 = _NEW_DATA_["HR_Ratio_iCH4_Raw"]
        ...
        _NEW_DATA_["HR_Ratio_iCH4_Raw"] = _OLD_DATA_["HR_Ratio_iCH4_Raw"][-1].value
except Exception,e:
    # print "Error %s (%r)" % (e,e)
    Log("Error %s (%r)" % (e,e))
    pass
...
# Calculate methane delta with CFADS laser
try:
    if _PERSISTENT_["state"] == "low_conc":
        ...
        if _DATA_["species"]  == 150:
            ...
            peak5_spec_low_conc += P5_A1_low_conc*_DATA_["peak87_baseave_spec"] + P5_H1_low_conc*_DATA_["peak30_spec"]+ P5_H1M1_low_conc*_DATA_["peak30_spec"]*_DATA_["peakheight_5"]
            ...
            # use VPDP std reference
            _NEW_DATA_["HP_Ratio_iCH4_Raw"] = ((delta_iCH4)/1000+1)*0.0112372
            HP_ratio_i13CH4 = _NEW_DATA_["HP_Ratio_iCH4_Raw"]
            ...
            _NEW_DATA_["HP_Ratio_iCH4_Raw"] = _OLD_DATA_["HP_Ratio_iCH4_Raw"][-1].value
    else:
        _NEW_DATA_["HP_Delta_iCH4_Raw"] = INVALID
        _NEW_DATA_["HP_Delta_iCH4_30s"] = INVALID
        _NEW_DATA_["HP_Delta_iCH4_2min"] = INVALID
        _NEW_DATA_["HP_Delta_iCH4_5min"] = INVALID
        _NEW_DATA_["Ratio_Raw_iCH4"] = INVALID
        _NEW_DATA_["Ratio_30s_iCH4"] = INVALID
        _NEW_DATA_["Ratio_2min_iCH4"] = INVALID
        _NEW_DATA_["Ratio_5min_iCH4"] = INVALID
        _PERSISTENT_["buffer30_iCH4"] = []
        _PERSISTENT_["buffer120_iCH4"] = []
        _PERSISTENT_["buffer300_iCH4"] = []
        _PERSISTENT_["ratio30_iCH4"] = []
    ...
    Log("Error %s (%r)" % (e,e))
    pass
# new data column that consolidates the low and high conc delta values
try:
    if _PERSISTENT_["state"] == "low_conc":
...
try:
    if _OLD_DATA_["species"][-1].value == 106 and _OLD_DATA_["species"][-2].value == 105 and _OLD_DATA_["species"][-3].value == 106:
        # print "species", _OLD_DATA_["species"][-1].value
        # print "peak88_baseave_spec_1", _OLD_DATA_["peak88_baseave_spec"][-1].value
        # print "peak87_baseave_spec", _OLD_DATA_["peak87_baseave_spec"][-2].value
        # print "peak88_baseave_spec_2", _OLD_DATA_["peak88_baseave_spec"][-3].value
        ...
        delta = applyLinear(temp,DELTA_iCO2)
        _NEW_DATA_["Delta_Raw_iCO2"] = delta

        # use VPDP std reference
        _NEW_DATA_["Ratio_iCO2_Raw"] = ((delta)/1000+1)*0.0112372
        ratio_i13CO2 = _NEW_DATA_["Ratio_iCO2_Raw"]

        # print "delta_iCO2", delta
        # print "temp", temp
        now = _OLD_DATA_["peak87_baseave"][-2].time
        # calculate delta time interval
        if _PERSISTENT_["last_delta_time_iCO2"] != 0:
            delta_interval_iCO2 = now - _PERSISTENT_["last_delta_time_iCO2"]
        else:
            delta_interval_iCO2 = 0
        _PERSISTENT_["last_delta_time_iCO2"] = now
        _PERSISTENT_["delta_interval_iCO2"] = delta_interval_iCO2
        # end of calculate delta time interval

        _NEW_DATA_["Delta_30s_iCO2"] = boxAverage(_PERSISTENT_["buffer30_iCO2"],delta,now,30)
        _NEW_DATA_["Delta_2min_iCO2"] = boxAverage(_PERSISTENT_["buffer120_iCO2"],delta,now,120)
        _NEW_DATA_["Delta_5min_iCO2"] = boxAverage(_PERSISTENT_["buffer300_iCO2"],delta,now,300)
        ratio = applyLinear(temp,RATIO_iCO2)
        _NEW_DATA_["Ratio_Raw_iCO2"] = ratio
        _NEW_DATA_["Ratio_30s_iCO2"] = boxAverage(_PERSISTENT_["ratio30_iCO2"],ratio,now,30)
        _NEW_DATA_["Ratio_2min_iCO2"] = boxAverage(_PERSISTENT_["ratio120_iCO2"],ratio,now,120)
        _NEW_DATA_["Ratio_5min_iCO2"] = boxAverage(_PERSISTENT_["ratio300_iCO2"],ratio,now,300)
        _NEW_DATA_["delta_interval_iCO2"] = _PERSISTENT_["delta_interval_iCO2"]
        ...
        _NEW_DATA_["Ratio_iCO2_Raw"] = _OLD_DATA_["Ratio_iCO2_Raw"][-1].value
        _NEW_DATA_["delta_interval_iCO2"] = _OLD_DATA_["delta_interval_iCO2"][-1].value
except:
    pass
...
try:
    ...
    _NEW_DATA_["13CO2"] = _NEW_DATA_["Ratio_iCO2_Raw"]*_NEW_DATA_["12CO2_dry"]
except:
    pass
...
#except:
    #pass
#print "13CO2_ppm_wet", _DATA_["13CO2_ppm_wet"]
...
try:
    ...
    _NEW_DATA_["HP_12CH4"] = INVALID

try:
    temp = applyLinear(_DATA_["ch4_conc_ppmv_final_dry"],C12_CH4_CFADS)
    _NEW_DATA_["HP_12CH4_dry"] = temp
    _NEW_DATA_["HP_13CH4"] = _NEW_DATA_["HP_Ratio_iCH4_Raw"]*_NEW_DATA_["HP_12CH4_dry"]
except:
    _NEW_DATA_["HP_12CH4_dry"] = INVALID
    _NEW_DATA_["HP_13CH4"] = INVALID
...
try:
    _NEW_DATA_["HR_13CH4"] = _NEW_DATA_["HR_Ratio_iCH4_Raw"]*_NEW_DATA_["HR_12CH4_dry"]
except:
    pass

try: # new ChemDetect section
    _NEW_DATA_["HP_or_HR_mode"] = 0 # init value

    if _PERSISTENT_["meas_mode"] == 0:
        if ch4 == False:
            _PERSISTENT_["meas_mode"] = 7 # iCO2 solo mode
            # print "meas_mode=", _PERSISTENT_["meas_mode"]
        else: # (ch4==True)
            if conc == "low":   # (ch4 conc)
                if co2 == True:
                    _PERSISTENT_["meas_mode"] = 1  # iCO2-iCH4-HP
                else:
                    _PERSISTENT_["meas_mode"] = 4  # iCH4-HP
            elif conc == "high":   # (ch4 conc)
                if co2 == True:
                    _PERSISTENT_["meas_mode"] = 2  # iCO2-iCH4-HR
                else:
                    _PERSISTENT_["meas_mode"] = 5  # iCH4-HR
            elif conc == "auto": # (ch4 conc)
                if co2 == True:
                    _PERSISTENT_["meas_mode"] = 3  # iCO2-iCH4-Auto
                else:
                    _PERSISTENT_["meas_mode"] = 6  # iCH4-Auto
            else:
                # print "Error: illegal conc value. meas_mode remains 0. conc=", conc
                Log("Error: illegal conc value. meas_mode remains 0. conc %f=" % conc)
    # print "meas_mode=", _PERSISTENT_["meas_mode"]
    _NEW_DATA_["MeasurementMode"] = _PERSISTENT_["meas_mode"]


    # For HR and Auto modes, the CH4 conc may become as low as ~10ppm, so for such low CH4,
    # we apply ChemDetect criteria for HP mode. The same thing happens for iCO2 mode.
    if _PERSISTENT_["meas_mode"]==1 or _PERSISTENT_["meas_mode"]==4:
        _NEW_DATA_["HP_or_HR_mode"] = 1 # HP mode always
    else:
        if _12_ch4_raw < CH4_HIGH_THRESHOLD: # = 18.0 ppm
            _NEW_DATA_["HP_or_HR_mode"] = 1
        else:
            _NEW_DATA_["HP_or_HR_mode"] = 2

    # print "HP or HR mode=", _NEW_DATA_["HP_or_HR_mode"]

    if _NEW_DATA_["HP_or_HR_mode"] == 1:
...
if _PERSISTENT_["meas_mode"] == 0:
    raise Exception, "Error: meas_mode=0, it must be non-zero."

if _NEW_DATA_["HP_or_HR_mode"] == 0:
    raise Exception, "Error: HP_or_HR_mode=0, it must be non-zero. Kill DataManager now"
...
if abs(_DATA_["cavity_pressure"]-148.0) > 10:
    good = False
...
if not good:
    Log("Updating WLM offset not done because of bad instrument status")
else:
    ...
    if _DATA_["species"] == 106: # Update the offset for virtual laser 2
        try:
    ...
    if _DATA_["species"] == 150: # Update the offset for virtual laser 3
        try:
    ...
    #if _DATA_["species"] == 25: # Update the offset for virtual laser 8
    ...
    if _DATA_["species"] in [150,151]: # Update the offset for virtual laser 4
        try:
    ...
    if _DATA_["species"] in [150,152]: # Update the offset for virtual laser 5
        try:
    ...
    if _DATA_["species"] == 11: # Update the offset for virtual laser 7
        try:
    ...
    if _DATA_["species"] == 153: # Update the offset for virtual laser 6
        try:
...
#     print '_REPORT_[', colname, ']=', _REPORT_[colname]
...
if _OLD_DATA_["species"][-1].value == 150:
  ...
  if _PERSISTENT_["chemdetect_inst"].current_var_values['RED'] == True:
     ...
     Log("WARNING: ChemDetect Status is RED")
