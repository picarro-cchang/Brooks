#  Baseline ripple fitter for CFADS methane region at 6057 wvn
#  23 Mar 2012:  Initial estimate of ripple frequencies using autocorrrelation of baseline
#  28 Feb 2014:  add a prefit with only 1 sinusoid for very good cavities
...
if INIT:
    ...
    analysis = []
    analysis.append(Analysis(os.path.join(BASEPATH,r"./CFADS-xx CH4 baseline ripple 1frequency.ini")))
    analysis.append(Analysis(os.path.join(BASEPATH,r"./CFADS-xx CH4 baseline ripple v01.ini")))

    oname = os.path.join(BASEPATH,time.strftime("FitterOutput_CH4.dat",time.localtime()))
    file(oname,"wb").close()
    first_fit = 1
    p0 = 1.2
    p1 = 0.6
