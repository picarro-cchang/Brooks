...
if _PERSISTENT_["init"]:
    ...
    _PERSISTENT_["state"] = "ich4"

REPORT_UPPER_LIMIT = 5000.0
REPORT_LOWER_LIMIT = -5000.0

TARGET_SPECIES = [105, 106, 109, 150, 152, 154]
...
type = optDict.get("type", "ico2").lower()
print "type = " + type
...
DELTA_iCH3D = (_INSTR_["iCH3D_concentration_iso_slope"],_INSTR_["iCH3D_concentration_iso_intercept"])
DELTA_i13CH4 = (_INSTR_["i13CH4_concentration_iso_slope"],_INSTR_["i13CH4_concentration_iso_intercept"])
...
if type == "ich4":
    ...
    if _PERSISTENT_["state"] == "ich4":
        _PERSISTENT_["state"] = "ico2"
        try:
...
#iCH3D
try:
    if _DATA_["species"] == 154:
        temp = protDivide(_DATA_["CH3D_spline_corrected"],_DATA_["amp_CH4_spline"])
        delta_iCH3D = applyLinear(temp,DELTA_iCH3D)
        #print "delta_iCH4", delta_iCH4
        _NEW_DATA_["Delta_Raw_iCH3D"] = delta_iCH3D
        temp = protDivide(_DATA_["C13_spline_corrected"],_DATA_["amp_CH4_spline"])
        delta_i13CH4 = applyLinear(temp,DELTA_i13CH4)
        #print "delta_iCH4", delta_iCH4
        _NEW_DATA_["Delta_Raw_i13CH4"] = delta_i13CH4
        now = _OLD_DATA_["CH3D_spline_corrected"][-1].time
        ...
        _NEW_DATA_["Delta_Raw_iCH3D"] = _OLD_DATA_["Delta_Raw_iCH3D"][-1].value
        _NEW_DATA_["Delta_Raw_i13CH4"] = _OLD_DATA_["Delta_Raw_i13CH4"][-1].value
...
if not good:
    ...
    elif _DATA_["species"] == 109: # Update the offset for virtual laser 3; temp use virtual laser 2 (YH 20120214)
        try:
            h2o_adjust = _DATA_["adjust_91"]
            h2o_adjust = min(max_adjust_H2O,max(-max_adjust_H2O,damp*h2o_adjust))
            newOffset0 = _FREQ_CONV_.getWlmOffset(2) + h2o_adjust
            _NEW_DATA_["wlm2_offset"] = newOffset0
            _PERSISTENT_["wlm2_offset"] = newOffset0
            _FREQ_CONV_.setWlmOffset(2,float(newOffset0))
            # print "New H2O (virtual laser 3) offset: %.5f" % newOffset0
        except:
            pass
            # print "No new H2O (virtual laser 3) offset"

    elif _DATA_["species"] == 150: # Update the offset for virtual laser 3
        try:
...
#print _PERSISTENT_["plot_iCH4"]
    ...
    elif _DATA_["species"] == 154:
        # Update the offset for virtual laser 6
        try:
            ch4_adjust = _DATA_["adjust154"]
            ch4_adjust = min(max_adjust,max(-max_adjust,damp*ch4_adjust))
            newOffset0 = _FREQ_CONV_.getWlmOffset(6) + ch4_adjust
            _NEW_DATA_["wlm6_offset"] = newOffset0
            _PERSISTENT_["wlm6_offset"] = newOffset0
            _FREQ_CONV_.setWlmOffset(6,float(newOffset0))
            # print "New (12)CH4(virtual laser 6) offset: %.5f" % newOffset0
        except:
            pass
            # print "No new (13)CH4 (virtual laser 6) offset"

        # Update the offset for virtual laser 7
        try:
            ch4_adjust = _DATA_["adjust154"]
            ch4_adjust = min(max_adjust,max(-max_adjust,damp*ch4_adjust))
            ...
            # print "New (12)CH4(virtual laser 7) offset: %.5f" % newOffset0
        except:
            pass
            # print "No new (13)CH4 (virtual laser 7) offset"

        # Update the offset for virtual laser 8
        try:
            ch4_adjust = _DATA_["adjust154"]
            ch4_adjust = min(max_adjust,max(-max_adjust,damp*ch4_adjust))
            newOffset0 = _FREQ_CONV_.getWlmOffset(8) + ch4_adjust
            _NEW_DATA_["wlm8_offset"] = newOffset0
            _PERSISTENT_["wlm8_offset"] = newOffset0
            _FREQ_CONV_.setWlmOffset(8,float(newOffset0))
            # print "New (12)CH4(virtual laser 8) offset: %.5f" % newOffset0
        except:
            pass
            # print "No new (13)CH4 (virtual laser 8) offset"
