...
class ProportionalMode_Flux(SampleManagerBaseMode):
  ...
  def RPC_FlowStart(self):
    ...
    self._clearStatus()
    # Additional steps for Flux Analyzer
    self._LPC_StopValveControl()
    self._LPC_CloseSolenoidValves()
    basePressure = self.base_pressure
    outletValveTarget = self.outlet_valve_target
    self._DriverRpc.startOutletValveControl(basePressure, 0)
    if self._RPC_ReadPressure() > basePressure:
        status = self._LPC_WaitPressureStabilize( basePressure, tolerance=0.1, timeout=600, checkInterval=1, lockCount=1 )
        if status==False:
          print "Pressure failed to stabilize."
          return False

    # Step valve to target value (valve stepped is opposite of valve controlled )
    #self._LPC_SetValveControl("VALVE_CNTRL_InletControlState")
    ...
    self._LPC_SetValve(OUTLETVALVE, outletValveTarget)
    self._LPC_OpenSolenoidValves("6")
    self._DriverRpc.startInletValveControl(self.operate_pressure_sp_torr, outletValveTarget)
    status = self._LPC_WaitPressureStabilize( self.operate_pressure_sp_torr, tolerance=0.02, timeout=600, checkInterval=1, lockCount=5 )
    if status==False:
      print "Pressure failed to stabilize."
      return False
  ...
  def RPC_FlowStop(self):
    """STOP FLOW"""
    # Close all solenoid valves first
    self._LPC_CloseSolenoidValves()
