...
#  2011-12-14:  Implement new version of CO spectroscopy from Chris Rella with dry mole fraction calculation
...
if not good:
    ...
    if _DATA_["species"] == 1: # Update the offset for virtual laser 1
        try:
            ...
            pass
    elif _DATA_["species"] == 4: # Update the offset for virtual laser 2
        try:
            adjust = _DATA_["co_adjust"]
            adjust = min(max_adjust,max(-max_adjust,adjust))
...
try:
    peak84_offset = _DATA_["peak84_raw"] + _INSTR_["co_offset"]
    co2_p14 = _OLD_DATA_["peak_14"][-1].value*0.706630873    #  Should we use co2_conc?  This variable is meant to be [CO2] in ppm
    peak84_spec_wet = peak84_offset + _INSTR_["co_water_linear"]*_DATA_["b_h2o_pct"] + _INSTR_["co_water_co2"]*_DATA_["b_h2o_pct"]*co2_p14 + _INSTR_["co_water_quadratic"]*_DATA_["b_h2o_pct"]**2 + _INSTR_["co_co2_linear"]*co2_p14
    if ver < 1.7:
        peak84_spec = peak84_spec_wet
    else:
        peak84_spec = peak84_spec_wet/(1.0+_INSTR_["co_wd_linear"]*_DATA_["b_h2o_pct"]+_INSTR_["co_wd_quadratic"]*_DATA_["b_h2o_pct"]**2)
    co_conc = applyLinear(peak84_spec,CO)
    _NEW_DATA_["co_conc"] = co_conc
except:
    _NEW_DATA_["co_conc"] = 0.0

try:
    h2o_reported = applyLinear(_DATA_["b_h2o_pct"],H2O) #error fixed. changed from h2o_pct to b_h2o_pct. This affects CKADS only.
    h2o_precorr = 0.5*(h2o_reported+_OLD_DATA_["h2o_reported"][-2].value)  # average of current and last uncorrected h2o measurements

    try:
        co2_conc_dry = _OLD_DATA_["co2_conc"][-1].value/(1.0+h2o_precorr*(_INSTR_["co2_watercorrection_linear"]+h2o_precorr*_INSTR_["co2_watercorrection_quadratic"]+h2o_precorr**2*_INSTR_["co2_watercorrection_cubic"]))
        _NEW_DATA_["co2_conc_dry"] = co2_conc_dry
    except:
        _NEW_DATA_["co2_conc_dry"] = 0.0

    try:
        h2o_actual = _INSTR_["h2o_selfbroadening_linear"]*(h2o_reported+_INSTR_["h2o_selfbroadening_quadratic"]*h2o_reported**2+_INSTR_["h2o_selfbroadening_cubic"]*h2o_reported**3)
