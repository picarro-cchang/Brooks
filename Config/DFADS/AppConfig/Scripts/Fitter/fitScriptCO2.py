...
d.sparse(maxPoints=1000,width=0.003,height=100000.0,xColumn="waveNumber",yColumn="uncorrectedAbsorbance",sigmaThreshold=2)
...
tstart = time.clock()
if (d["spectrumId"] in [10,12]) and (d["ngroups"] > 10):
...
#  Quadratic fit to peak if it is larger than 200 ppb/cm
    if co2_peak_baseavg > 200:
        ...
        d.sparse(maxPoints=1000,width=0.00025,height=100000.0,xColumn="waveNumber",yColumn="uncorrectedAbsorbance",sigmaThreshold=2.5)
        d.evaluateGroups(["waveNumber","uncorrectedAbsorbance"])
        d.defineFitData(freq=d.groupMeans["waveNumber"],loss=1000*d.groupMeans["uncorrectedAbsorbance"],sdev=1/sqrt(d.groupSizes))
    ...
    RESULT = {"co2_res":co2_res,"galpeak14_final":galpeak14_final,"co2_str":str_14,"vpeak_14":vpeak_14,"vy_14":vy_14,
        ...
        "co2_numPoints":d["datapoints"],"co2_ngroups":d["ngroups"],
