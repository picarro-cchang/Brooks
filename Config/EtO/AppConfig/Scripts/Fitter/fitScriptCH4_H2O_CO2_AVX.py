...
class BroadbandControl (object):
    ...
    def setPZTValue(self, param=None,schemeInfo={}):
        thisScheme=schemeInfo['schemeName']
        ...
        if self.pztMode == 6: #shift PZT by scheme
            if thisScheme != self.lastScheme:
                ...
                if self.verbose: print 'Duplicate PZT setpoint ignored'
        self.lastScheme=thisScheme
...
class AuxControl(object):
    ...
    def getSetState(self, scheme_info):
        if self.key in scheme_info.keys():
            states = scheme_info[self.key]
            N = len(states)
            j = self.iteration % N
            self.iteration += 1
            return j, states[j]
...
class MFC_Control(AuxControl): #individualMFC control
    def setup (self, **kwargs):
    ...
    def read (self, verbose=False):
        if self.MFCfound:
            res1 = self.MFC.sendAndParse(self.MFC.query)          # read MFC and valve states
            time.sleep(0.1)
            self.results = dict(res1.items())
            if verbose: print self.results
        
    def set(self, scheme_info, verbose=False):
        if self.MFCfound:
            j, flow = self.getSetState(scheme_info)
            if j is not None:
                if verbose: print 'step %d: setting %s flow to %.3f' % (j, self.MFC.nickname, flow)
                command1 = '%sS%.3f' % (self.MFC.query,flow)
                self.MFC.sendAndParse(command1)

class MultiMFC(object): 
    def __init__(self, configfn):
        self.configfn = configfn
        self.connect()
        
    def connect(self):
        cfg = ConfigObj(self.configfn)
        print cfg
        print os.path.exists(self.configfn)
        print self.configfn
        flowinfo = cfg['flowControl']
        self.numMFCs = flowinfo.as_int('numMFCs')
        
        self.MFCs = []
        for n in range(self.numMFCs):
            prefix = 'mfc%d_' % (n+1)
            mx = flowinfo.as_float(prefix + 'max')
            name = flowinfo[prefix + 'name']
            addr = flowinfo[prefix + 'addr']
            if n == 0:
                mfc = RS232.device(dtype='Alicat MFC', name='Alicat %d sccm' % mx, 
                                   query=addr, response='%s +' % addr, nickname=name)
            else:
                mfc = RS232.multiAlicat(port=self.MFCs[0].MFC.port, serialDevice=self.MFCs[0].MFC.serialDevice,
                                        dtype='Alicat MFC', name='Alicat %d sccm' % mx,
                                        query=addr, response='%s +' % addr, nickname=name)
            mfcFound = not(mfc.port is None)
            self.MFCs.append(MFC_Control(name,MFCfound=mfcFound, MFC=mfc))    
            

    def read(self, verbose=False):
        self.results = {}
        for m, mfc in enumerate(self.MFCs):
            if m != 0:
                time.sleep(0.1)
            if mfc.MFCfound:
                self.results.update(mfc.MFC.sendAndParse(mfc.MFC.query))
        if verbose: print self.results
        
    def set(self, scheme_info, verbose=False):
        for m, mfc in enumerate(self.MFCs):
            if mfc.MFCfound:
                # setFlowList is a list of flow setpoints for the MFCs.  The i_th element is set, where i = self.iteration % len(setFlowList)
                j, flow = mfc.getSetState(scheme_info)
                if j is not None:
                    if verbose: print 'step %d: setting %s flow to %.3f' % (j, mfc.MFC.nickname, flow)
                    if m != 0:
                        time.sleep(0.1)
                    command = '%sS%.3f' % (mfc.MFC.query,flow)
                    mfc.MFC.sendAndParse(command)
                    
class MPVControl(AuxControl): 
    def setup(self, **kwargs):
        
        self.mpv = RS232.device(dtype='Valco MPV',query='VR', response='I-PD-AMTX88RD1', nickname='MPV_Valco')

        self.MPVfound = not(self.mpv.port is None)

        
    def read(self, verbose=False):
        if self.MPVfound:
            res4 = self.mpv.sendAndParse('CP')          # read MPV state
            print res4
            time.sleep(0.1)
            self.results = dict(res4.items())
            if verbose: print self.results
        
    def set(self, scheme_info, verbose=False):
        if self.MPVfound:
            j, valve = self.getSetState(scheme_info)
            print j, valve
            print scheme_info
            if j is not None:
                if verbose: print 'Moving to the next gas' 
                command = 'GO%d' % valve
                print command
                self.mpv.sendCommand(command, readback = True)
                   
                
class SolenoidValveControl(AuxControl):
    def read(self, verbose=False):
        vm = Driver.getValveMask()
        self.results = {'valveMask':vm}
        if verbose: print self.results
    
    def set(self, scheme_info, verbose=False):
        j, mask = self.getSetState(scheme_info)
        if j is not None:
            if verbose: print 'Setting Valve Mask to %d' % mask
            Driver.setValveMask(mask)
            
class PressureSetpointControl(AuxControl):
    def read(self, verbose=False):
        self.results = {'currentPressureSetPoint':Driver.rdDasReg("VALVE_CNTRL_CAVITY_PRESSURE_SETPOINT_REGISTER")}
        if verbose: print self.results
        
    def set(self, scheme_info, verbose=False):
        j, setpoint = self.getSetState(scheme_info)
        if j is not None:
            try:
                Driver.wrDasReg("VALVE_CNTRL_CAVITY_PRESSURE_SETPOINT_REGISTER", setpoint)
                if verbose: print 'Setting Cavity Pressure Setpoint to %.3f' % setpoint
            except:
                if verbose: print 'Unable to set Cavity Setpoint to %.3f' % setpoint
...
if INIT:
    ...
    expt_fn = os.path.join(BASEPATH,r"../../Schemes/BroadBand/CurrentExperimentDefinition.ini")
    FlowGuy = MultiMFC(expt_fn)
    MPVGuy = MPVControl('setMPV')
    ValveGuy = SolenoidValveControl('setSolenoidValve')
    PressureGuy = PressureSetpointControl('setCavityPressure')
    ...
    fsr_ch4_res = 0.0
    fsr_vch4_conc_ppmv = 0.0
    fsr_ch4_vy = 0.0
    fsr_ch4_adjconc_ppmv = 0.0
    fsr_ch4_conc_ppmv_final = 0.0
    fsr_base = 0.0
    fsr_splinemax = 0.0
    fsr_ch4_shift = 0.0
    fsr_ch4_adjust = 0.0
...
BroadbandLocker.setPZTValue(schemeInfo=SCHEME_INFO)  #sets to scheme value, only for pztMode = 5
BroadbandLocker.adjustPZTtoNuTarget(param=[0.75,10000])

FlowGuy.read()
ValveGuy.read()
PressureGuy.read()
MPVGuy.read(verbose = True)

if SCHEME_INFO is not None:
    FlowGuy.set(SCHEME_INFO, verbose=True)
    ValveGuy.set(SCHEME_INFO, verbose=True)
    PressureGuy.set(SCHEME_INFO, verbose=True)
    MPVGuy.set(SCHEME_INFO, verbose=True)
...
goodCH4 = sum((d.groupMeans["waveNumber"] < 6057.39) & (d.groupMeans["waveNumber"] > 6056.7))
...
#  Polishing step with fixed center and y if line is weak or shift is large
    ...
    #FSR fit with comb as frequency axis
    peakLossnu_ch4 = 6057.08931 #determined from CFADS spline
    ...
    ANALYSIS.append(r)
    fsr_ch4_vy = r[1002,5]
    fsr_vch4_conc_ppmv = 10*r[1002,2]
    fsr_ch4_shift = r["base",3]
#  Polishing step with fixed center, variable y if line is strong and shift is small
    if (r[1002,2] > 0.005) and (abs(fsr_ch4_shift) <= 0.07):
        init["base",3] = fsr_ch4_shift
        fsr_ch4_adjust = fsr_ch4_shift
        r = anCH4[1](d,init,deps)
        ANALYSIS.append(r)
#  Polishing step with fixed center and y if line is weak or shift is large
    else:
    ...
    fsr_ch4_amp = r[1002,2]
    fsr_ch4_conc_raw = 10*r[1002,2]
    fsr_ch4_y = r[1002,5]
    fsr_base = r["base",0]
    fsr_ch4_adjconc_ppmv = fsr_ch4_y*fsr_ch4_conc_raw*(140.0/P)
    fsr_splinemax = r[1002,"peak"]
    fsr_ch4_peakvalue = fsr_splinemax+fsr_base
    fsr_base_avg  = initExpAverage(fsr_base_avg,fsr_base,1,1000,counter)
    fsr_ch4_peak_baseavg = fsr_ch4_peakvalue-fsr_base_avg
    fsr_ch4_conc_ppmv_final = fsr_ch4_peak_baseavg/216.3
    fsr_ch4_res = r["std_dev_res"]

    BroadbandLocker.adjustNuTarget(param=[0.1, 1e-5, fsr_ch4_adjust, peakLossnu_ch4]) 
    BroadbandLocker.adjustPZTtoNuSpectrum(param=[0.3,1000, fsr_ch4_adjust, peakLossnu_ch4])
    old_wlm_offset, wlm_offset = BroadbandLocker.adjustWLM_OFFSET(param=[0.05, 2e-3, ch4_adjust, peakLossnu_ch4, 1])
...
if r != None:
    try:
        ...
        RESULT.update({"fsr_ch4_res":fsr_ch4_res,"fsr_vch4_conc_ppmv":fsr_vch4_conc_ppmv,"fsr_ch4_vy":fsr_ch4_vy,
                       "fsr_ch4_adjconc_ppmv":fsr_ch4_adjconc_ppmv,"fsr_ch4_conc_ppmv_final":fsr_ch4_conc_ppmv_final,
                       "fsr_ch4_base":fsr_base,"fsr_ch4_base_avg":fsr_base_avg,"fsr_ch4_splinemax":fsr_splinemax,
                       "fsr_ch4_shift":fsr_ch4_shift,"fsr_ch4_adjust":fsr_ch4_adjust,"fsr_ch4_y":fsr_ch4_y})
    ...
    RESULT.update(FlowGuy.results)
    RESULT.update(ValveGuy.results)
    RESULT.update(PressureGuy.results)
    RESULT.update(MPVGuy.results)
    #for key in RESULT:
