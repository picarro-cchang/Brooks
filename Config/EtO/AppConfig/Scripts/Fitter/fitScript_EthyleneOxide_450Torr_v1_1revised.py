...
if INIT:
    fname = os.path.join(BASEPATH,r"./ETO_450Torr/Spectral library for ETO 6080 wvn 80 C 450 Torr v1.ini")
    ...
    #fsr *= 0.9999  # ad hoc transformation from 140 Torr to 450 Torr
    ...
    anH2O = []
    anH2O.append(Analysis(os.path.join(BASEPATH,r"./ETO_450Torr/H2O_6080_VCVY_v2.ini")))
    anH2O.append(Analysis(os.path.join(BASEPATH,r"./ETO_450Torr/H2O_6080_FCFY_v2.ini")))
    
    anCH4 = []
    anCH4.append(Analysis(os.path.join(BASEPATH,r"./ETO_450Torr/CH4_R6_VCVY_v2.ini")))
    anCH4.append(Analysis(os.path.join(BASEPATH,r"./ETO_450Torr/CH4_R6_FCFY_v2.ini")))
    anCH4.append(Analysis(os.path.join(BASEPATH,r"./ETO_450Torr/CH4_R6_VCFY_v2.ini")))
    anCH4.append(Analysis(os.path.join(BASEPATH,r"./ETO_450Torr/CH4_R6_VCFY2_v2.ini")))
    
    anCO2 = []
    anCO2.append(Analysis(os.path.join(BASEPATH,r"./ETO_450Torr/CO2_R6_VCFY_v2.ini")))
    anCO2.append(Analysis(os.path.join(BASEPATH,r"./ETO_450Torr/CO2_R6_FCFY_v2.ini")))
    
    anETO = []
    anETO.append(Analysis(os.path.join(BASEPATH,r"./ETO_450Torr/ETOwide_v8_FCFY.ini")))
...
#if (d["spectrumId"]==181) and (d["datapoints"]>500):
if (d["spectrumId"]==181) and (d["datapoints"]>450):
    pzt = mean(d.pztValue)
    ...
    init[1002,5] = ch4_y
    init[1003,2] = 0.0004603*str2
    init[13,"strength"] = 0.003280*str2               # From water step test 11. Sep 2019
    init[26,"y"] = 5.8575 + 0.01864*0.0004603*str2    # From water step test 11. Sep 2019 with croos broadening scaled by 450/140
    r = anCO2[0](d,init,deps)
    ANALYSIS.append(r)
    co2_shift = r["base",3]
    if r[26,"peak"] < 30 or abs(co2_shift) > 0.05 or (r[26,"y"]/5.8575) < 0.75 or (r[26,"y"]/5.8575) > 1.25:
    ...
    if peak4 > 30:
        wlm_adjust = h2o_adjust #% fsr 
        pzt_adjust = -wlm_adjust*pzt_per_fsr/fsr #-(f0 + h2o_adjust - f_eto)*pzt_per_fsr/fsr
        init_base3 = h2o_adjust
    elif peak26 > 30: 
        wlm_adjust = co2_adjust #% fsr
        pzt_adjust = -wlm_adjust*pzt_per_fsr/fsr #-(f0 + co2_adjust - f_eto)*pzt_per_fsr/fsr
        init_base3 = co2_adjust        
    elif ch4_ppm > 0.03: 
        wlm_adjust = ch4_adjust #% fsr
        pzt_adjust = -wlm_adjust*pzt_per_fsr/fsr #-(f0 + ch4_adjust - f_eto)*pzt_per_fsr/fsr
        init_base3 = ch4_adjust        

    if abs(pzt_adjust)>= pzt_per_fsr/2 :
        pzt_adjust /= 2
    init["base",3] = init_base3 
    #pzt_adjust = 0.0
    ...
    init[2,"y"] = y2
    init[3,"y"] = 4.4359 + 0.1695*h2o_pct
    ...
    base1 = r["base",1]
    base2 = r["base",2] 
    #c2h4_ppb = 10000*r[1007,2]       #  These splines do not exist for 450 Torr at start of October 2019
...
if not IgnoreThis:    
    RESULT = {"pzt_adjust":pzt_adjust,"pzt":pzt,"wlm_adjust":wlm_adjust,"tiptop":d.fitData["loss"][i0],
