...
#  2020 0110:  Slight change to methane adjust criterion to allow correction at lower concentration
#              Add branch to variable centration EtO fit for special case of span gas
#  2020 0113:  V1.10 adds ethylene to the smalll-molecule fits for highly contaminated samples
#  2020 0114:  Also changed pzt control to adjust mode position when reference lines are absent
...
if INIT:
    ...
    anCO2.append(Analysis(os.path.join(BASEPATH,r"./ETO/CO2_R6_VCVY_Pcal.ini")))

    anETO = []
    anETO.append(Analysis(os.path.join(BASEPATH,r"./ETO/ETOwide_v8_FCFY.ini")))
    anETO.append(Analysis(os.path.join(BASEPATH,r"./ETO/ETOwide_v8_VCFY.ini")))
    
#   Globals
    last_time = None
    f_eto = 6080.0630    #  Target for "0th mode" frequency
    f_co2_r26 = 6081.368008
...
i0 = argmin(abs(d.fitData["freq"] - f_co2_r26))
f0 = d.fitData["freq"][i0]
while (f0 - f_co2_r26) < -0.5*fsr:
    f0 += fsr
while (f0 - f_co2_r26) > 0.5*fsr:
    f0 -= fsr
...
mean_Pcavity = mean(d.cavityPressure)
std_Pcavity = std(d.cavityPressure)
tunerMean = mean(d.tunerValue)
tunerMeanstd = std(d.tunerValue)
pztMean = mean(d.pztValue)
pztMeanstd = std(d.pztValue)
...
if d["spectrumId"]==181:
    ...
    ch4_shift = r["base",3]
    ch4_vy = r[1002,5]
    if (r[1002,2] < 0.002 and r[2,"peak"] < 10 and r[20,"peak"] < 10) or abs(ch4_shift) > 0.05:
        r = anCH4[1](d,init,deps)
        ANALYSIS.append(r)
    elif (r[1002,2] < 0.002 or ch4_vy < 0.5 or ch4_vy > 1.5) and (r[2,"peak"] > 10 and r[2,"y"] > 0.5 and r[2,"y"] < 2):
    ...
    ANALYSIS.append(r)
    co2_shift = r["base",3]
    if r[26,"peak"] < 2 or abs(co2_shift) > 0.05 or (r[26,"y"]/(1.8345/140.0*mean_Pcavity)) < 0.75 or (r[26,"y"]/1.8345) > 1.25:
    ...
    pzt_adjust = -(f0 + wlm_adjust - f_co2_r26)*pzt_per_fsr/fsr        
    ...
    ANALYSIS.append(r)
    if r[1004,2] > 0.1 and r["std_dev_res"] < 0.2:    #  Added for special case of pure EtO span gas
        r = anETO[1](d,init,deps)
        ANALYSIS.append(r)
...
if not IgnoreThis:    
   RESULT = {"pzt_adjust":pzt_adjust,"pzt":pzt,"wlm_adjust":wlm_adjust,"tiptop":d.fitData["loss"][i0],"badGroups":badGroups,
           "Pcavity_mean":mean_Pcavity,"y26abs":y26abs,
           ...
           "co2_res":co2_residuals,"co2_str":str26,
           "y_parameter":abs(y26),"co2_baseline":base0,"co2_baseline_slope":base1,
           "co2_base":base26,"freq_offset":co2_adjust,"co2_peak":peak26,
           "co2_tuner_mean":tunerMean,"co2_tuner_std": tunerMeanstd,
           "co2_pzt_mean":pztMean,"co2_pzt_std":pztMeanstd,
           "co2_fittime":fit_time,#"species":1,
           "cavity_pressure":mean_Pcavity,"cavity_temperature":T,"solenoid_valves":solValves,
           "das_temp":dasTemp}
