...
if _PERSISTENT_["init"]:
    ...
    # pass### "ChemDetect_FileName = ", ChemDetect_FileName
...
if conc == "low":
    if _PERSISTENT_["state"] == "high_conc":
        try:
            _SPEC_COLL_.setSequence("FBDS_mode_LOW_CH4")
    ...
    if _12_ch4_raw > 0.05:
        if _12_ch4_raw > CH4_HIGH_THRESHOLD:
            if _PERSISTENT_["state"] == "low_conc":
                ...
                pass### "Switching to HIGH_CONC mode"
                try:
                    _SPEC_COLL_.setSequence("FBDS_mode")
                    # _DRIVER_.setMultipleNoRepeatScan()
                    pass### "No exception when switching to HIGH_CONC mode"
                except: # For Virtual analyzer
                    pass### "Exception when switching to HIGH_CONC mode"
        elif _12_ch4_raw < CH4_LOW_THRESHOLD:
            if _PERSISTENT_["state"] == "high_conc":
                # Switch to low  concentration schemes
                pass### "Switching to LOW_CONC mode"
                try:
                    _SPEC_COLL_.setSequence("FBDS_mode_LOW_CH4")
                    # _DRIVER_.setMultipleNoRepeatScan()
                except: # For Virtual analyzer
...
try:
    ...
    pass### "Error %s (%r)" % (e,e) #For VA: Errors happened are "deque index out of range" (due to IF line) (probably because there are no '-3' and '-2'
...
if not good:
    ...
    #pass### _PERSISTENT_["plot_iCH4"]
        ...
        try:
            ...
            #pass### "New CO2 (virtual laser 6) offset: %.5f" % newOffset0
        except:
            pass
            pass### "No new CO2 (virtual laser 6) offset"
...
# if _OLD_DATA_["species"][-1].value == 150:
    ...
    # if _PERSISTENT_["chemdetect_inst"].current_var_values['RED'] == True:
        # pass### "WARNING: ChemDetect Status is RED"

if _DATA_["species"] in TARGET_SPECIES and (_PERSISTENT_["plot_iCH4"] or _PERSISTENT_["plot_iCO2"]):
    if _OLD_DATA_["species"][-1].value == 150:
        pass### ' '
        pass### '12CH4_high_range = ', _NEW_DATA_["12CH4_high_range"]
        pass### 'ChemDetect: NOTOK_HC_res2         = ', _PERSISTENT_["chemdetect_inst"].current_var_values['NOTOK_HC_res2']
#   pass### 'ChemDetect: NOTOK_HC_base_offset2 = ', _PERSISTENT_["chemdetect_inst"].current_var_values['NOTOK_HC_base_offset2']
#   pass### 'ChemDetect: NOTOK_HC_slope_offset= ', _PERSISTENT_["chemdetect_inst"].current_var_values['NOTOK_HC_slope_offset']
    ...
    # pass### "_REPORT_[ChemDetect] = ", _REPORT_["ChemDetect"]
    ...
    for name in pztNames:
        try:
            if _DATA_["pzt_%s_stdev" % name] > PZT_STD_DEV_LIMIT:
                _REPORT_['ALARM_STATUS'] |= ALARM_PZT_STD_MASK
                break
        except KeyError:
            pass### "No '%s' PZT std dev in fitter results" % name

    shifts = ['shift_0', 'shift_5', 'shift_24', 'shift_30', 'ch4_high_shift']
    for s in shifts:
        if abs(_DATA_[s]) > SHIFT_ABS_LIMIT:
            _REPORT_['ALARM_STATUS'] |= ALARM_SHIFT_MASK
            break
    ...
    for offset in wlmOffsets:
        ...
        if abs(_PERSISTENT_[offset]) > OFFSET_ABS_LIMIT:
            _REPORT_['ALARM_STATUS'] |= ALARM_WLM_OFFSET_MASK
            break
