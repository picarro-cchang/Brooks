...
def boxAverage(buffer,x,t,tau):
    ...
    while t-buffer[0][1] > tau:
        buffer.pop(0)
    return np.mean([d for (d,t) in buffer])
...
if _PERSISTENT_["init"]:
    ...
    _PERSISTENT_['outletValve_min'] = _DRIVER_.rdDasReg('VALVE_CNTRL_OUTLET_VALVE_MIN_REGISTER')
    _PERSISTENT_['outletValve_max'] = _DRIVER_.rdDasReg('VALVE_CNTRL_OUTLET_VALVE_MAX_REGISTER')
