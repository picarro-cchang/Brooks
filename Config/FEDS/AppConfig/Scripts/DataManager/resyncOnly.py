...
def resync(dataLabel):
    ...
    try:
        while _OLD_DATA_[dataLabel][index].time > scriptTime - setLag: 
            index -= 1
        #pass### "index = %d, time since last fit = %f" % (index,scriptTime-_OLD_DATA_[dataLabel][-1].time)
        if index < -1:
            result = linInterp(_OLD_DATA_[dataLabel][index],_OLD_DATA_[dataLabel][index+1],scriptTime-setLag)
            syncStatus = True
    except:
        pass
    result = applyLinear(result,_USER_CAL_[dataLabel])
...
if "CO2" in scanningMode:
    allGood = allGood and resync("co2_conc")
if "CH4" in scanningMode:
    allGood = allGood and resync("ch4_conc")
if "H2O" in scanningMode:
    allGood = allGood and resync("h2o_conc")
...
if allGood:
    ...
    _REPORT_["species"] = _OLD_DATA_["species"][-1].value
    _REPORT_["solenoid_valves"] = _OLD_DATA_["solenoid_valves"][-1].value
    _REPORT_["das_temp"] = _OLD_DATA_["das_temp"][-1].value
    _REPORT_["cavity_pressure"] = _OLD_DATA_["cavity_pressure"][-1].value