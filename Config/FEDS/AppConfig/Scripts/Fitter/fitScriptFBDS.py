...
#  29 Jan 2014:  Fixed minor bug in logic for adjust_0 = 0 and adjust_5 = 0.
...
if any(cal):
    pztAves = {}
    pztStdevs = {}
    actualVirtualLaser = (d.laserUsed >> 2) & 0x7
    for n in virtualLasers.keys():
        vl = ((actualVirtualLaser == int(n)) & cal)

        aveKey = "pzt_%s_ave" % virtualLasers[n]
        stdevKey = "pzt_%s_stdev" % virtualLasers[n]
        pztVals = d.pztValue[vl]
        if len(pztVals) != 0:
            pztAves.update({aveKey : mean(d.pztValue[vl])})
            pztStdevs.update({stdevKey : std(d.pztValue[vl])})
...
if not IgnoreThis:
    ...
    RESULT.update(pztAves)
    RESULT.update(pztStdevs)
