...
#  22 Jun 2011:  Changed logic of water region fit to use methane peak for centration when methane is strong and water is weak
#  28 Jun 2011:  Added a constrain for adjust88 update: (ch4_conc_diff <= 5) -- YH
...
if species == 105:  # C12O2
    ...
    #C12H4_conc_raw = 0.40148*peak_0/0.907717
    ...
    if (peak_30 > 2) and (abs(shift_30) < 0.04) and (abs(vy_30-1.09) < 0.4):
        ...
        adjust_30 = shift_30
    elif (r[1002,2] > 0.06) and (abs(shift_30) < 0.04):
        #  If water is weak but methane is strong, use methane centration and fixed y
        adjust_30 = shift_30
        init["base",3] = shift_30
        r = anH2O[1](d,init,deps)
        ANALYSIS.append(r)
    else:
        #  If water and methane are both weak, use FY at nominal frequency to estimate water 
        adjust_30 = 0.0
        r = anH2O[1](d,init,deps)
        ANALYSIS.append(r)
