...
class SurveyorFlowMode(SampleManagerBaseMode):
  ...
  def RPC_FlowStart(self):
    ...
    flowSensorPresent = (0 != (self._RPC_ReadHardwarePresent() & (1<<interface.HARDWARE_PRESENT_FlowSensorBit)))
    print "flowSensorPresent", flowSensorPresent

    if flowSensorPresent:
        self._LPC_SetFlowControl(interface.FLOW_CNTRL_EnabledState)
    else:
