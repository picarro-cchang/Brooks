...
if INIT:
    fname = os.path.join(BASEPATH,r"./isoH2O7000/spectral library v1_1.ini")
    ...
    anH2O = []
    anH2O.append(Analysis(os.path.join(BASEPATH,r"./isoH2O7000/Full_range.ini")))
...
spectrum = (d.subschemeId & 0x3FF)[0]
...
if ((spectrum == 123) or (spectrum == 124) or (spectrum == 0)) and (d["ngroups"] > 9):
    initialize_Baseline()
    ...
    if peak2 > 10 and abs(h2o_shift) < 0.05 and abs(h2o_y-y2_nominal) < 0.25:
...
if not IgnoreThis:    
    RESULT = {"peak1":peak1,"peak2":peak2,"peak3":peak3,"h2o_y":h2o_y,
            ...
            "raw18":raw_18,"raw_D":raw_D,"ngroups":d["ngroups"],
            "spectrum":spectrum,"interval":interval,"fit_time":fit_time
            }
