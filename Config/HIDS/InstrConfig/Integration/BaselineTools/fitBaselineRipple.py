#  Fit script for isotopic water baseline at 7200 wvn
#  2013 1203:  Modifications to bring sinusoid reporting up to date, try to handle range of [H2O]
...
if INIT:
    fname = os.path.join(BASEPATH,r"./spectral library v3_1.ini")
    loadSpectralLibrary(fname)
    loadPhysicalConstants(fname)
    loadSplineLibrary(fname)

    analysis = []
    analysis.append(Analysis(os.path.join(BASEPATH,r"./HIDS-xx baseline ripple v2_1.ini")))
    analysis.append(Analysis(os.path.join(BASEPATH,r"./HIDS-xx baseline ripple v2_2.ini")))
...
r = analysis[0](d,init,deps)                   #  This fit adjusts frequency offset early,
ANALYSIS.append(r)                             #  appropriate for strong water lines

if r[2,"peak"] < 2 or abs(r["base",3]) > 0.1:
    r = analysis[1](d,init,deps)               #  This fit adjusts frequency offset after sinusoids,
    ANALYSIS.append(r)                         #  appropriate for weak water lines
