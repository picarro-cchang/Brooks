...
if INIT:
    ...
    anH2O.append(Analysis(os.path.join(BASEPATH,r"./isoH2O7000/Full_range_FC_FY_v1_1.ini")))
...
#   Parameters used in the fits -- only valid for operation with AIR carrier (11 May 2011)
    ...
    G1_H2 = instrParams['G1_quadratic']
    G2_H2 = instrParams['G2_quadratic']
    G3_H2 = instrParams['G3_quadratic']
...
#  Preset center and ys for second stage fit
    ...
    if peak2 > 10 and abs(h2o_shift) < 0.05 and abs(h2o_vy-y2_nominal) < 0.25:
        init[1,"y"] = 0.3611+(1.16e-5)*peak2
        init[2,"y"] = 0.4032+(1.30e-5)*peak2
        init[3,"y"] = 0.2181+(1.44e-5)*peak2
...
#   Spectroscopic adjustments
    peak1_offset = peak1+offset1+G1_H2*peak2**2
    peak2_offset = peak2+offset2+G2_H2*peak2**2
    peak3_offset = peak3+offset3+G3_H2*peak2**2
