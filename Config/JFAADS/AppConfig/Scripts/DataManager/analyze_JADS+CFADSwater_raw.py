...
try: # new ChemDetect section    
    ...
    try:
        ...
        _NEW_DATA_["res2_a1"] = (_NEW_DATA_["res2"] - a0_res2) / n2o_raw
...
if not good:
    ...
    if _DATA_["species"] == 11: # Update the offset for virtual laser 7
        try:
            ch4_adjust = n2o_gain*_DATA_["adjust_75"]
            #ch4_adjust = 0.0
...
if _DATA_["species"] in TARGET_SPECIES:
    if _OLD_DATA_["species"][-1].value == 47:
      ...
      if _PERSISTENT_["chemdetect_inst"].current_var_values['RED'] == True:
        _NEW_DATA_["ChemDetect"] = 1.0  # BAD data
      else:
        _NEW_DATA_["ChemDetect"] = 0.0  # Good data 
    else:
      _NEW_DATA_["ChemDetect"] = _PERSISTENT_["ChemDetect_previous"]
