...
# if _DATA_["species"] in TARGET_SPECIES:
    ...
    try: # new ChemDetect section    
        ...
        res2_fit = a0_res2 + _REPORT_["N2O_raw"] * a1_res2  # changed from _NEW_DATA_
        _NEW_DATA_["res2_diff"] = _NEW_DATA_["res2"] - res2_fit
        _NEW_DATA_["res2_a1"] = (_NEW_DATA_["res2"] - a0_res2) / _REPORT_["N2O_raw"]  # changed from _NEW_DATA_    
    except:
        pass
...
if _DATA_["species"] in TARGET_SPECIES:
    if _OLD_DATA_["species"][-1].value == 47:
      print ' '
      print 'N2O_raw = ', _REPORT_["N2O_raw"]  # changed from _NEW_DATA_
