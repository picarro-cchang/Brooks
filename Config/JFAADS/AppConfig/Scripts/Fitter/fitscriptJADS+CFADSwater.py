...
#   9 Aug 2013:  Added CFADS water fit for water concentration calibration test
...
if INIT:
    fname = os.path.join(BASEPATH,r"./JADS/JADS spectral library v3_7.ini")
...
if d["spectrumId"]==11 and d["ngroups"]>5:
    ...
    cal = (d.subschemeId & 4096) != 0
    if any(cal):
        h2o_pzt_mean = mean(d.pztValue[cal])
        h2o_pzt_stdev = std(d.pztValue[cal])    
...
if not IgnoreThis:    
    RESULT = {"res_a":res_a,"res_11":res_11,"res_12":res_12,"badshot_nh3":badshot_nh3,
              ...
              "y_75":y_75,"base_75":base_75,"shift_75":shift_75,"adjust_75":adjust_75,
              ...
              "h2o_pzt_mean":h2o_pzt_mean,"h2o_pzt_stdev":h2o_pzt_stdev,
