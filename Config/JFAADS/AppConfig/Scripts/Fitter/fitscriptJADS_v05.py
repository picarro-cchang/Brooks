...
#  SIDs:
#    2  Ammonia and water from AADS
#    3  "Big water" from AADS (not used)
#    4  Ammonia only from AADS
#   25  CH4 from CFADS
#   45  N2O only
...
if INIT:
    fname = os.path.join(BASEPATH,r"./JADS/JADS spectral library v3_2.ini")
...
if (species==1 or species==2) and d["ngroups"]>18:
...
if not IgnoreThis:    
    RESULT = {"res_a":res_a,"res_11":res_11,"res_12":res_12,"badshot_nh3":badshot_nh3,
              ...
              "peak_41":peak_41,"peak41_spec":peak41_spec,"co2_conc_6058":co2_conc_6058,
