...
#  21 Jun 2013:  Added preset y-parameter to HDO lines in the N2O region.  Adjusted HDO line shape parameters.
#  24 Jun 2013:  Added preset frequency shift to HDO lines in N2O region.  Based on CO2+variable H2O data set.
...
if INIT:
    fname = os.path.join(BASEPATH,r"./JADS/JADS spectral library v3_5.ini")
    ...
    anN2O = []
    anN2O.append(Analysis(os.path.join(BASEPATH,r"./JADS/N2O+CO2+HDO+CH4_VC_FY_v2_1.ini")))
...
if (species == 47 and d["ngroups"] > 6):  #  fit N2O, CO2, and HDO
    ...
    init[4,"y"] = 0.7991+0.0625*h2o_conc
    #init[4,"center"] = 6562.56298 - 0.000259*h2o_conc
    #init[5,"center"] = 6562.59088 - 0.000259*h2o_conc
    init[4,"center"] = 6562.56268 + 0.0003*h2o_conc
    init[5,"center"] = 6562.59058 + 0.0003*h2o_conc
    init[1,"center"] = 6562.59880 + 2.71e-5*h2o_conc
    ...
    if (peak_1 > 1.5 or peak_2 > 1 or peak_4 > 0.5) and abs(shift_n2o) < 0.05:    
...
if (species == 45 or species == 47):  #  fit N2O only with preset HDO and ammonia
    ...
    #init[4,"center"] = 6562.56268 - 0.0003*h2o_conc
    #init[5,"center"] = 6562.59058 - 0.0003*h2o_conc
    #init[4,"center"] = 6562.55518 - 0.000259*h2o_conc
    #init[5,"center"] = 6562.58308 - 0.000259*h2o_conc
    init[1,"center"] = 6562.59880 - 2.71e-5*h2o_conc
    r = anN2O[1](d,init,deps)
    ANALYSIS.append(r)
    peak_1a = r[1,"peak"]   
    base_1a = r[1,"base"]
    hdo_center = r[4,"center"]
...
if not IgnoreThis:    
    RESULT = {"res_a":res_a,"res_11":res_11,"res_12":res_12,"badshot_nh3":badshot_nh3,
              ...
              "nh3_pzt_mean":nh3_pzt_mean,"nh3_pzt_stdev":nh3_pzt_stdev,"hdo_center":hdo_center}
