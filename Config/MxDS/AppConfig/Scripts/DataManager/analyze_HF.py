...
#  .....
#  2017 0303:  Added post-fit corrections for water cross-talk to HF and HF cross-talk to water (jah)
...
H1 = _INSTR_["water_to_hf"]
M1 = _INSTR_["hf_to_water"]

try:
    temp = applyLinear(_DATA_["hf_ppbv"],HF_CONC) + H1*_DATA_["peak_82"]
    ...
    if ((_DATA_["species"]==60)and (abs(_DATA_["CavityPressure"]-_PERSISTENT_["Pressure_save"]) < 2.0)):
        ...
        _PERSISTENT_["previousS_HF"],_PERSISTENT_["previousNoise_HF"], _PERSISTENT_["tau_HF"] = variableExpAverage(_PERSISTENT_["bufferZZ_HF"],_PERSISTENT_["bufferExpAvg_HF"],temp,now,1800,0,_PERSISTENT_["previousS_HF"],_PERSISTENT_["previousNoise_HF"])

    print _PERSISTENT_["previousS_HF"]
    ...
    #_NEW_DATA_["HF"] = _NEW_DATA_["HF_raw"]
    _NEW_DATA_["HF"] = temp
    print "Exception: %s, %r" % (e,e)
    pass

try:
    _NEW_DATA_["H2O"] = applyLinear(_DATA_["h2o_conc_60"],H2O_CONC) + M1*_DATA_["peak_77"]
except Exception, e:
    print "Exception: %s, %r" % (e,e)
    pass
