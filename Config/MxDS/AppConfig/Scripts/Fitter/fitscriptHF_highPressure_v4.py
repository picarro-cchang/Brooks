...
#  2014 0411:  added baseline level and sinusoids to spectral model
#  2016 0523:  revised "fixed center" fit definition for O2 region -- it was actually allowing centration to change
#  2016 0525:  added reporting of residuals, fit time, and interval to match other G2000 fit scripts (no effect on fitting)
#  2016 0616:  removed VY77 fit for larger HF concentration (noisy), tweaked spectral library to match VC and FC fits
#  2016 0727:  Changed fit definition files to let y-parameter adjust
#  2016 0823:  Changed coefficients in concentration precalculation (for 140 Torr)
#  2016 0826:  Changed requirement on ngroups to exclude bad spectra
#  2016 1118:  Changed coefficients in water concentration precalculation based on CFADS comparison 17. Nov. 2016
#  2017 0203:  Changed fit definition ini for H2O-O2 VC to make it less prone to misidentify water as oxygen
...
if INIT:
    fname = os.path.join(BASEPATH,r"./MADS/spectral library MADSxx_v2_3_20160616.ini")
    ...
    y_77 = 0.0
    shift_77 = 0.0
    base77_ave = instrParams['Baseline_level']
    peak77_baseave = 0.0
    peak_79 = 0.0
    str_79 = 0.0
    base_79 = 0.0
...
tstart = time.clock()
if d["spectrumId"]==60 and d["ngroups"]>24:
#   Fit water at 7824.07 wvn, VC
    ...
    h2o_conc_60 = 0.1496*peak_80
    hf_ppbv = 0.2041*peak_77
    hf_ppbv_ave = 0.2041*peak77_baseave
    counter += 1
    
if d["spectrumId"]==61 and d["ngroups"]>60:
#   Wide range fit with VC 
    initialize_Baseline()
    r = anHF[4](d,init,deps)
    ANALYSIS.append(r)
    if (r[81,"peak"]<10 and r[82,"peak"]<10) or abs(r["base",3])>0.05:
...
if (ignore_count == 0):      
    RESULT = {"base0":base0,"base1":base1,"base77_ave":base77_ave,"y_77":y_77,"y_79":y_79,
          ...
          "ngroups":d["ngroups"],"numRDs":d["datapoints"],"fit_time":fit_time,"interval":interval,          
