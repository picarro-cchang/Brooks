...
if _DATA_["species"] == 65:
    ...
    try:
        temp = applyLinear(_DATA_["h2o2_ppmv"],H2O2_CONC)
        _NEW_DATA_["H2O2_raw"] = temp
        now = _OLD_DATA_["H2O2_raw"][-1].time
...
if _DATA_["species"] == 66:
    try:
        _NEW_DATA_["H2O2_raw"] = _OLD_DATA_["H2O2_raw"][-1].value
