...
plot(a['data'])
grid(True)
title('Frequency shift, Mean %.5f, StdDev %.5f' % (a['mean'],a['std']))
subplot(2,1,2)
a = arrays['stddevres']
plot(a['data'])
grid(True)
...
output = []
output.append("# Fitter configuration parameters from C2H6 baseline fit "+time.asctime())
output.append("C2H6_Baseline_level = %.6f" % arrays['base0']['mean'])
output.append("C2H6_Baseline_slope = %.6f" % arrays['base1']['mean'])
output.append("C2H6_Sine0_ampl =%.3f" % arrays['amp_ripp_1']['mean'])
output.append("C2H6_Sine0_freq = %.6f" % arrays['center_ripp_1']['mean'])
output.append("C2H6_Sine0_period = %.6f" % arrays['freq_ripp_1']['mean'])
output.append("C2H6_Sine0_phase = %.6f" % p1)
output.append("C2H6_Sine1_ampl = %.3f" % arrays['amp_ripp_2']['mean'])
output.append("C2H6_Sine1_freq = %.6f" % arrays['center_ripp_2']['mean'])
output.append("C2H6_Sine1_period = %.6f" % arrays['freq_ripp_2']['mean'])
output.append("C2H6_Sine1_phase = %.6f" % p2)
