...
def makeScan(base,incr,stepAndDwell,id,vLaser,threshold=None,extra1=0):
    ...
    for s,d in stepAndDwell:
        for i in range(d):
            if threshold == None:
                result.append(Row(base+s*incr,1,id,vLaser,extra1=extra1))
                result.append(Row(base-2*incr,1,id,vLaser,extra1=extra1))
            else:
                result.append(Row(base+s*incr,1,id,vLaser,threshold,extra1=extra1))
                result.append(Row(base-2*incr,1,id,vLaser,threshold,extra1=extra1))
    return result
