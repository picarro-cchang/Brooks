...
if INIT:
    ...
    fname = os.path.join(
        BASEPATH, r"../../../InstrConfig/Calibration/InstrCal/FitterConfig_HCl_140Torr.ini")
    ...
    anHCl.append(Analysis(os.path.join(
        BASEPATH, r"./HCl/HCl+H2O 140 Torr VC VY v1_1.ini")))
    anHCl.append(Analysis(os.path.join(
        BASEPATH, r"./HCl/HCl+H2O 140 Torr FC FY v1_1.ini")))
    anHCl.append(Analysis(os.path.join(
        BASEPATH, r"./HCl/HCl+H2O 140 Torr VC FY v1_1.ini")))
    anHCl.append(Analysis(os.path.join(
        BASEPATH, r"./HCl/HCl+H2O+HC 140 Torr VC FY v1_1.ini")))
    ...
    if "safe_loss_level" in instrParams:
        ...
        safe_loss_level = 2000.0  # In ppb/cm
...
if not(disable_dynamic_threshold) and (rd_rate < min_rd_rate) and (current_threshold > min_rd_threshold):
    ...
    if (d["spectrumId"] == 63) and (d["ngroups"] >= spectrum_min_groups):
        incomplete_spectrum = 0
        initialize_Baseline()
        # Fit wide spectrum with water, methane and HCl
        ...
        data_vector = d.fitData["loss"] - (r.model.funcList[0](r.model.xModifier(d.fitData["freq"])) + r.model.funcList[
                                        4](r.model.xModifier(d.fitData["freq"])) + r.model.funcList[5](r.model.xModifier(d.fitData["freq"])))
        model_vector = r.model.funcList[1](r.model.xModifier(d.fitData["freq"])) + r.model.funcList[2](r.model.xModifier(d.fitData[
            "freq"])) + r.model.funcList[3](r.model.xModifier(d.fitData["freq"])) + r.model.funcList[6](r.model.xModifier(d.fitData["freq"]))
        ...
        # Post-fit wide spectrum with water, methane, HCl and non-methane
        # hydrocarbons (VOCs!)
        ...
        PF_hcl_conc = hcl_lin * r[70, "strength"] + \
            ch4_hcl_lin * r[1002, 2] + h2o_hcl_lin * r[62, "peak"]
        PF_ch3oh_conc = 13 * r[1003, 2]
        PF_c2h4_conc = 100 * r[1004, 2]
        PF_res = r["std_dev_res"]
        if abs(PF_c2h4_conc) > 2.0 or abs(PF_ch3oh_conc) > 1.0:
            hcl_adjust = 0.0
    ...
    bad_baseline = int((baseline_level > 2000) or (baseline_slope > 30))

    if (ignore_count ==0) and not IgnoreThis:   # and not bad_baseline:
        RESULT = {"res":res,"bad_baseline":bad_baseline,"baseline_level":baseline_level,"baseline_slope":baseline_slope,"base70":base70,
                ...
                "PF_ch3oh_conc":PF_ch3oh_conc,"PF_c2h4_conc":PF_c2h4_conc,"PF_res":PF_res,"PF_base70":PF_base70,
                "pzt_mean":pzt_mean,"pzt_stdev":pzt_stdev, "threshold":current_threshold}
