 ...
 *
 * DESCRIPTION:
 *   USB FX2 firmware for FX2 development kit
 *
 * SEE ALSO:
...
void TD_Init(void) {           // Called once at startup
    ...
    // NB. There are restrictions regarding the sizes of buffers for EP2 and EP4.
    //  The total buffer size is 1024 bytes, which is shared between the endpoints
...
//    EP2CFG = 0xAA;         // Activate EP2 for bulk output, 1024 bytes, 2x buffered
    SYNCDELAY;
    EP4CFG = 0xA0;           // Activate EP4 for bulk output, 512 bytes, double buffered
    SYNCDELAY;
    ...
    // enable autopointers to allow access to endpoint buffer memory
    AUTOPTRSETUP |= 1;
    SYNCDELAY;                    
...
void TD_Poll(void) {           // Called repeatedly while the device is idle
    ...
    if ( GPIFTRIG & 0x80 ) {           // if GPIF interface IDLE
        if ( ! (EP2468STAT & bmEP4EMPTY) ) { // There are some data destined for the auxiliary board
            ...
            for (i=0; i<count; i++) {      // If we do not fetch the data, the number of bytes in the FIFO should just rise
              value = XAUTODAT1;
            }
            display = TRUE;
            SYNCDELAY;
    ...
    if (++LED_Count == 10000) {
        ++LED_Status;
        LED_On (LED_Status & 0x7);
        LED_Off (~(LED_Status) & 0x7);
        LED_Count = 0;
    }
