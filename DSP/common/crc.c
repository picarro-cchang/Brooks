...
/*                                                                     */
/* Table generated by Gary S. Brown.                                   */

static unsigned int crc32table[] = { /* CRC polynomial 0xedb88320 */
...
//#define UPDATE_CRC32(D, CRC) (crc32table[((int)CRC ^ D) & 0xFF] ^ ((CRC >> 8) & 0x00FFFFFF))
...
unsigned int calcCrc32 (unsigned int crc, unsigned int *buffer, unsigned int length32 ) {
    ...
    for ( index=0 ; index < length32 ; index++ ) {
        data = *(buffer+index);
        crc  = (crc>>8) ^ crc32table[ (crc^data) & 0xFF ]; data >>=8 ;
        crc  = (crc>>8) ^ crc32table[ (crc^data) & 0xFF ]; data >>=8 ;
        crc  = (crc>>8) ^ crc32table[ (crc^data) & 0xFF ]; data >>=8 ;
