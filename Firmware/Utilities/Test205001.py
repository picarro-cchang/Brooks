...
#
# FILE:
#   Test205001.py tests the G2000 power board PWM drive for the warm box
#
# DESCRIPTION:
#
# SEE ALSO:
...
class TestPowerBoardWarmBoxTecDriver(object):
    ...
    def run(self):
        ...
        try:
            regVault = Driver.saveRegValues(["WARM_BOX_MANUAL_TEC_REGISTER",
            ...
            Driver.wrDasReg("WARM_BOX_TEMP_CNTRL_AMAX_REGISTER",setPoints[-1])
            Driver.wrDasReg("WARM_BOX_MANUAL_TEC_REGISTER",quiescentValue)
            Driver.wrDasReg("WARM_BOX_TEMP_CNTRL_STATE_REGISTER",TEMP_CNTRL_ManualState)
            Driver.wrDasReg("TEC_CNTRL_REGISTER",TEC_CNTRL_Enabled)
            time.sleep(5.0)
            print "Warm Box TEC sweep"
            # Step coarse current DAC
            for s in setPoints:
                Driver.wrDasReg("WARM_BOX_MANUAL_TEC_REGISTER",s)
                Driver.wrFPGA("FPGA_PWM_WARMBOX","PWM_PULSE_WIDTH",int(s))
                time.sleep(0.25)
                r = float(self.sourcemeter.ask("READ?").split(",")[0])
                print s,r
                sweepMon.append(r)
            Driver.wrDasReg("WARM_BOX_TEMP_CNTRL_STATE_REGISTER",TEMP_CNTRL_DisabledState)
            Driver.wrFPGA("FPGA_PWM_WARMBOX","PWM_PULSE_WIDTH",quiescentValue)
            time.sleep(0.25)
        ...
        result1.columnTitles = ["Warm box TEC PWM","Voltage across 5 ohm load"]
...
if __name__ == "__main__":
    ...
    tst = TestPowerBoardWarmBoxTecDriver(tp)
    tst.run()
    tp.appendReport()
    tp.makeHTML()
