...
def _getVersionFromJsonFile():
    ...
    # problem here is mobile and g2000 builds have different
    # build numbers, ignoring the json file
    #import simplejson as json
...
def _getAppVersion():
    if hasattr(sys, "frozen"):
        ...
        try:
            ...
            # PicarroExeSetup.py
            try:
                from Host.Common import setup_version as buildVersion
                verStr = buildVersion.versionNumString()
                buildTypeStr = buildVersion.buildType()
        ...
        #verStr = _getVersionFromJsonFile()
        verStr = "DEVEL"
        buildTypeStr = "running from source"
...
class ErrorCollection(object):
    ...
    def __init__(self, filename, isWhitelist):
        ...
        if os.path.isfile(self.filename):
            LogMsg(4, "loading previously saved error items from '%s'" % self.filename)
        ...
        # else make sure the output dir exists so we can save to it
            ...
            LogMsg(1, "saved errors file '%s' does not exist, no previous items to load" % self.filename)
            path = os.path.split(self.filename)[0]

            LogMsg(1, "creating dir '%s' if it does not exist" % path)
...
class FileSearcher(object):
    ...
    def findNextError(self):
        ...
        # Useful delay after parsing file completed for debugging
        if g_debugDelay > 0:
            time.sleep(g_debugDelay)
        return
...
class EventLogWatcher(object):
    ...
    def handleOptions(self, options):
        ...
        LogMsg(4, "EventLogWatcher::handleOptions: logSearchPath = '%s" % self.logSearchPath)
        LogMsg(4, "EventLogWatcher::handleOptions: logCacheFilename = '%s" % self.logCacheFilename)
        LogMsg(4, "EventLogWatcher::handleOptions: logResultsFilename = '%s" % self.logResultsFilename)
        LogMsg(4, "EventLogWatcher::handleOptions: logWhitelistFilename = '%s" % self.logWhitelistFilename)
        ...
        if options.loadWhitelistFromFilename is not None:
            ...
            if os.path.isfile(self.logWhitelistFilename):
                LogMsg(1, "deleting existing whitelist file '%s'" % self.logWhitelistFilename)
...
"""
    ...
    g_debugDelay = options.debugDelay

    if options.getVersion is True:
        print _getAppVersion()
        sys.exit(1)
