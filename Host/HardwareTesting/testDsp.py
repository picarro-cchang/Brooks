...
def checkRam(analyzerUsb,startAddr,endAddr,nTrials):
...
def writeRead(analyzerUsb,addr,value):
    def writeMem(addr,value):
        analyzerUsb.hpiaWrite(addr)
        analyzerUsb.hpidWrite(c_int(value))
    def readMem(addr):
...
#    checkRam(analyzerUsb,0,0x40000,10000)

    # Check SDRAM
#    checkRam(analyzerUsb,0x80000000,0x81000000,10000)
    ...
    for i in range(100):
        print "Interrupt count %d" % rdRegUint(analyzerUsb,12)
        analyzerUsb.hpicWrite(0x00010003)
        sleep(0.5)

    # Read back register 11 which should be updated by a PRD
    while True:
        print "PRD count %d" % rdRegUint(analyzerUsb,11)
        sleep(0.5)
