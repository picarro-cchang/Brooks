...
Adds support for creating JSON-RPC APIs to Flask.
...
from Host.Utilities.MobileKit.AnalyzerViewer.SimpleJSONRPCServer import SimpleJSONRPCDispatcher as Dispatcher
import sys
import xmlrpclib
try:
...
class JSONRPCHandler(Dispatcher):
    ...
    def namespace(self, prefix):
        """
        This returns a :class:`JSONRPCNamespace` object, which has
        :meth:`~JSONRPCNamespace.register` and
        :meth:`~JSONRPCNamespace.register_function` methods. These forward
        ...
        """
        return JSONRPCNamespace(self, prefix)


class JSONRPCNamespace(object):
    """
    This is a simple proxy that can register methods, and passes them on to
    the :class:`JSONRPCHandler` that created it with a given name added as a
    ...
    def register_function(self, function, name=None):
        ...
        :meth:`JSONRPCHandler.register_function` method.
    ...
    def register(self, *args, **kwargs):
        ...
        :meth:`JSONRPCHandler.register`, it's shorter and easier to type.
        """
        return self.register_function(*args, **kwargs)
    ...
    def namespace(self, name):
        ...
        :param prefix: The name to prefix the methods with.
        """
        return JSONRPCNamespace(self.handler, self.prefix + '.' + name)


def dump_method_call(method, *params):
    """
    This marshals the given method and parameters into a proper JSON-RPC method
...
def load_method_response(response):
    """
    This returns the actual value returned from an JSON-RPC response. If it's
    a :obj:`Fault` instance, it will return the fault instead of the value.
    This is also useful for testing.

    :param response: The marshaled JSON-RPC method response or fault.
    """
    try:
...
def test_jsonrpc_call(client, rpc_path, method, *params):
    """
...
class JSONRPCTester(object):
    ...
    def call(self, method, *params):
        ...
        the marshaled method call, and a content type of ``text/json``. It will
        ...
        :param params: The parameters to pass to the method.
        """
        return test_jsonrpc_call(self.client, self.rpc_path, method, *params)
