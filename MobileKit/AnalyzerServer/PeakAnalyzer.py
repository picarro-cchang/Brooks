#!/usr/bin/python
"""
File Name: PeakAnalyzer.py
Purpose: Uses a Keeling plot to determine the isotopic ratio of a sample

File History:
     3-Nov-2011  sze   Initial version
    17-Dec-2013  sze   Removed usage of scipy
...
def linfit(x,y,sigma):
    ...
    df = len(x)-2
    return namedtuple('LinFit','coeffs cov rab chisq df')(asarray([b,a]),asarray([[sb2,sab],[sab,sa2]]),rab,chisq,df)
...
class PeakAnalyzer(object):
    ...
    def pushData(self, peakname, doc_data):
        ...
        if doc_data:
            ...
            if not self.last_peakname == peakname:
                if DEBUG['level']>0: print "I should be doing a replace now"
                replace_log = "True"
            ...
            if not collectingNow:
                ...
                if not lastCollecting:
                    if tempStore:
                        ...
                        if DEBUG['level']>0: print "Started collecting"
    ...
    def run(self):
        ...
        if DEBUG['level']>0: print "PeakAnalyzer.run()"
            ...
            if self.usedb:
                ...
                try:
                    if self.file_path:
                        ...
                        if DEBUG['level']>0: print "self.userlogfiles = '%s'" % self.userlogfiles
                        if DEBUG['level']>0: print "self.userlogfiles (abs) = '%s'" % os.path.abspath(self.userlogfiles)
...
def main(argv=None):
    ...
    for copt in [
                 ...
                 , 'legacyValveStop'
                 , "debug"          #True/False show debug pass ### (in stdout)
                 ]:
