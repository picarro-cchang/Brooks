...
class PeakFinder(object):
    ...
    def checkPeak(self, ssbuff, level, pos, minAmpl, nlevels, npoints):
    ...
    def findPeaks(self, data, hList, scaleList, kernels, ssbuff, cache, minAmpl, z, c, concIndex, distIndex, etmIndex, valveIndex,
        ...
        for i in range(nlevels):
            ...
            #  into the buffer
            if c - hList[i] < 0:
                ssbuff[i, :c + hList[i] + 1] += conc * kernels[i][hList[i] - c: 2*hList[i] + 1]
                ssbuff[i, npoints - hList[i] + c:] += conc * kernels[i][0:hList[i] - c]
            elif c + hList[i] >= npoints:
                ssbuff[i, c - hList[i]:] += conc * kernels[i][0:npoints - c + hList[i]]
                ssbuff[i, :c + hList[i] + 1 - npoints] += conc * kernels[i][npoints - c + hList[i]:2*hList[i] + 1]
            else:
                ssbuff[i, c - hList[i]:c + hList[i] + 1] += conc * kernels[i][0:2*hList[i] + 1]
            if i > 0:
                ...
                isPeak = peakF.checkPeak(ssbuff, i - 1, c - hList[i] - 1, minAmpl)
                col = (c - hList[i] - 1) % npoints
                if isPeak and cache[distIndex, col] > 0.0:
                    ...
                    if not reject:
                        ...
                        peaks.append(tuple([v for v in cache[:, col]] + [amplitude, sigma]))
