...
WriteFile(handle,"%-26s%-26s%-26s%-26s%-26s%-26s%-26s\r\n" % ("EPOCH_TIME","ALARM_STATUS","GPS_ABS_LONG","GPS_ABS_LAT","CH4","C13_Delta_Raw","ValveMask"))
...
idDelta = header.index('C13_Delta_Raw')
idValves = header.index('ValveMask')
for line in ip.readlines():
    line = line.split()
    epochTime = float(line[idEpochTime])
    data = "%-26.3f%-26d%-26.10e%-26.10e%-26.10e%-26.10e%-26.10e\r\n" % (epochTime, 0, float(line[idLong]), \
        float(line[idLat]), float(line[idMethane]), float(line[idDelta]), float(line[idValves]))
    while True:
        time.sleep(0.25)
        try:
