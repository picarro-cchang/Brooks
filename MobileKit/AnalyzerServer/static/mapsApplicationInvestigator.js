...
function updatePath(pdata, clr, etm, pos) {
    ...
    */
    ...
    if (clr === CNSNT.normal_path_color) {
        concMarker = newConcMarker(CSTATE.map, where, size, color);
        ...
        if ('windN' in pdata) {
            ...
            CSTATE.windRayByPos[pos] = newWindRay(CSTATE.map, where, bearing, Math.max(10.0,size));
            CSTATE.windRayByPos[pos].setVisible(CSTATE.showWindRay);
        }
    }
    else {
        concMarker = newConcMarker(CSTATE.map, where, 2, "black");
        concMarker.myProperties = {shape:"circle", size:2, color:"black", pos:pos, etm:etm, conc:pdata.ch4 };
    }
...
function initialize_gdu(winH, winW) {
    ...
    var abubbleCookie, pbubbleCookie, wbubbleCookie, swathCookie, activePlatNameCookie, windRayCookie;
...
function updateConcPlot() {
    if (CSTATE.follow) $.plot($("#concPlot"), [ {data: CSTATE.concPlotFollowing, lines: {show: true}, color:"#00f" } ], {xaxis:{ticks:5}} );
}
