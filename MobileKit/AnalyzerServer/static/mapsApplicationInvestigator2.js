...
function newConcMarker(map, latLng, size) {
    mk = new google.maps.Marker({position: latLng, icon: new google.maps.MarkerImage(makeCircle(size),null,null,newPoint(0.5*size,0.5*size))});
    mk.setMap(map);
    return mk;
}
...
function attachConcMarkerListener(concMarker) {
    ...
    concMarkerListener = new google.maps.event.addListener(concMarker, 'mouseover', function() {
        var i, pos = concMarker.pos;
        CSTATE.concPlotNotFollowing = [];
        for (i=pos-100; i<pos+100; i++) {
            if (i in CSTATE.concByPos) {
                CSTATE.concPlotNotFollowing.push([i-pos,CSTATE.concByPos[i]]);
            }
        }
        if (!CSTATE.follow) {
            $.plot($("#concPlot"), [ {data: CSTATE.concPlotNotFollowing, lines: {show: true}, color:"#f00" } ], {xaxis: {ticks:5}} );
        }
    });
...
function clearConcMarkerArray() {
    var i, ky;
    for (i = 0; i < CSTATE.concMarkers.length; i += 1) {
        CSTATE.concMarkers[i].setMap(null);
    }
    CSTATE.concMarkers = [];
...
function clearWindRayMarkerArray() {
    var i;
    for (i = 0; i < CSTATE.windRayMarkers.length; i += 1) {
        CSTATE.windRayMarkers[i].setMap(null);
    }
    CSTATE.windRayMarkers = [];
}
...
function showWindRay() {
    var i;
    for (i=0;i<CSTATE.windRayMarkers.length;i++) CSTATE.windRayMarkers[i].setVisible(true);
}

function hideWindRay() {
    var i;
    for (i=0;i<CSTATE.windRayMarkers.length;i++) CSTATE.windRayMarkers[i].setVisible(false);
}
...
function modalPaneMapControls() {
    var modalPane, showDnoteCntl, showPnoteCntl
        ...
        , changeStabClassCntl, showWindRayCntl;
    var dchkd, pchkd, achkd, pbchkd, abchkd, wbchkd, swchkd, wrchkd, hdr, body, footer, c1array, c2array;
...
function updatePath(pdata, clr, etm, pos) {
    ...
    */
    ...
    var concMarker = newConcMarker(CSTATE.map, where, size);
    concMarker.pos = pos;   // Save position and epoch time in object so listener can access them
    concMarker.etm = etm;
    attachConcMarkerListener(concMarker);
    CSTATE.concByPos[pos] = pdata.ch4;
    CSTATE.concMarkers.push(concMarker);
    if ('windN' in pdata) {
        var bearing = CNSNT.rtd * Math.atan2(pdata.windE,pdata.windN);
        CSTATE.windRayMarkers.push(newWindRay(CSTATE.map, where, bearing, Math.max(10.0,size)));
    }
...
function makeSquare(side) {
    ...
    if (!(side in CSTATE.squares)) {
        ...
        context.fill();
        CSTATE.squares[side] = canvas.toDataURL("image/png");
    }
    return CSTATE.squares[side];
};

function makeCircle(side) {
    var context;
    if (!(side in CSTATE.circles)) {
        var canvas = document.createElement("canvas");
        canvas.height = side;
        canvas.width = side;
        context = canvas.getContext("2d");
        ...
        context.fillStyle = "#0000ff"
        context.fill();
        CSTATE.circles[side] = canvas.toDataURL("image/png");
    }
    return CSTATE.circles[side];
};
    ...
    var successData = function(data) {
        ...
        if (resultWasReturned) {
            ...
            if (data.result.EPOCH_TIME) {
                ...
                restart = false;
                var newMin = data.result.CH4[0];
                for (i=0; i<n; i++) {
                    newMin = Math.min(newMin, data.result.CH4[i]);
                }
                    ...
                    if (n > 1) {
                        ...
                        if (CNSNT.prime_view) {
                            ...
                            for (i=0;i<CSTATE.methaneHistory.length; i++) {
                                CSTATE.concPlotFollowing.push([i,CSTATE.methaneHistory[i]]);
                            }
                            updateConcPlot();
...
function updateConcPlot() {
    if (CSTATE.follow) $.plot($("#concPlot"), [ {data: CSTATE.concPlotFollowing, lines: {show: true}, color:"#00f" } ], {xaxis:{ticks:[]}} );
}
