...
#
"""
File Name: JsonReportSupportByGeo.py
...
class P3ServiceFactory(object):
    ...
    def getService(self):
        ...
        service.rprocs = '["AnzLogMeta:byEpoch","AnzLog:byPos","AnzLog:byEpoch","AnzMeta:byAnz","AnzLog:byGeo"]'
        # service.rprocs = '["AnzLogMeta:byEpoch","AnzLog:byPos","AnzLog:byEpoch","AnzMeta:byAnz"]'
        service.debug = True
        service.timeout = 60
...
def getWithRetries(p3Service, *a, **k):
    ...
    for i in range(maxAttempts):
        ...
        if 'error' in result and result['error']:
            ...
            return result
    raise RuntimeError("Command failed after %d attempts\nErrors: %s" % (maxAttempts, failures))
...
class ReportPathData(object):
    ...
    def makePathData(self, swCorner, neCorner, runsData):
        ...
        for ir, params in enumerate(runsData):
            ...
            #            'resolveLogname':True, 'doclist': True}
            result = getWithRetries(p3, "gdu", "1.0", "AnzLog", qryparms)['return']['result']
