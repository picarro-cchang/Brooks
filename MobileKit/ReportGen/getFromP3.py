...
LOG_DURATION = True
LOG_DURATION_FILE = os.path.join(os.path.split(AppPath)[0], "P3Timing.dat")

logLock = threading.Lock()


def logDuration(msg, tstart, duration):
    if LOG_DURATION:
        logLock.acquire()
        try:
            fp = open(LOG_DURATION_FILE, "a")
            print >>fp, "%-25s at %15.1f for %10.3f" % (msg, tstart, duration)
            fp.close()
        finally:
            logLock.release()
...
class P3_Accessor(object):
    ...
    def getTicket(self):
        ...
        while retries < 5:
            try:
                ...
                logDuration("ByEpochTimeGetTicket", tstart, duration)
        ...
        def fetchBuffer():
            ...
            while True:
                try:
                    ...
                    rtndata_str = resp.read()
                    duration = time.clock() - tstart
                    logDuration("ByEpochTime", tstart, duration)
                    # Only push rows which are of the correct logtype
                    etm = params["startEtm"]
                    for drow in json.loads(rtndata_str):
                        if drow == threadDict["lastRow"]:
                            continue    # Duplicate row, discard
                        if ('LOGTYPE' in drow) and (drow['LOGTYPE'] != params["logtype"]):
                            continue
    ...
    def genAnzLog(self, logtype):
        def _genAnzLog(startEtm, endEtm=None, limit=2000, **kwds):
            ...
            if kwds:
                get_params.update(kwds)
            if endEtm is not None:
                get_params["endEtm"] = endEtm
    ...
    def getTicket(self):
        ...
        while retries < 5:
            try:
                ...
                rtndata_str = resp.read()
                duration = time.clock() - tstart
                logDuration("ByPosGetTicket", tstart, duration)
                print "POST Response: %s: %s" % (resp.code, rtndata_str)
                break
            except urllib2.URLError, e:
                print "Error: ", e
        ...
        def fetchBuffer():
            ...
            while True:
                try:
                    ...
                    rtndata_str = resp.read()
                    duration = time.clock() - tstart
                    logDuration("ByPos", tstart, duration)
                    # Only push rows which are of the correct logtype
                    for drow in json.loads(rtndata_str):
                        if ('LOGTYPE' in drow) and (drow['LOGTYPE'] != params["logtype"]):
                            continue
                        ...
                        if "endPos" in params and row >= params["endPos"] - 1:
                            break
