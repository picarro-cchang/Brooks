...
from JsonReportSupport import getTicket, JsonReportGen, ReportGenStatus
from ReportCommon import getMsUnixTime, msUnixTimeToTimeString
...
# Make an initial call to strptime in the main thread to avoid problems with
#  calling it in a child thread
...
"""
https://dev.picarro.com/dev/rest/gdu/NONE/1.0/AnzLog?lrtSortList=%5B%22LOGNAME%2
2%2C+%22EPOCH_TIME%22%5D&endEtm=1339372740&resolveLogname=True&rtnFmt=lrt&startE
tm=1339333980&qry=byGeo&anz=FCDS2008&box=%5B-121.93108%2C+36.58838%2C+-121.88112
%2C+36.62807%5D&forceLrt=True&limit=all&doclist=True

/rest/RptGen?qry=submit&
/rest/RptGen?qry=getStatus&
/rest/RptGen?qry=getFirst&
/rest/RptGen?qry=getNext&
"""


@app.route('/rest/RptGen', methods=['GET', 'POST'])
def rptGen():
    qry = request.values.get("qry")
    if qry == "test":
        contents = request.values.get("contents")
        msUnixTime = getMsUnixTime()
        timeString = msUnixTimeToTimeString(msUnixTime)
        result = {"rpt_contents_hash": getTicket(contents),
                  "rpt_start_ts": timeString, "request_ts": timeString}
        ...
        result = JsonReportGen(REPORTROOT, contents).run({"force": force})
        response = make_response(json.dumps(result))
    elif qry == "getStatus":
        #contents_hash = request.values.get('contents_hash')
        #start_ts = request.values.get('start_ts')
        result = ReportGenStatus(REPORTROOT, request.values).run()
        response = make_response(json.dumps(result))
    elif qry == "getFirst":
...
@app.route('/rest/instrUpload2')
def instrUpload2():
    # Services upload of instructions for generating JSON files to be
    #  interpreted by JavaScript report generator
    contents = request.values.get('contents')
    # Pass directory in which report files are to be placed and the
    #  contents of the instruction file
    ticket = JsonReportGen(REPORTROOT, contents).run()
    data = {'contents': contents, 'ticket': ticket}
    response = make_response(json.dumps(data))
    response.headers['Content-Type'] = 'application/json'
    return response
...
def getJson():
    ...
    if 'region' in request.values:
        ...
        fname = os.path.join(REPORTROOT, ticket, '%s.%d.json' % (name, region))
    else:
        fname = os.path.join(REPORTROOT, ticket, '%s.json' % (name,))
    if os.path.exists(fname):
        ...
        response = "File %s not found" % fname
    return response


@app.route('/rest/getJsonReport')
def getJsonReport():
    ticket = request.values.get('ticket', '')
    region = int(request.values.get('region', 0))
    swcorner = request.values.get('swcorner', '')
    necorner = request.values.get('necorner', '')
    return render_template('displayReport.html', region=region, ticket=ticket, swcorner=swcorner, necorner=necorner)
...
if __name__ == "__main__":
    ...
    parser.add_option("-r", dest="reportRoot",
                      help="Specify root directory for report files [default: %default]",
                      default=globals()["REPORTROOT"])
    options, args = parser.parse_args()
    globals()["REPORTROOT"] = options.reportRoot
    app.run(host='0.0.0.0', port=5300, debug=DEBUG, threaded=True)
