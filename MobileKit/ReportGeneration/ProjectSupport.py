...
from ReportCommon import getTicket
...
class Job(object):
    ...
    def __init__(self,resourceManager,name,inputs=[],outputs=[],instructions=None):
        ...
        for rt in inputs+outputs:
            if ticket is None:
                ...
                raise ValueError("All resources for a job must have same ticket.")
        self.ticket = ticket
    ...
    def asDict(self):
        ...
        return dict(name=self.name, status=self.statusText.get(self.status,""), 
    ...
    def propagate_failure(self,badResources):
        ...
        for rt in self.outputs:
            ...
            self.rm.makeFailed(rt,errorMsg="Input resource(s) not made successfully")
    ...
    def updateAndGetStatus(self,getResourceStatus=None,propagate_failure=None):
        ...
        if getResourceStatus is None: getResourceStatus = self.rm.getStatus
        if propagate_failure is None: propagate_failure = self.propagate_failure
...
class ProjectManagerInterface(object):
    ...
    def responseHandler(self,responseList):
        ...
        sockDict = dict(REPLY=self.submit,SEND_JOB=self.source,STOP_WORKERS=self.controller)
    ...
    def dispatch(self,workerIndex,job,jobNum):
        paramDict = dict(msg="JOB",jobNum=jobNum,job="REPORT",
            args=(job.name,job.instructions,job.ticket,job.inputs,job.outputs))
...
class ProjectManager(object):
    ...
    def allocateWork(self):
        ...
        if (not self.workerStartPending) and (len(self.workers) < self.poolSize):
            ...
            p = mp.Process(target=Worker(self.workerIndex,JOB_DISTRIBUTION_PORT,JOB_COMPLETE_PORT,STOP_WORKERS_PORT).run)
            p.start()
            ...
            if cmd["msg"] == "PROJECT":
                ...
                if ps.projectComplete() == ProjectSubmission.PROJECT_COMPLETE:
                    ...
                    if ps.ticket not in self.jobsByProject:
                        ...
                        for j in ps.jobList: self.jobs[j.name] += 1
                ...
                response = ResponseTuple("REPLY",{"nWorkers": len(self.workers)})
            elif cmd["msg"] == "DELETE_PROJECT":
                ticket = cmd["ticket"]
                status = self.deleteProject(ticket)
                response = ResponseTuple("REPLY",{"ticket": ticket, "status": status})
            elif cmd["msg"] == "GET_PROJECT_STATUS":
                ticket = cmd["ticket"]
                if ticket not in self.projectSubmissions:
                    response = ResponseTuple("REPLY",{"error": "Project not found"})
                else:
                    ps = self.projectSubmissions[ticket]
                    jobs = self.jobsByProject[ticket]
                    response = ResponseTuple("REPLY",{"project": ps.getStatus(), "jobs": [j.asDict() for j in jobs]})
                ...
                for j in self.jobsByProject[ticket]:
                    if j.status in [ Job.JOB_READY_TO_RUN ]:
                        ...
                        while len(self.activeProjects) > 0:
                            ...
                            if t in projectsToRemove: continue
...
class ProjectSubmission(object):
    ...
    def projectComplete(self):
        ...
        """
        if self.deliverableResources is None: self.analyzeSubmission()
    ...
    def analyzeSubmission(self):
        ...
        if self.jobList is None:
            ...
            if "summary" in self.instructions:
                ...
                self.jobList.append(Job(self.rm,"Make Summary",[],[summaryMap],self.instructions))
                self.deliverableResources.append(summaryMap)
            for i,region in enumerate(self.instructions["regions"]):
                baseMap = RT("BASE_MAP", i)
                pathMap = RT("PATH_MAP", i)
                markerMap = RT("MARKER_MAP",i)
                compositeMap = RT("COMPOSITE_MAP",i)
                regionalReport = RT("REGIONAL_REPORT",i)
                self.jobList.append(Job(self.rm,"Make Base, region %d" % i,[],[baseMap],self.instructions))
                self.jobList.append(Job(self.rm,"Make Path, region %d" % i,[],[pathMap],self.instructions))
                self.jobList.append(Job(self.rm,"Make Marker, region %d" % i,[],[markerMap],self.instructions))
                self.jobList.append(Job(self.rm,"Make Composite, region %d" % i,[baseMap,pathMap,markerMap],[compositeMap],self.instructions))
                self.jobList.append(Job(self.rm,"Make RegionalReport, region %d" % i,[pathMap,markerMap,compositeMap],[regionalReport],self.instructions))
            ...
            self.jobList.append(Job(self.rm,"Make ProjectReport",regionalReports,[projectReport],self.instructions))
...
class Resource(object):
    ...
    def _readDetailedStatus(self,filename):
        ...
        with file(filename,"rb") as fp:
            while True:
                try:
                    self.detailedStatus = merge_dictionary(self.detailedStatus,cPickle.load(fp))
                except EOFError:
                    break

    def updateStatus(self,newStatus,newDetailedStatus):
        self.status = newStatus
        self.detailedStatus = merge_dictionary(self.detailedStatus,newDetailedStatus)
...
class ResourceManager(object):
    ...
    def __init__(self,root,maxResources=512):
        self.root = root
    ...
    def makeStarted(self,rt):
        ...
        for e in resource.extList:
            if e: assert not os.path.exists(resource.path + e)
    ...
    def makeSucceeded(self,rt,doneMsg="done"):
        ...
        newDetailedStatus = {"end":time.strftime("%Y%m%dT%H%M%S"), "done":doneMsg}
        resource.updateStatus(newStatus,newDetailedStatus)
    ...
    def makeFailed(self,rt,errorMsg="error"):
        ...
        newDetailedStatus = {"end":time.strftime("%Y%m%dT%H%M%S"), "error":errorMsg}
        resource.updateStatus(newStatus,newDetailedStatus)
        self.writeQueue.put({"cmd": "RENAME",
                             "oldName":resource.path+resource.extList[resource.IN_PROGRESS],
                             "newName":resource.path+resource.extList[newStatus],
    ...
    def handleRequest(self,request):
        ...
        if funcName in self.funcList:
            try:
                ...
                reply = {"error": "%s" % e, "traceback": traceback.format_exc()}
        else:
...
class Worker(object):
    """Represents a worker which can be started, given jobs and stopped
        ...
        The attribute maxJobs can be used to limit the number of jobs a worker can perform before it is
         ...
         worker.
    """ 
    def __init__(self, index, rPort, sPort, cPort):
        self.index = index
        self.rPort = rPort
        self.sPort = sPort
        self.cPort = cPort
    ...
    def doJob(self,cmd):
        jobMap = {"REPORT":self.report, "ECHO":self.echo, 
                  "SQUARE":self.square, "SLEEP":self.sleep}
    ...
    def run(self):
        ...
        self.sender.send(json.dumps(dict(msg="STARTING",workerIndex=self.index)))
        ...
        try:
            while True:
                ...
                if socks.get(self.receiver) == zmq.POLLIN:
                    ...
                    if not result["available"]: break
        finally:
            self.sender.send(json.dumps(dict(msg="TERMINATING",workerIndex=self.index)))
...
Breaking up a project into jobs:
    ...
    for i in range(nRegions):
        ...
        Job([BASE_MAP(i),PATH_MAP(i),MARKER_MAP(i)],[COMPOSITE_MAP(i)],seq); seq+=1
        Job([PATH_MAP(i),MARKER_MAP(i),COMPOSITE_MAP(i)],[REGIONAL_REPORT(i)],seq); seq+=1
    Job([REGIONAL_REPORT(i) for i in range(nRegions)],[PROJECT_REPORT],seq); seq+=1
