...
def submitProject():
    ...
    if contents:
        ...
        if evts.get(socket) == zmq.POLLOUT:
            socket.send(json.dumps({"msg": "PROJECT", "reportRoot": REPORTROOT, "contents":contents}))
            evts = dict(poller.poll(5000))
            if evts.get(socket) == zmq.POLLIN:
                data = json.loads(socket.recv())
            else:
                data = {'error': "No reply: Check project manager has been started"}            
        else:
...
def getReportEnv():
    ...
    result = dict(REPORTROOT=REPORTROOT, STATICROOT=STATICROOT, APIKEY=rgs.APIKEY, 
...
if __name__ == "__main__":
    if len(sys.argv) > 1: REPORTROOT = sys.argv[1]
