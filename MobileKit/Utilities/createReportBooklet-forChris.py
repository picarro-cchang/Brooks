...
def getKMLParamsFromFile( fn ):
    kmlParamsList = []
    config = ConfigObj(fn) 
    n_kml = int(config["ReportParams"]["n_kml"])
    if ( int( config["ReportParams"]["plot_kml"] ) == 0 ):   #If the user doesn't want KML plotted, return an empty list
        return kmlParamsList
    kml_key_list = []
    for i in range(n_kml):
        kml_key_list.append("KMLParams" + str(i+1))
    for plat in kml_key_list:
        fn = str( config[plat]["kml_filename"] )
        lw = str( config[plat]["kml_linewidth"] )
        lc = str( config[plat]["kml_linecolor"] )
        filter = str( config[plat]["kml_filter"] )
        kmlParamsList.append( KmlParams( fn, lw, lc, filter ) )
    print "Found ", str(len(kmlParamsList)), " KML blocks"
    return kmlParamsList
...
if __name__ == "__main__":
    ...
    if makeMaps:
        ...
        if (useFileInput):
            ...
            kmlList = getKMLParamsFromFile(inptFileName) # list is empty if no KML requested
        else:
            ...
            kmlList = [] # for now the only way to plot KML is via config file
    else:
    ...
    if (ready):
        ...
        if makeMaps:
            ...
            writeAndRunSummaryInstructions( areaName, analyzer, gridPars, minRunParamsList, kmlList )
            ...
            writeAndRunGridInstructions( areaName, analyzer, gridPars, minRunParamsList, markerList, kmlList )
