...
## This belongs in some sort of parser class. 
...
def getMarkerListFromHTML( fn, color="00FFFF" ):
    color = color.replace('#',"")
    markerList = []
    f = file(fn,"rb")
    ...
    while (line != ""):
        if (( ('<tr>' in line) and ( not ('thead' in line)) and ( not ('/tr' in line)) )):
            ...
            markerList.append( MarkerInfo( name, lat, lng, color) )
        line = f.readline()
    f.close()
...
class InstructionsWriter(object):
    ...
    def WriteTileInstructions( self, fn, displayConfig ):
        ...
        # Write the marker block
        # combine the user marker list and the "master" list, taken from the xml file. 
        # this is done if displayConfig.bubbleColor is not None
        allMarkersList = []
        # print "****************DEBUG:********** bubblecolor: ", displayConfig.bubbleColor
        if displayConfig.bubbleColor is not "None":    
            xml_fn = self.cfg.topDir+'\\'+self.cfg.reportDir+'\\'+self.cfg.instructionsSubDir+'\\'+self.cfg.summaryMapsSubDir+'\\'+self.cfg.sumDirBaseName+'1\\composite.0.markerTable.html'
            if (os.path.isfile(xml_fn)):
                allMarkersList.extend(getMarkerListFromHTML(xml_fn, displayConfig.bubbleColor))
        if (displayConfig.overlayUserMarkers==1):
            allMarkersList.extend(cfg.userMarkerList)
        instructions += self.GetMarkerBlock(allMarkersList)
        instructions += "}"         
        instFile = file(fn,"wb")
    ...
    def WriteAllSummaryInstructions( self ):
        os.chdir( cfg.instructionsSubDir )
        fnBase = "instructions_"
        for i in range(len(cfg.summaryMapConfigList)):
            fn = fnBase + cfg.summaryMapDirList[i] + ".json"
    ...
    def WriteAllTileInstructions( self ):
        os.chdir( cfg.instructionsSubDir )
        fnBase = "instructions_"    
        for i in range(len(cfg.tileDisplayConfigList)):
            fn = fnBase + cfg.bookletMapDirList[i] + ".json"
...
class InstructionsProcessor(object):            
    ...
    def RunTileMaps(self):
        ...
        for i in range(100):
            print "**************************************"
        print "Debug: Cur dir: ", os.getcwd()
        os.chdir( cfg.topDir + '\\' + cfg.reportDir )
        os.chdir( cfg.instructionsSubDir )
        print "Dubug: Now the dir is: ", os.getcwd()
    ...
    def RunSummaryInstructions(self):
        if not cfg.assembleOnly:
            self.RunKeyMap()
            self.RunSummaryMaps()
...
class ReportAssembler(object):
    ...
    def getMarkerListFromHTML( self, fn ):
        ...
        if not os.path.isfile(fn):
            print "Warning, markers HTML file does not exist!!!"
            return markerList
        f = file(fn,"rb")
        # Loop thru the lines and look for <tr>. Ignore the first instance, which is accompanied by <thead>
        line = f.readline()
...
if __name__ == "__main__":
    ...
    print "Copyright (c) 2013 Picarro, Inc., All Rights Reserved.\n"
    ...
    writer.WriteAllSummaryInstructions()
    instrProc = InstructionsProcessor(cfg)
    #instrProc.RunAllInstructions()
    instrProc.RunSummaryInstructions()
    writer.WriteAllTileInstructions()
    instrProc.RunTileInstructions()
