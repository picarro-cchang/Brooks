...
class DrawOnMapBatch(object):
    ...
    def run(self):
        self.gMap = GoogleMap()
        varList = {'DAT_FILE':'datFile','OUT_DIR':'outDir','SIGMA_FACTOR':'sigmaFactor',
                   'MIN_AMP':'minAmpl', 'MAX_AMP':'maxAmpl','PIE_RADIUS':'pieRadius',
        ...
        for secName in self.config:
            ...
            self.sigmaFactor = 2.0 if self.sigmaFactor is None else float(self.sigmaFactor)
            self.pieRadius = 50.0 if self.pieRadius is None else float(self.pieRadius)
            ...
            try:
                ...
                self.findTargetMaps(peakName,self.zoom)
                self._draw(datName,peakName,self.padX,self.padY)
            except Exception:
                print traceback.format_exc()
    
    def findTargetMaps(self,peakFile,zoom,minOverlap=0.05):
        """Goes through peakFile and finds bounding box of the path in terms of latitudes 
        ...
        dat = aReadDatFile(peakFile)
        xpath = dat.GPS_ABS_LONG
        ypath = dat.GPS_ABS_LAT
        latmin,latmax = ypath.min(),ypath.max()
        lngmin,lngmax = xpath.min(),xpath.max()
        latmean = 0.5*(latmin+latmax)
        lngmean = 0.5*(lngmin+lngmax)
        #
        Xb = lngmax - lngmin
        Yb = latmax - latmin
        ...
        Yo = (ny*Yp-Yb)/(ny+1)
        #
        lngStart = lngmean - 0.5*(nx*Xp-(nx-1)*Xo)
        latStart = latmean - 0.5*(ny*Yp-(ny-1)*Yo)
        #
        self.platList = []
        for ix in range(nx):
            xmin = lngStart + ix*(Xp-Xo)
            xmax = xmin + Xp
            xcen = 0.5*(xmin+xmax)
            inX = (xpath>=xmin) & (xpath<xmax)
            for iy in range(ny):
                ymin = latStart + iy*(Yp-Yo)
                ymax = ymin + Yp
                ycen = 0.5*(ymin+ymax)
                ...
                if (0<=x<nx) and (0<=y<ny) and (amp>self.minAmpl):
                    if (self.maxAmpl is None) or (amp <=self.maxAmpl):
                        ...
                        if "WIND_N" in pk._fields:
                            ...
                            radius = self.pieRadius; speedmin = 0.5
                            meanBearing = (180.0/np.pi)*np.arctan2(windE,windN)
                            if not(np.isnan(wind) or np.isnan(meanBearing)):
                                minBearing = meanBearing-min(self.sigmaFactor*windSdev,180.0)
                                maxBearing = meanBearing+min(self.sigmaFactor*windSdev,180.0)
