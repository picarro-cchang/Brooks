                ...
                if fit>0:
                    ...
                    if (-padX<=x<nx+padX) and (-padY<=y<ny+padY) and self.showFov:
                        ...
                        if "WIND_N" in d._fields:
                            windN = d.WIND_N
                            windE = d.WIND_E
                            cwindMean = d.CWIND_MEAN
                            cwindSDev = d.CWIND_SDEV
                            dstd = (np.pi/180.0)*d.WIND_DIR_SDEV
                            bearing = np.arctan2(windE,windN)
                            speed = np.sqrt(windE*windE + windN*windN)
                            width = self.pg.getMaxDist(self.stabClass,self.minLeak,speed,0.0,self.minAmpl,umin=1.0)
                            if cwindSDev > abs(cwindMean): width = 0.0
