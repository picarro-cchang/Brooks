...
import csv
fname = r'R:\crd_G2000\FCDS\1061-FCDS2003\Comparison\Meter set leaks Diablo.csv'
fp = open(fname, 'rb')
op = open("plats.txt","w")
...
for i,row in enumerate(reader):
    if i>0:
        ...
        if len(which)>0:
            print>>op, "%20s %20s %20s" % (lat, lng, "'%s" % names[which.pop()])
        else:
            print>>op, "%20s %20s %20s" % (lat, lng, "'UNKNOWN")
fp.close()
