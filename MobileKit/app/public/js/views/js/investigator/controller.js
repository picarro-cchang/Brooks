...
mapmaster.Controller = function(){
	...
	if(getURLParameter("prime") === "false"){
	    ...
	    CNSNT.resturl = "https://10.200.2.100:8081/node/rest";
	 ...
	 //    var initArgs = {
	  ...
	  CNSNT.sys = initArgs.psys
    CNSNT.identity = initArgs.identity
	...
	var initialize = function(winH, winW) {
		...
		// 	svgPow = (3.3 - $( "#time_slider" ).slider( "option", "value" ));
		// 	redrawCanvas();
		// 	drawLegend();
		// },
		...
		// showPanel('setting');
		$('#car_signal-top').tappable(toggleSignalTop)	;
		$('#car_signal-front').tappable(toggleSignalFront);
	...
	var toggleSignalTop = function(){
		var sig = $('#car_signal-top');
		if(!sig.hasClass('active')){
	...
	var toggleSignalFront = function(){
		var sig = $('#car_signal-front');
		if(!sig.hasClass('active')){
	...
	var toggleChart = function(){
		if($('.icon-chart').hasClass('selected')){
			...
			$('#map2_flot, #map_realtime_flot').animate({bottom:'-20%'}, transition_time, function(){
				$('#map2_canvas').css({height:'100%'});
			});		
		}else{
			$('.icon-chart').addClass('selected');
			$('#map2_flot, #map_realtime_flot').animate({bottom:'0%'}, transition_time, function(){
