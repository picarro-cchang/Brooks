...
def  LaserDac(clk,reset,dac_clock_in,chanA_data_in,chanB_data_in,strobe_in,
              dac_sclk_out,dac_sync_z_out,dac_din_out):
    ...
    chanA_control = Signal(intbv(0)[8:])
    chanB_control = Signal(intbv(0)[8:])
    CONTROL_LENGTH = 8
    DATA_LENGTH = 16
    ...
    def  logic():
        while True:
            ...
            if reset:
                counter.next = 0
                dac_din_out.next = 0
                dac_sync_z_out.next = 1
            else:
                if state == t_State.IDLE:
                    dac_sync_z_out.next = 1
                    if strobe_in:
                        counter.next = 0
                        dac_sync_z_out.next = 0
    ...
    @always_comb
    def  comb1():
        dac_sclk_out.next = dac_clock_in
        chanA_control.next = 0x00   # Write buffer A
        chanB_control.next = 0x34   # Write buffer B, load A & B

    @always_comb
    def  comb2():
        if CHANA_CONTROL_START<=counter<CHANA_CONTROL_END:
            dac_din_out.next = chanA_control[CHANA_CONTROL_END-1-int(counter)]
        elif CHANA_DATA_START<=counter<CHANA_DATA_END:
            dac_din_out.next = chanA_data_in[CHANA_DATA_END-1-int(counter)]
        elif CHANB_CONTROL_START<=counter<CHANB_CONTROL_END:
            dac_din_out.next = chanB_control[CHANB_CONTROL_END-1-int(counter)]
        elif CHANB_DATA_START<=counter<CHANB_DATA_END:
