# buildWin7UserMigrationTools.py
#
# Set up the build environment and builds Win7 user migration tools. Use this
# for building the apps from the command line.
#
# This executes python setupWin7UserMigrationTools.py py2exe and ensures PYTHONPATH is properly
...
def _buildExes(scriptName, toolsList):
    ...
    if curDir.lower() != "user":
        print "Not running in expected folder 'User'!"
        sys.exit(1)
...
Builds the Win7 user migration tools.
"""
    ...
    toolsList = ["Win7UserMigrate_Part1.py", "Win7UserMigrate_Part2.py"]
    toolsFolder = "UserMigrationTools"

    if options.product is not None:
        if options.product == "part1":
            toolsList = ["Win7UserMigrate_Part1.py"]
            toolsFolder = "UserMigrationPart1"

        elif options.product == "part2":
            toolsList = ["Win7UserMigrate_Part2.py"]
            toolsFolder = "UserMigrationPart2"
    ...
    if not options.skipConfirm:
        ...
        if response == "Y" or response == "y":
            ...
            print "Win7 User Migration Tools build canceled"
            sys.exit(0)
    ...
    _buildExes("setupWin7UserMigrationTools.py", toolsList)
