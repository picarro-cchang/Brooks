# File Name: DiagEventCounterTableGui.py
# Purpose: This module handles the EventCounterTable page
#
# File History:
...
####################################################################
#
class EventCountTableGui(wx.Panel):
    ...
    def CreateControls(self):
        ...
        list = []
        for table in range(len(Global.DIAG_EventCountTableTypeDict)-1):
            list.append( Global.DIAG_EventCountTableTypeDict[table] )
    ...
    def onUpdateButtonClick(self,parent):
        numEntries = self.diag.DIAG_ReadTableTotal( Global.DIAG_EventCountTables, self.tableNum )
        self.table = self.diag.DIAG_ReadEventCountTable( self.tableNum, numEntries )
...
class GeneralList( wx.ListCtrl ):
    ...
    def OnGetItemText(self, item, col):
        if len(self.table) <= 0:
            return ""
        key = Global.DIAG_EventTypeDict[item]
        if col==0:
            return key
