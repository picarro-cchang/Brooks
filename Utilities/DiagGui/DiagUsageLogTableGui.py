# File Name: DiagUsageLogTableGui.py
# Purpose: This module handles the UsageLogTable page
#
# File History:
...
####################################################################
#
class UsageLogTableGui(wx.Panel):
    ...
    def CreateControls(self):
        ...
        list = []
        for table in range(len(Global.DIAG_UsageTableTypeDict)-1):
            list.append( Global.DIAG_UsageTableTypeDict[table] )
    ...
    def onUpdateButtonClick(self,parent):
        numEntries = self.diag.DIAG_ReadTableTotal( Global.DIAG_UsageLogTables, self.tableNum )
        self.table = self.diag.DIAG_ReadUsageLogTable( self.tableNum, numEntries )
...
class GeneralList( wx.ListCtrl ):
    ...
    def SetupList(self):
        ...
        self.InsertColumn( 0, "" )
        col = 1
        for field in Global.DIAG_UsageLogStruct._fields_:
