...
z = []  # Position from GPS
dz = [] # Raw velocities from GPS
zhead = [] # Heading calculated from wind speed

OFFSET = -1000   # Offset for weather station data to be in sync with GPS data

def syncSources(msInterval):
    """Yields timestamp, GPS data and weather station data, all interpolated
    onto a regular grid, sampled at interval msInterval (in milliseconds)"""
    while True:
...
SyncCdataTuple = namedtuple('SyncCdataTuple',['ts','lon','lat','zPos','wHead','wDir'])
...
def syncCdata(cartData,syncData):
    ...
    wDir = ws.WS_COS_DIR+1j*ws.WS_SIN_DIR
    return SyncCdataTuple(ts,lon,lat,zPos,wHead,wDir)
...
DerivCdataTuple = namedtuple('DerivCdataTuple',['ts','lon','lat','zPos','wHead','wDir','zHead'])

def derivCdataSource(syncCdataSource):
    zBuff = []
    for d in syncCdataSource:
        zBuff.append((d.ts,d.zPos))
        zBuff[:] = zBuff[-3:]
        if len(zBuff) == 3:
            ts2, z2 = zBuff[2]
            ts0, z0 = zBuff[0]
            zHead = 1000.0*(z2-z0)/(ts2-ts0)
            yield DerivCdataTuple(*(d+(zHead,)))
...
for i,d in enumerate(derivCdataSource(syncCdataSource)):
    now = time.time()
    if abs(d.zHead) > 2.0:  # Car is moving fast if speed > 5m/s
        gps_head1.append(toDeg*angle(d.zHead))
        ws_head1.append(toDeg*angle(d.wHead))
        ws_dir1.append(toDeg*angle(d.wDir))
    else:
        gps_head2.append(toDeg*angle(d.zHead))
        ws_head2.append(toDeg*angle(d.wHead))
        ws_dir2.append(toDeg*angle(d.wDir))
    if now-lastPlot > 3.0:
        line1.set_data(gps_head1,ws_head1)
        line2.set_data(gps_head2,ws_head2)
        line3.set_data(gps_head1,ws_dir1)
