...
var CNSNT = {
        ...
        //  and an optional checkbox which can be inserted before the first column to allow a row to be
        //  selected. The key checkBoxHeading is present if the checkbox is to be included, and its value
        //  is used to label the checkbox column in the header line
        regions: {id:"regionTable",headings:['Plat','Min Lng','Max Lng','Min Lat','Max Lat','Base Image'],checkBoxHeading:'',
                  parsers:[String,parseFloat,parseFloat,parseFloat,parseFloat,String]},
        runs: {id:"runTable",headings:['Analyzer','Start Epoch','End Epoch','Path','Peaks','Wedges','Swath','Min Ampl','Max Ampl','Excl Radius','Stab Class'],checkBoxHeading:'',
...
function makeEmptyTable(template)
{
    ...
    if (template.hasOwnProperty('checkBoxHeading')) {
        result += '<table id="'+id+'" class="withCheckbox"><thead><tr>';
        result += '<th>' + template.checkBoxHeading + '</th>';
    } else {
...
function addToTable(template,data)
{
    ...
    if (template.hasOwnProperty('checkBoxHeading')) {
        newRow += '<td><input type="checkbox" onchange="$(this).parentsUntil(\'tbody\',\'tr\').toggleClass(\'selected\')"/></td>';
    }
    ...
    $('#' + template.id + ' > tbody tr').each( function(index,tableRow) { 
        // We need to check equality of newRow and an existing row in the table. However, if there is a checkbox,
...
function tableAsStruct(template)
{
    ...
    $('#' + template.id + ' > tbody tr').each( function(ir,row) {
        ...
        $(this).children('td').each( function(ic,col) {
            if (!(template.hasOwnProperty('checkBoxHeading') && ic==0)) {
...
function initialize(winH,winW)
{
    ...
    $("#id_delete_regions_button").click(function(){ $("#regionTable .selected").remove(); });
