...
INSTALLER_SCRIPTS_DIR = ('src', 'main', 'python', 'AddOns', 'DatViewer', 'InstallerScriptsWin7')

RELEASE_VERSION_FILES = [('src', 'main', 'python', 'AddOns', 'DatViewer', 'release_version.py')]
INTERNAL_VERSION_FILES = [('src', 'main', 'python', 'AddOns', 'DatViewer', 'setup_version.py')]

JSON_VERSION_FILE = ('src', 'main', 'python', 'AddOns', 'DatViewer', 'version.json')

class BuildDatViewer(Builder):
    def __init__(self, project, logger):
        super(BuildAiAutosampler, self).__init__(project, logger)
        logger.info("Instantiating BuildDatViewer")

    def initialize(self, product):
        project = self.project
        logger = self.logger
        assert product.lower() == "datviewer"
        project = self.project
    ...
    def publish(self):
        ...
        output_file_path = self.get_report_file_path("build_datviewer_exe")
        with open(output_file_path, "a") as output_file:
            output_file.write("=== %s ===\n" % time.asctime())
            cmd = "doit dist_dir=%s build_datviewer_exe" % project.expand_path("$dir_dist")
            stdout, return_code = self.run_command(cmd, True)
            output_file.write(stdout)
            if return_code != 0:
                raise BuildFailedException("Error while executing run_py2exe/build_datviewer_exe")
    ...
    def make_installers(self):
        ...
        iss_filename = "setup_DatViewer.iss"
        ...
        output_file_path = self.get_report_file_path("make_datviewer_installer")
        with open(output_file_path, "a") as output_file:
            stdout, return_code = self.run_command(" ".join(args), True)
            output_file.write(stdout)
            if return_code != 0:
                raise BuildFailedException("Error while making DatViewer installer")
