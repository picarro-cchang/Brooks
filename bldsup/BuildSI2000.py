...
class BuildSI2000(Builder):
    ...
    def initialize(self, product):
        ...
        # Save build types in a file, so unit tests and integration tests can use
        source_dir = project.expand_path('$dir_source_main_python')
        write_file = os.path.join(source_dir, "Host", "Utilities", "BuildHelper", "BuildTypes.py")
        with open(write_file, "w") as f:
            f.write("# Auto generated by PyBuilder\n")
            f.write("buildTime = %s\n" % (time.time()))
            f.write("buildTypes = ['%s']" % ("', '".join(types_to_build)))
