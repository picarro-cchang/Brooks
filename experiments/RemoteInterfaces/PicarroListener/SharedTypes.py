...
RPC_PORT_FSR_HOPPING_CONTROLLER = 50016
RPC_PORT_STARTUP_CALIBRATOR = 50018
...
class makeHandler(object):
    ...
    def process(self, timeLimit):
        ...
        """
        assert isinstance(timeLimit, (int, float))
        start = time.time()
        nprocessed = 0
        finished = False
...
def ctypesToDict(structure):
    ...
    Returns: Dictionary with structure which mirrors the original structure, or the
        argument itself.
    """
    if isinstance(structure, (float, int, str)):
        return structure
    else:
...
def lookup(symbol):
    ...
    """
    assert isinstance(symbol, (int, float, str))
...
class Operation(object):  # pylint: disable=R0903
    ...
    def __init__(self, opcode, operandList=None, env=0):
        assert isinstance(opcode, (int, str))
        assert isinstance(operandList, type(None)) or hasattr(
...
class OperationGroup(object):
    ...
    def __init__(self, priority, period, operationList=None):
        ...
        assert isinstance(operationList, type(None)) or hasattr(
