...
uint8_t tca9539_read(uint8_t slave_address, uint8_t register_address) {
 ...
 begin:
  ...
  case TW_START:
    ...
    logger_msg_p("tca9539",log_level_DEBUG,PSTR("Set start condition"));
    break;
  case TW_MT_ARB_LOST:
    logger_msg_p("tca9539",log_level_DEBUG,PSTR("Arbitration lost"));
    goto begin;

  default:
    logger_msg_p("tca9539",log_level_ERROR,PSTR("Could not set start condition"));
    return -1;
  }
  ...
  while ((TWCR & _BV(TWINT)) == 0);
  twst = TW_STATUS;
  logger_msg_p("tca9539", log_level_DEBUG, PSTR("TWI status is 0x%x"),twst);
  switch (twst) {
  case TW_MT_SLA_ACK:
    logger_msg_p("tca9539", log_level_DEBUG, PSTR("SLA+W transmitted, ACK received"));
    break;

  case TW_MT_SLA_NACK:
    logger_msg_p("tca9539", log_level_DEBUG,
  ...
  default:
    logger_msg_p("tca9539",log_level_ERROR,PSTR("Could not select device"));
  ...
  switch (twst) {
    case TW_MT_DATA_ACK:
      ...
      logger_msg_p("tca9539", log_level_DEBUG, PSTR("Data transmitted, ACK received"));
      break;

    case TW_MT_DATA_NACK:
      // Data transmitted, NACK received
      logger_msg_p("tca9539", log_level_DEBUG, PSTR("Data transmitted, NACK received"));
      goto quit;
  ...
  case TW_REP_START:
    ...
    logger_msg_p("tca9539",log_level_DEBUG,PSTR("Set repeated start condition"));
    break;
  ...
  for (int8_t bytenum = 0; bytenum >= 0; bytenum--) {
    ...
    case TW_MR_DATA_NACK:
      logger_msg_p("tca9539", log_level_DEBUG, PSTR("NACK received after data request"));
      // No data or end of data -- fall through

    case TW_MR_DATA_ACK:
      data_union.b[bytenum] = TWDR;
      logger_msg_p("tca9539", log_level_DEBUG, PSTR("Read %u from register %u"),
...
int8_t tca9539_write(uint8_t slave_address, uint8_t register_address, uint8_t data) {
 ...
 begin:
  ...
  case TW_START:
    // Start condition transmitted
    logger_msg_p("tca9539",log_level_DEBUG,PSTR("Start condition set"));
    break;
  case TW_MT_ARB_LOST:
    logger_msg_p("tca9539",log_level_DEBUG,PSTR("Arbitration lost"));
    goto begin;
  ...
  switch (twst) {
    case TW_MT_DATA_ACK:
      logger_msg_p("tca9539", log_level_DEBUG, PSTR("Slave ACKed address 0x%x"),
		   register_address);
      // Data transmitted, ACK received
      break;
