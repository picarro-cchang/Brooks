...
export class NetworkPanelEditor extends PureComponent<PanelEditorProps<NetworkOptions>> {
    ...
    onNetworkTypeChange = networkType => {
        if (networkType.value === 'DHCP') {
            this.props.onOptionsChange({ ...this.props.options, ip: ''});
        ...
        this.props.onOptionsChange({ ...this.props.options, networkType: networkType.value});
    };
    onIPChange = event => {
        this.enableApplyButton();
        this.enableUndoButton();
        this.props.onOptionsChange( { ...this.props.options, ip: event.target.value });
    };
    onGatewayChange = event => {
        this.enableApplyButton();
        this.enableUndoButton();
        this.props.onOptionsChange({...this.props.options, gateway: event.target.value});
    };
    onNetmaskChange = event => {
        this.enableApplyButton();
        this.enableUndoButton();
        this.props.onOptionsChange({...this.props.options, netmask: event.target.value});
    };
    onDnsChange = event => {
        this.enableApplyButton();
        this.enableUndoButton();
        this.props.onOptionsChange({...this.props.options, dns: event.target.value});
    };
    handleApplyClick() {
        console.log('click');
    ...
    getNetworkSettings () {
        PicarroAPI.getRequest(getRoute).then(response => {
            response.text().then(data => {
                ...
                this.props.onOptionsChange({ ...this.props.options, networkType: jsonData['networkType']});
                this.props.onOptionsChange({ ...this.props.options, ip: jsonData['ip']});
                this.props.onOptionsChange({ ...this.props.options, gateway: jsonData['gateway']});
                this.props.onOptionsChange({ ...this.props.options, netmask: jsonData['netmask']});
                this.props.onOptionsChange({ ...this.props.options, dns: jsonData['dns']});
            })
        })
    };
    checkInput = () => {
