...
def get_last_decim(client, measurement):
    rs = client.query(
        "SELECT last_decimation FROM crds_workspace_1 WHERE meas_name='{measurement}' ORDER BY time DESC LIMIT 5".format(
...
while True:
    ...
    if stop_time_ms > start_time_ms:
        ...
        rs = client.query("SELECT min(*),max(*),mean(*),count(*) INTO crds1_15s FROM crds " +
                          "WHERE time>=%dms AND time<%dms AND valve_stable_time>30 GROUP BY time(15s),*" %
                          (start_time_ms, stop_time_ms))
        client.write_points([{
            "measurement": "crds_workspace_1",
            "fields": {
