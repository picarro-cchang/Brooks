...
def get_last_decim(client, measurement):
    rs = client.query("SELECT last_decimation FROM crds_workspace_1 WHERE meas_name='{measurement}' ORDER BY time DESC LIMIT 5".format(
...
def compress_measurement(client, src_meas, dest_meas, field_key, duration, start_time_ms, stop_time_ms):
    start = time.time()
    rs = client.query(
        'SELECT sum_tot/sum_count AS "mean_{field_key}", sum_count AS "count_{field_key}",'
...
while True:
    ...
    if stop_time_ms > start_time_ms:
        ...
        start = time.time()
        rs = client.query("SELECT min(*),max(*),mean(*),count(*) INTO crds1_15s FROM crds " +
                        "WHERE time>=%dms AND time<%dms AND valve_stable_time>30 GROUP BY time(15s),*" % (start_time_ms, stop_time_ms))
        client.write_points([{"measurement": "crds_workspace_1", "fields": {"last_decimation": int(stop_time_ms)}, "tags": {"meas_name": "crds"}}])

        for src_duration, dest_duration, dest_duration_ms in zip(durations[:-1], durations[1:], durations_ms[1:]):
            src_meas = measurement + "1_" + src_duration
            dest_meas = measurement + "1_" + dest_duration
            ...
            client.write_points([{"measurement": "crds_workspace_1", "fields": {"last_decimation": int(stop_time_ms)},
