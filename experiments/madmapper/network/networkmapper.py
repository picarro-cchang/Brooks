...
class NetworkMapper(object):
    ...
    def get_all_active_hosts(self):
        ...
        hosts = []
        ip_addresses = [subprocess.Popen(['ping', '-c', '1', '-W', '1', f'{network_prefix}{x}'],
                                stderr=subprocess.PIPE,
    ...
    def get_all_picarro_hosts(self):
        ...
        for ip_address in active_hosts:
            if self.is_picarro_host(ip_address) == True:
                driver = CmdFIFO.CmdFIFOServerProxy(f'http://{ip_address}:{host_port}',
            ClientName='NetworkMapper')
            try:
                instrument_dict = driver.fetchLogicEEPROM()[0]
                serial_number = f'{instrument_dict.get("Chassis")}-{instrument_dict.get("Analyzer")}{instrument_dict.get("AnalyzerNum")}'
