...
class UsbRelay:
    ...
    def do_full_disco(self, cycles):
        for k in range(cycles):
            print(f"Cycle {k} in progress, {100*((k+1.)/cycles)}% done")
            for i in range(self.relayCount):
                self.do_disco(i)
...
def main():
    ...
    print(f"Numato Relay Board Driver for {args.serial_port_name} created.")
    print(f"RPC server will be available at {args.rpc_server_port} in a sec.")

    try:
        numato_driver.rpc_serve_forever()
    except KeyboardInterrupt:
        print("RPC server has ended it's lifecycle after brutal KeyboardInterrupt, good job.")

    print("RPC server has ended")
