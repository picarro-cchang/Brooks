...
class _Method:
    ...
    def __call__(self, *args, **kwargs):
        try:
            ...
            raise RemoteException("".join(
                Pyro4.util.getPyroTraceback())) from e
...
class CmdFIFOServerTunnel(CmdFIFOServer):
    ...
    def register_function(
        ...
        if proxy is None:
            ...
            if name is None or original_name is None:
                raise CmdFIFOError(
                    "Need to supply both name and original_name")
    ...
    def system_methodSignature(self, method_name):
        ...
        if method_name in self.proxys:
            return self.proxys[method_name][0].system.methodSignature(
                self.proxys[method_name][1])
        else:
    ...
    def system_methodHelp(self, method_name):
        ...
        Returns a string containing documentation for the specified method."""
        if method_name in self.proxys:
            return self.proxys[method_name][0].system.methodHelp(
                self.proxys[method_name][1])
        else:
            return super().system_methodHelp(method_name)


def register_proxy_rpcs_from_configs(rpc_tunnel_configs,
                                     RpcTunnelServer,
                                     master_ip=None):
    """
    ...
    for proxy_config in rpc_tunnel_configs:
        if master_ip is None:
            ...
            ip = master_ip
        proxy = CmdFIFOServerProxy("http://{}:{}".format(
            ip, proxy_config["proxy_server_port"]),
        ...
        for f in proxy_config["functions"]:
            ...
            if "new_name" not in f or f[
                    "new_name"] == "" or f["new_name"] is None:
