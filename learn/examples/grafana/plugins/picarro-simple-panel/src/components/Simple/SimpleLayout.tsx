...
export class SimpleLayout extends Component<Props, any> {
    ...
    onButtonClick(e: any) {
        PicarroAPI.getRequest("http://numbersapi.com/random/trivia").then(response => {
            response.text().then(text => {
                ...
                PicarroAPI.postData("http://localhost:8888/print", { message: text }).then(response => {
    ...
    onValveChange(e: any) {
        PicarroAPI.postData("http://localhost:8888/valve_mode", { mode: e.target.value }).then(response => {
            response.text().then(text => { console.log(text) });
        });
    }
        ...
        return (
            <div>
                ...
                <div>
                    ...
                    <button className="btn btn-primary btn-large" onClick={(e) => this.onButtonClick(e)}>
                        Click Me
                    </button>
                    <div>Message = {this.state.message}</div>
                </div>
                <IntervalRenderer interval={1000}>
                    <RadioButtons name="opt" data={data} perRow={4} valueField={"value"} textField={"text"}
