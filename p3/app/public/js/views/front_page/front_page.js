...
// {"identity":"081ebf61998a99032c362110b0acf7df","sys":"investigator demo"}
	...
	var init = function(){
		...
		$('.layout-container').click(function(){
			if($('body').hasClass("vertical-layout")){
				...
				$('body').addClass("horizontal-layout");
			}else{
				$('body').removeClass("horizontal-layout");
	...
	var shareLog = function(log_name, baseline){
		...
		$.ajax({
		  ...
		  data_type:"JSON"
			...
			var url = "http://dev.picarro.com/dataview?s=" + d.short_url.split("/")[2]
			$('.share-input').val(url)
	...
	var downloadFile = function(){
		var log_name = $(this).attr("id").split("$")[1] + ".dat";
			// this doesn't quite work. pushing it
			DS.getLogFile(log_name, function(return_url,bar){
				// $.get(return_url);
				window.open(
		          return_url,
		          '_blank' // <- This is what makes it open in a new window.
		        );
						//  Todo - pass in datatype of file? http://api.jquery.com/jQuery.get/
						// $.get('https://dev.picarro.com/investigator/rest/gdu/4aec765b96e386a723388ff6724ae0a1/1.0/AnzLog?qry=byEpoch&alog=CFADS2290-20130319-223852Z-DataLog_User_Minimal.dat.dat&startEtm=0&decimated=8&varList=["CH4","GPS_ABS_LAT","GPS_ABS_LONG"]&limit=25000&rtnFmt=file', function(data) {alert(data);
		        //       alert('Load was performed.');
		        //     });
				log('attempted download of '+  return_url)
			} );
	...
	var initMap = function(){
		log_map = L.map('map_container', {
			maxZoom:18,
            attributionControl:false
        }).setView([40.54654795, -75.588032833], 13);
	...
	var renderDays = function(){
	        		...
	        		dom.push('<div class="accordion-inner">')	        		
						...
						for(var i = 0 ; i < logs.length ; i++){
							if(!map_filter_active || map_filter_hash[logs[i].short_name]){
								...
								dom.push('<h5><i class="p3-icon-plus"></i><i class="p3-icon-minus"></i>' + logs[i].short_name.split('-')[0] + ' : ' + dateFormat(logs[i].time,"UTC:hh:MM TT") + ' : ')	
								dom.push('<span> ' + secondsToString(logs[i].durr) + '</span></h5>')
								dom.push('<div class="bar-wrapper">')
								dom.push('<div class="launch-modal btn btn-small btn-inverse" id="modal$' + logs[i].short_name + '">Share</div>')
								dom.push('<div class="download btn btn-small btn-inverse" id="modalDownload$' + logs[i].short_name + '">Download</div>')
								dom.push('<div class="launch-investigate btn btn-small btn-inverse" id="investigate$' + logs[i].short_name + '">Investigate</div>')
								dom.push('</div>')
		...
		$('.log-wrapper h5').on("click", function(){
			if($(this).hasClass("toggle-open")){
				...
				$(this).parent().find(".log-content").animate({height:100})
			}
		})
	}
