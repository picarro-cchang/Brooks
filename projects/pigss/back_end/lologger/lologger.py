...
def get_epochtime_of_the_local_month():
    # please somebody kill me or double check my math or give me a better way to make this thing work
    # this function is supposed to generate an epoch time of when *month starts and ends in the local timezone*
    # first - generate offset from UTC
    utc_offset = calendar.timegm(time.localtime()) - calendar.timegm(time.gmtime())

    # second - get local current (year and month) and local next months (year and month)
    ...
    # third - generate epoch time from a struct_time object, generated by string, minus the utc_offset
    start = calendar.timegm(time.strptime(f"{current_year}-{current_month:0{2}}", "%Y-%m")) - utc_offset
    # end of the current month is a beginning of the next month minus one, right? i am not going crazy, right?
    end = calendar.timegm(time.strptime(f"{next_year}-{next_month:0{2}}", "%Y-%m")) - utc_offset - 1
    return start, end
...
class LOLoggerThread(threading.Thread):
    ...
    def get_connection(self, db_path):
        ...
        if self.redundant_json:
            ...
            self.add_enrty_to_file_tracking(json_file_path)
    ...
    def add_enrty_to_file_tracking(self, db_path):
        ...
        if os.path.exists(self.file_tracking_db_path):
            ...
            self.file_tracking_db_connection = sqlite3.connect(self.file_tracking_db_path)
            query = '''CREATE TABLE db_entry (filepath TEXT NOT NULL,
        ...
        if len(cursor.fetchall()) == 0:
            # if not - calculate epoch times of the beggining and end of the current month in the local timezone
            start, end = get_epochtime_of_the_local_month()

            # insert to file_tracking_db
            cursor.execute(''' INSERT INTO db_entry(filepath,start_date,end_date)
              VALUES(?,?,?) ''', (db_path,start ,end))
