...
class TimeAggregationService(ServiceTemplate):
    ...
    async def on_startup(self, app):
        log.debug("Time aggregation service is starting up")
    ...
    async def on_shutdown(self, app):
        log.debug("Time aggregation service is shutting down")
        ...
        if 'target' in req['targets'][0]:
            ...
            if best_duration == '0':
                ...
                rs = await self.client.query(
                    ...
                    f' (SELECT ("mean_{target}"*"count_{target}") AS tot, "count_{target}" AS count,'
                    f' "max_{target}" AS max, "min_{target}" AS min FROM crds_{best_duration}.crds_{best_duration}'
