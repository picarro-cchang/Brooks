...
from common.string_pickler import (ArbitraryObject, BadDataBlock, ChecksumErr, IncompletePacket, InvalidHeader, bytes_as_object,
                                   unpack_arbitrary_object)
...
class Listener(threading.Thread):
    ...
    def __init__(self,
        ...
        try:
            if ArbitraryObject in self.elementType.__mro__:
    ...
    def _ProcessArbitraryObjectStream(self) -> None:
        while 1:
            try:
                obj, residual = unpack_arbitrary_object(self.data)  # type: Any, bytes
    ...
    def _ProcessCtypesStream(self) -> None:
        while len(self.data) >= self.recordLength:
            result = bytes_as_object(self.data[0:self.recordLength], self.elementType)  # type: Any
            if self.streamFilter is not None:
