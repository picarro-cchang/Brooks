# buildVaporizerCleaner.py
#
# Set up the build environment and build Host components. Use this
# for building the VaporizerCleaner application from the command line.
#
# This executes python vaporizerCleanerSetup.py py2exe and ensures PYTHONPATH is properly
# set to pull local libs from its own folder tree instead of what is
# in Picarro.pth.
#
# This script can be run from a cmd window. vaporizerCleanerSetup.py should NOT be run
...
def buildExes():
    ...
    if curDir.lower() != "vaporizercleaner":
        print "Not running in expected folder 'VaporizerCleaner'!"
        sys.exit(1)
    ...
    # run "python vaporizerCleanerSetup.py py2exe"
    retCode = subprocess.call(['python.exe', 'vaporizerCleanerSetup.py', 'py2exe'], env=buildEnv)
...
def _generateBuildVersion(product, osType, ver):
    ...
    with open('setup_version.py', 'w') as fp:
        fp.writelines(
            ["# autogenerated by buildVaporizerCleaner.py, %s\n" % time.asctime(),
             "\n",
             "def versionString():\n",
...
def main():
    ...
    osType = _getOsType()
    product = "VaporizerCleaner-%s-INTERNAL" % osType
