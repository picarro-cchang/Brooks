...
Driver for building the VaporizerCleaner installer package.
"""
...
def makeInstaller(opts):
    """
    Build the VaporizerCleaner installer from a clean checkout.
    """
    ...
    # productFamily incorporates the OS type (e.g., 'VaporizerCleaner_win7')
    productFamily = "VaporizerCleaner_%s" % osType
    ...
    print "VaporizerCleaner build script start: %s" % time.strftime("%Y/%m/%d %H:%M:%S %p", startTime)
    ...
    if opts.buildExes:
        ...
        exeDir = os.path.join(SANDBOX_DIR, "host", "AddOns", "VaporizerCleaner", "dist")
...
def _compileInstaller(product, osType, ver):
    ...
    setupFilePath = "%s\\setup_VaporizerCleaner.iss" % installScriptDir

    print "building from '%s'" % setupFilePath

    args = [ISCC,
            "/dvaporizerCleanerVersion=%s" % _verAsString(product, ver),
    ...
    #with OS.chdir(os.path.join(SANDBOX_DIR, 'host', 'AddOns', 'VaporizerCleaner')):
...
def _generateReleaseVersion(product, ver):
    ...
    with OS.chdir(os.path.join(SANDBOX_DIR, 'host')):
        filename = os.path.normpath(os.path.join(os.getcwd(), 'AddOns', 'VaporizerCleaner',
                                                 'release_version.py'))

        with open(filename, 'w') as fp:
            fp.writelines(
                ["# autogenerated by VaporizerCleaner release.py, %s\n" % time.asctime(),
                 "\n",
                 "def versionString():\n",
...
def _buildExes():
    """
    Build VaporizerCleaner executables.
    """
    ...
    with OS.chdir(os.path.join(SANDBOX_DIR, 'host', 'AddOns', 'VaporizerCleaner')):
        retCode = subprocess.call(['python.exe', 'vaporizerCleanerSetup.py',
                                   'py2exe'],
                                   env=buildEnv)

        if retCode != 0:
            print "Error building VaporizerCleaner. retCode=%d" % retCode
            sys.exit(retCode)
...
Builds a new release of the VaporizerCleaner installer.
"""
