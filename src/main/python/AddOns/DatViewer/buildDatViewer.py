...
# for building DatViewer.exe from the command line.
#
# This executes python datviewerSetup.py py2exe and ensures PYTHONPATH is properly
# set to pull local libs from its own folder tree instead of what is
# in Picarro.pth.
#
# This script can be run from a cmd window. datviewerSetup.py should NOT be run
...
def buildExes():
    ...
    if curDir.lower() != "datviewer":
        print "Not running in expected folder 'DatViewer'!"
        sys.exit(1)
    ...
    retCode = subprocess.call(['python.exe', 'datviewerSetup.py', 'py2exe'], env=buildEnv)

    if retCode != 0:
            print "Error building DatViewer. retCode=%d" % retCode
            sys.exit(retCode)
...
def _generateBuildVersion(product, osType, ver):
    ...
    with open('setup_version.py', 'w') as fp:
        fp.writelines(
            ["# autogenerated by buildDatViewer.py, %s\n" % time.asctime(),
             "\n",
             "def versionString():\n",
