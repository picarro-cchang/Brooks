...
class Calibration(wx.Frame):
    def __init__(self,main_frame):
        ...
        self.id_title =wx.StaticText(self.setup_IDs_panel, -1, 'Isotope Value', size=id_size)
        self.id_0 = wx.TextCtrl(self.setup_IDs_panel, -1, 
                value = self.main_frame.standards_library['Standard_1'][isotope_index], size = id_size)
        self.id_1 = wx.TextCtrl(self.setup_IDs_panel, -1,
                value = self.main_frame.standards_library['Standard_2'][isotope_index], size = id_size)
        self.id_2 = wx.TextCtrl(self.setup_IDs_panel, -1,
                value = self.main_frame.standards_library['Standard_3'][isotope_index], size = id_size)
            
        self.rg_title = wx.StaticText(self.setup_IDs_panel, -1, 'Standard Runs', size=id_size)
        self.rg_0 = wx.TextCtrl(self.setup_IDs_panel, -1, 
                value = self.main_frame.standards_library['Standard_1'][range_index], size = id_size)
        self.rg_1 = wx.TextCtrl(self.setup_IDs_panel, -1,
                value = self.main_frame.standards_library['Standard_2'][range_index], size = id_size)
        self.rg_2 = wx.TextCtrl(self.setup_IDs_panel, -1,
                 value = self.main_frame.standards_library['Standard_3'][range_index], size = id_size)
        ...
        number_of_columns = self.main_frame.page_two.sidebar.page_one.data_grid.GetNumberCols()
        label_lst = []
        first_two_columns = 1
        ...
        text_selection = wx.StaticText(self.setup_IDs_panel,-1,'Select Species for Calibration:', size = (150,-1))
        
        sizer_column_selection = wx.BoxSizer(wx.HORIZONTAL)
        sizer_column_selection.Add(wx.StaticText(self.setup_IDs_panel,-1,'',size=(10,-1)))
        ...
        text_buffer = wx.StaticText(self.setup_IDs_panel,
                                 -1,
                                 '',
                                 size =(-1,-1))
        text_one = wx.StaticText(self.setup_IDs_panel,
                                 ...
                                 text_string,
                                 size =(-1,-1))
        text_two =  wx.StaticText(self.setup_IDs_panel,
                                 -1,
                                 '',
                                 size =(-1,10))
        text_three = wx.StaticText(self.setup_IDs_panel,
                                 -1,
                                 '',
                                 size =(-1,10))
    ...
    def calculate_calibration(self):
        '''calculate the linear regression using the user choosen calibration data'''
        shift_index = 1
        isotope_index = 0
        ...
        if self.main_frame.data_loaded_flag:
            ...
            for i,standard in enumerate(self.st_lst):
                    ...
                    try:
                        ...
                        if standard_range != empty_list:
                            ...
                            for value in range_split:
                                measured = self.main_frame.page_two.plot_area.peak_data[isotope_index][self.calibration_species+1][int(value)-shift_index]
                                print measured, np.isnan(measured)
                                if np.isnan(measured):
                                    missing_data_flag = True
                                else:
                                    print value.strip(), measured, isotope_value
                                    self.calibration_dict[str(value.strip())] = [measured,isotope_value]
                    except:
                        pass
            if not missing_data_flag:
                ...
                wx.MessageBox('Encountered blank data assigned to calibration species. Please check data and try again.',
                                          'Error', wx.OK | wx.ICON_ERROR)
            
    def calibrate_all_data(self):
        '''calibrates all data using the optimal linear regression from the calculate_calibration method'''
        isotope_index = 0
        index_shift = 1
        skip_first_two_columns = 2
        number_of_columns = self.main_frame.page_two.sidebar.page_one.data_grid.GetNumberCols()
        number_of_rows = self.main_frame.page_two.sidebar.page_one.data_grid.GetNumberRows()
        ### copies over the data from the calibration in the event that no previous calibration was
        ### performed previously, i.e. the intercept and slope are not recorded in the peak data file.
        if self.main_frame.b_current == None and self.main_frame.m_current == None:
            ...
            ### logic necessary for recursive calibrations... since the series data remains raw
            ### only the peak integrated data is ever re-calibrated... this is necessary to 
            ### include nested linear fits in the manor... these parameters are used in the 
            ### plot_area_two in the reprocess_data_with_new_bounds...
            self.main_frame.b_current = self.main_frame.b_current*self.main_frame.m + self.main_frame.b
        ...
        for i in range(skip_first_two_columns,number_of_columns):
            for j in range(number_of_rows):
                try:
                    old_value = self.main_frame.page_two.plot_area.peak_data[isotope_index][i-index_shift][j]
                    if not np.isnan(old_value):
                        calibrated_value = self.main_frame.m*float(old_value)+self.main_frame.b
                        self.main_frame.page_two.sidebar.page_one.data_grid.SetCellValue(j,i,str(round(calibrated_value,2)))
                        self.main_frame.page_two.plot_area.peak_data[isotope_index][i-index_shift][j] = calibrated_value
                except ValueError:
                    pass
    ...
    def plot_calibration(self):
        ...
        SS_residual = sum([(y[i]-(self.main_frame.m*x[i]+self.main_frame.b))**2 for i,x_i in enumerate(x)])
        y_average = np.average(y)
        SS_total = sum([(y[i]-y_average)**2 for i,x_i in enumerate(x)])
        ...
        for i,standard in enumerate(self.st_lst):
            if i != skip_first_row:
                ...
                if standard_range != '':
                    ...
                    try:
                        range_split = standard_range.split(',')
                        str_range = str([int(value) for value in range_split]).replace('[','').replace(']','')
                        self.main_frame.standards_library[key_lst[i-index_shift]][range_index] = str_range
                        ...
                        self.main_frame.standards_library[key_lst[i-index_shift]][range_index] = ''
