...
Usage: python dataProcessorSetup.py py2exe

Notes: Use python buildDataProcessor.py from the command line, which sets up
...
def _getBuildVersion():
    try:
        ...
        # buildDataProcessor.py (command line script for internal builds)
        try:
...
def _getBuildType():
    try:
        ...
        # Next try setup_version which is generated by the
        # command line script for internal builds named buildxxx.py
        try:
            import setup_version as buildVersion
            buildTypeStr = buildVersion.buildType()
...
targetApp = Target(description = "PumpDataProcessor", # used for the versioninfo resource
                   script = "PumpDataProcessor.py", # what to build
                   other_resources = [(RT_MANIFEST,
                                      1,
                                      manifest_template % dict(prog="PumpDataProcessor")
                                      )],
                   ## does this work? was commented out
                   #icon_resources = [(1, "../../../Assets/icons/EnviroSense.ico")],
                   dest_base = "PumpDataProcessor"
                  )

# End of special DataProcessor setup stuff
################################################################
...
if "PYTHONPATH" not in os.environ:
    ...
    print "Run 'python buildDataProcessor.py' instead to build PumpDataProcessor from the command line."
    sys.exit(1)
...
data_files = [(".", ["../../../Assets/icons/EnviroSense.ico"]),
...
productName = "Picarro SDM DataProcessor"
if buildTypeStr != "":
    productName = "Picarro SDM DataProcessor (%s)" % buildTypeStr

descStr = "Picarro SDM Software"
