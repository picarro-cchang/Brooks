...
CustomConfigObj -- responsible for parsing a list of
    ...
    set_subsection(self, sectionlist, option, value)
        set the option value to a deeper level of section
...
class CustomConfigObj(configobj.ConfigObj):
    ...
    def set_subsection(self, sectionlist, option, value, format = None):
        if format != None:
            value = format % value
        sectionDict = self.__getitem__(sectionlist[0])
        for section in sectionlist[1:]:
            sectionDict = sectionDict.__getitem__(section)

        if not self.ignoreOptionCase:
            sectionDict.update({option: str(value)})
        else:
            for optionKey in sectionDict.keys():
                if optionKey.lower() == option.lower():
                    sectionDict.update({optionKey: str(value)})
                    return
