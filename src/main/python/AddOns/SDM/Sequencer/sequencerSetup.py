...
Usage: python sequencerSetup.py py2exe

Notes: Use python buildSequencer.py from the command line, which sets up
...
def _getBuildVersion():
    try:
        ...
        # buildSequencer.py (command line script for internal builds)
        try:
...
targetApp = Target(description = "PumpSequencer", # used for the versioninfo resource
                   script = "PumpSequencer.py", # what to build
                   other_resources = [(RT_MANIFEST,
                                      1,
                                      manifest_template % dict(prog="PumpSequencer")
                                      )],
                   ## does this work? was commented out
                   #icon_resources = [(1, "../../../Assets/icons/EnviroSense.ico")],
                   dest_base = "PumpSequencer"
                  )

# End of special Sequencer setup stuff
################################################################
...
if "PYTHONPATH" not in os.environ:
    ...
    print "Run 'python buildSequencer.py' instead to build PumpSequencer from the command line."
    sys.exit(1)
...
productName = "Picarro SDM PumpSequencer"
if buildTypeStr != "":
    productName = "Picarro SDM Sequencer (%s)" % buildTypeStr
