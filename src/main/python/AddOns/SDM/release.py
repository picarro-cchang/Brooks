...
Driver for building the SDM installer package.
"""
...
def makeInstaller(opts):
    """
    Build the SDM installer from a clean checkout.
    """
    ...
    # productFamily incorporates the OS type (e.g., 'SDM_win7')
    productFamily = "SDM_%s" % osType
    ...
    print "SDM build script start: %s" % time.strftime("%Y/%m/%d %H:%M:%S %p", startTime)
    ...
    if opts.buildExes:
        ...
        # check for dist folders for the 3 apps that comprise SDM
        appFolders = ["DataProcessor", "Priming", "Sequencer"]

        for folder in appFolders:
            exeDir = os.path.join(SANDBOX_DIR, "host", "AddOns", "SDM", folder, "dist")
...
def _compileInstaller(product, osType, ver):
    ...
    setupFilePath = "%s\\setup_SDM.iss" % installScriptDir

    print "building from '%s'" % setupFilePath

    args = [ISCC,
            "/dsdmVersion=%s" % _verAsString(product, ver),
            "/dinstallerVersion=%s" % _verAsNumString(ver),
            "/dproductVersion=%s" % _verAsNumStringUI(ver),
    ...
    #with OS.chdir(os.path.join(SANDBOX_DIR, 'host', 'AddOns', 'SDM')):
...
def _generateReleaseVersion(product, ver):
    ...
    with OS.chdir(os.path.join(SANDBOX_DIR, 'host')):
        folders = ["DataProcessor", "Priming", "Sequencer"]

        for folder in folders:
            filename = os.path.normpath(os.path.join(os.getcwd(), 'AddOns', 'SDM', folder,
                                                     'release_version.py'))
            _generateReleaseVersionFilepath(product, ver, filename)


def _generateReleaseVersionFilepath(product, ver, filename):
    with open(filename, 'w') as fp:
        fp.writelines(
            ["# autogenerated by SDM release.py, %s\n" % time.asctime(),
             "\n",
             "def versionString():\n",
...
def _buildExes():
    ...
    setups = {"DataProcessor" : "dataProcessorSetup.py",
              "Priming"       : "primingSetup.py",
              "Sequencer"     : "sequencerSetup.py"}

    for folder in setups:
        scriptName = setups[folder]
        with OS.chdir(os.path.join(SANDBOX_DIR, 'host', 'AddOns', 'SDM', folder)):
            retCode = subprocess.call(['python.exe', scriptName,
                                       'py2exe'],
                                       env=buildEnv)

            if retCode != 0:
                print "Error building folder. scriptName=%s retCode=%d" % (scriptName, retCode)
                sys.exit(retCode)
...
Builds a new release of the SDM installer.
"""
