...
Usage: python vaporizerCleanerSetup.py py2exe

Notes: Use python buildVaporizerCleaner.py from the command line, which sets up
...
def _getBuildVersion():
    try:
        ...
        # buildVaporizerCleaner.py (command line script for internal builds)
        try:
...
VaporizerCleaner = Target(description = "VaporizerCleaner", # used for the versioninfo resource
                     script = "VaporizerCleaner.py", # what to build
                     other_resources = [(RT_MANIFEST,
                                        1,
                                        manifest_template % dict(prog="VaporizerCleaner")
                                        )],
                    ##    icon_resources = [(1, "cogwheel.ico")],
                    dest_base = "VaporizerCleaner"
                    )

# End of special VaporizerCleaner setup stuff (except to use VaporizerCleaner below)
################################################################
...
if "PYTHONPATH" not in os.environ:
    ...
    print "Run 'python buildVaporizerCleaner.py' instead to build VaporizerCleaner from the command line."
    sys.exit(1)
...
data_files = [(".", ["../../Assets/icons/cogwheel.ico",
                     "Vaporizer Cleaning Procedure 2-17-10.pdf"])]
...
productName = "Picarro VaporizerCleaner"
if buildTypeStr != "":
    productName = "Picarro VaporizerCleaner (%s)" % buildTypeStr
