...
from ctypes import byref, c_ubyte, c_uint, c_int, create_string_buffer
...
if __name__ == "__main__":
    ...
    #for i in range(1000):
        #tmp = (c_uint*nWords)()
        #check = c_ubyte()
        #usb.usbControlMsg(handle,0xC0,0xB4,0x0000,0x00,byref(check),1,5000)
        #status = usb.usbBulkRead(handle,epIn,byref(tmp),2*nWords,5000)
        #print "%d, %08x, %08x" % (status,htonl(tmp[0]),htonl(tmp[1]))
    
    #for i in range(20):
        #tmp = (c_uint*nWords)()
        #usb.usbBulkRead(handle,epIn,byref(tmp),4*nWords,5000)
        #print "%08x" % htonl(tmp[0])
        #usb.usbBulkRead(handle,epIn,byref(tmp),4*nWords,5000)
        #sleep(1.5)
        #check = c_ubyte()
        #usb.usbControlMsg(handle,0xC0,0xB4,0x0000,0x00,byref(check),1,5000)
        #print "%08x [%2x]" % (htonl(tmp[0]),check.value)
    ...
    while True:
        nWords = input("Number of 32-bit words to read? ")
        if nWords == 0: break
        tmp = (c_uint*nWords)()
        usb.usbBulkRead(handle,epIn,byref(tmp),4*nWords,5000)
        print "Error string: %s" % usb.usbStrerror()
        for k in range(nWords): 
            print "%08x" % htonl(tmp[k]),
        print
