...
#define LATTICE_FPGA_PROGRAMS_BASE (DSP_DATA_ADDRESS + 4 * LATTICE_FPGA_PROGRAMS_OFFSET)
...
volatile LatticeFpgaProgramType *latticeFpgaPrograms = (volatile LatticeFpgaProgramType *)LATTICE_FPGA_PROGRAMS_BASE;
...
void dspDataInit(void)
{
    ...
    memset((void *)latticeFpgaPrograms, 0, 4 * NUM_LATTICE_FPGA_PROGRAMS * LATTICE_FPGA_PROGRAM_PARAMS_SIZE);
}
