...
#
# DESCRIPTION:
#   Dual ported memory for storing ringdown and recorder information.
#
# This memory is used in one of two modes, selected using the "mode" port.
#
# When mode=0: (Used in traditional ringdown processing)
#   data_mem (8192x18 bit words, organized as two banks, each with 4096 points)
#       is used for storing ringdown waveforms
#   meta_mem (8192x16 bit words, organized as two banks, each with 8 channels of 512 points)
#       is used for storing metadata in the period immediately preceeding each ringdown
#   param_mem (128x32 bit words, organized as two banks, each consisting of 64 parameters)
#       is used for storing parameters for each ringdown
#
# When mode=1: (Used to characterize laser tuning by recording WLM outputs during a scan)
#   data_mem (8192x18 bit words, organized as two 4096 point recording buffers)
#       is used for storing ratio data acquired from the wavelength monitor
#   meta_mem (8192x16 bit words, organized as two 4096 point playback buffers)
#       is used for storing current waveforms for sync updates of the SGDBR laser
#
# SEE ALSO:
...
#   06-Apr-2018  sze  Added functionality for characterizing SGDBR laser tuning
#
#  Copyright (c) 2009-2018 Picarro, Inc. All rights reserved
#
from myhdl import Signal, always_comb, instances, intbv, toVHDL

from DualPortRamRw import DualPortRamRw
from Host.autogen.interface import (DATA_BANK_ADDR_WIDTH, EMIF_ADDR_WIDTH,
                                    EMIF_DATA_WIDTH, FPGA_REG_WIDTH,
                                    META_BANK_ADDR_WIDTH,
                                    PARAM_BANK_ADDR_WIDTH, RDMEM_DATA_WIDTH,
...
def AnalyzerMemory(clk, reset, dsp_addr, dsp_data_out, dsp_data_in, dsp_wr,
                   mode,
                   pb_data_out, pb_wfm_sel,
                   rec_data_in, rec_wfm_sel,
                   rec_addr, rec_we,
    ...
    wr_dataB = Signal(intbv(0)[RDMEM_DATA_WIDTH:])
    ...
    data_mem = DualPortRamRw(clockA=clk, enableA=enA_data, wr_enableA=wr_enable,
                             ...
                             wr_dataA=wr_dataA,
                             clockB=clk, enableB=enB_data, wr_enableB=weB_data,
    ...
    rd_metaB = Signal(intbv(0)[RDMEM_META_WIDTH:])
    sel_meta = LOW

    meta_mem = DualPortRamRw(clockA=clk, enableA=enA_meta, wr_enableA=wr_enable,
                             addressA=meta_addrA, rd_dataA=rd_metaA,
                             wr_dataA=wr_metaA,
                             clockB=clk, enableB=enB_meta, wr_enableB=weB_meta,
                             addressB=meta_addrB, rd_dataB=rd_metaB,
                             wr_dataB=wr_meta,
    ...
    def comb():
        ...
        sel_data_and_metadata = dsp_addr[:RDMEM_RESERVED_BANK_ADDR_WIDTH] == 6 or dsp_addr[
            :RDMEM_RESERVED_BANK_ADDR_WIDTH] == 2
        wr_enable.next = dsp_wr and not sel_data_and_metadata

        enB_data.next = 1
        sel_data = dsp_addr[:RDMEM_RESERVED_BANK_ADDR_WIDTH] == 4 or dsp_addr[
            :RDMEM_RESERVED_BANK_ADDR_WIDTH] == 0 or sel_data_and_metadata
        data_addrA.next[DATA_BANK_ADDR_WIDTH:] = dsp_addr[
        ...
        if mode == 0:  # This is ringdown mode
            data_addrB.next[DATA_BANK_ADDR_WIDTH:] = data_addr
            data_addrB.next[DATA_BANK_ADDR_WIDTH] = bank
            weB_data.next = data_we
            wr_dataB.next = wr_data
        else:  # This is recorder mode, data memory is used for recording
            data_addrB.next[DATA_BANK_ADDR_WIDTH:] = rec_addr
            data_addrB.next[DATA_BANK_ADDR_WIDTH] = rec_wfm_sel
            weB_data.next = rec_we
            wr_dataB.next = rec_data_in

        # Handle data write from DSP
        wr_dataA.next = dsp_data_out[RDMEM_DATA_WIDTH:]

        enB_meta.next = 1
        sel_meta = dsp_addr[:RDMEM_RESERVED_BANK_ADDR_WIDTH] == 5 or dsp_addr[
            :RDMEM_RESERVED_BANK_ADDR_WIDTH] == 1 or sel_data_and_metadata
        meta_addrA.next[META_BANK_ADDR_WIDTH:] = dsp_addr[
            META_BANK_ADDR_WIDTH:]
        # The bank is selected (on the DSP side) using dsp_addr[14]
        meta_addrA.next[META_BANK_ADDR_WIDTH] = dsp_addr[
            RDMEM_RESERVED_BANK_ADDR_WIDTH + 2]
        ...
        if mode == 0:  # This is ringdown mode
            ...
            weB_meta.next = meta_we
            meta.next = rd_metaB
            pb_data_out.next = 0
        else:  # This is recorder mode, meta memory is used for playback
            meta_addrB.next[META_BANK_ADDR_WIDTH:] = rec_addr
            meta_addrB.next[META_BANK_ADDR_WIDTH] = pb_wfm_sel
            weB_meta.next = LOW
            meta.next = 0
            pb_data_out.next = rd_metaB

        # Handle data write from DSP
        wr_metaA.next = dsp_data_out[RDMEM_META_WIDTH:]

        enB_param.next = 1
        sel_param = dsp_addr[:RDMEM_RESERVED_BANK_ADDR_WIDTH] == 7 or dsp_addr[
            :RDMEM_RESERVED_BANK_ADDR_WIDTH] == 3
        param_addrA.next[PARAM_BANK_ADDR_WIDTH:] = dsp_addr[
        ...
        if sel_data_and_metadata:
            ...
            dsp_data_in.next[EMIF_DATA_WIDTH:FPGA_REG_WIDTH] = rd_metaA[
                FPGA_REG_WIDTH:]
        elif sel_data:
...
if __name__ == "__main__":
    ...
    mode, pb_wfm_sel, rec_wfm_sel, rec_we = [Signal(LOW) for i in range(4)]
    pb_data_out = Signal(intbv(0)[RDMEM_META_WIDTH:])
    ...
    rec_data_in = Signal(intbv(0)[RDMEM_DATA_WIDTH:])
    ...
    toVHDL(AnalyzerMemory, clk=clk, reset=reset, dsp_addr=dsp_addr,
           dsp_data_out=dsp_data_out, dsp_data_in=dsp_data_in,
           dsp_wr=dsp_wr, mode=mode, pb_data_out=pb_data_out,
           pb_wfm_sel=pb_wfm_sel, rec_data_in=rec_data_in,
