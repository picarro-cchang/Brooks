...
def Ltc2604Dac(clk,reset,dac_clock_in,chanA_data_in,chanB_data_in,
               chanC_data_in,chanD_data_in,strobe_in,dac_sck_out,
               dac_sdi_out,dac_ld_out):
    """
    Parameters:
    clk             -- 50MHz clock
    reset           -- reset
    dac_clock_in    -- 10MHz clock
    ...
    def  comb2():
        ...
        if CHANA_CONTROL_START<=counter and counter<CHANA_CONTROL_END:
            dac_sdi.next = chanA_control[CHANA_CONTROL_END-1-int(counter)]
        elif CHANA_DATA_START<=counter and counter<CHANA_DATA_END:
            dac_sdi.next = chanA_data[CHANA_DATA_END-1-int(counter)]
        elif CHANB_CONTROL_START<=counter and counter<CHANB_CONTROL_END:
            dac_sdi.next = chanB_control[CHANB_CONTROL_END-1-int(counter)]
        elif CHANB_DATA_START<=counter and counter<CHANB_DATA_END:
            dac_sdi.next = chanB_data[CHANB_DATA_END-1-int(counter)]
        elif CHANC_CONTROL_START<=counter and counter<CHANC_CONTROL_END:
            dac_sdi.next = chanC_control[CHANC_CONTROL_END-1-int(counter)]
        elif CHANC_DATA_START<=counter and counter<CHANC_DATA_END:
            dac_sdi.next = chanC_data[CHANC_DATA_END-1-int(counter)]
