...
if __name__ == "__main__":
    ...
    try:
        ...
        regs = [" %s_%s" % (blockName.upper(), reg.upper())
                for reg in config["REGISTERS"]]
        lines = splitIntoLines(",".join(regs), splitLen, ",")

        importList.append("%s EMIF_ADDR_WIDTH, EMIF_DATA_WIDTH" % impString)
        importList.append("%s FPGA_REG_WIDTH, FPGA_REG_MASK, FPGA_%s" % (
            impString, blockName.upper()))
        importList.append("")
        for line in lines:
        ...
        for section in config:
            if section not in ["IMPORTS", "PORTS", "REGISTERS"]:
                for field in config[section]:
                    name = "%s_%s_%s" % (
                        blockName.upper(), section.upper(), field.upper())
                    importList.append("%s %s_B, %s_W" %
                                      (impString, name, name))
        importList.append("")
        print>>op, "\n".join(importList)
        print>>tp, "\n".join(importList)
        ...
        for port in config["PORTS"]:
            ...
            if value == "1":
                ...
                portSignalList.append(
                    "%s = Signal(intbv(0)[%s:])" % (port, value))
            else:
                portSignalList.append("%s = FPGA_%s" %
                                      (port, blockName.upper()))
        ...
        if len(lines) >= 1:
            for i, line in enumerate(lines):
                print>>op, "%s%s%s" % (decl if i == 0 else leadSpace, lines[
                                       i], "):" if i == len(lines) - 1 else "")
        else:
            print>>op, "%s%s" % (decl, "):")
        ...
        for reg in config["REGISTERS"]:
            ...
            print>>op, "                    elif dsp_addr[EMIF_ADDR_WIDTH-1:] == %s_%s_addr: # %s" % (
                blockName.lower(), reg.lower(), access)
            if "w" in access:
                print>>op, "                        if dsp_wr: %s.next = dsp_data_out" % (
                    reg,)
            if "r" in access:
                print>>op, "                        dsp_data_in.next = %s" % (
                    reg,)
        print>>op, "                    else:"
        ...
        lines = splitIntoLines(
            ",".join([" %s=%s" % (p, p) for p in ports]), splitLen, ",")
        if len(lines) >= 1:
            for i, line in enumerate(lines):
                print>>op, "%s%s%s" % (leadStr if i == 0 else leadSpace, lines[
                                       i], ")" if i == len(lines) - 1 else "")
        else:
            print>>op, "%s%s" % (decl, ")")
        ...
        if len(lines) >= 1:
            for i, line in enumerate(lines):
                print>>tp, "%s%s%s" % (leadStr if i == 0 else leadSpace, lines[
                                       i], " )" if i == len(lines) - 1 else "")
        else:
            print>>tp, "%s%s" % (decl, " )")
        ...
        for reg in config["REGISTERS"]:
            if reg in config:
                regSize, access = config["REGISTERS"][reg]
                print>>xp, '    <reg type="mask" ident="%s" description="" access="%s">' % (
                    reg.upper(), access)
                lsb = 0
                for field in config[reg]:
                    width = int(config[reg][field])
                    print>>xp, '        <bitfield ident="%s" description="" lsb="%d" width="%d">' % (
                        field.upper(), lsb, width)
                    for j in range(1 << width):
                        print>>xp, '            <fielddef value="%d"></fielddef>' % j
                    print>>xp, '        </bitfield>'
                    lsb += width
                print>>xp, '    </reg>'
            else:
                regSize, access = config["REGISTERS"][reg]
                print>>xp, '    <reg type="uint16" ident="%s" description="" access="%s">' % (
                    reg.upper(), access)
                if 'w' in access:
                    print>>xp, '        <save/>'
                print>>xp, '        <display format="" units=""></display>'
                print>>xp, '    </reg>'
        print>>xp, '</fpga_block>'
    finally:
        op.close()
