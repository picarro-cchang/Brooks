...
def bench():
    ...
    def stimulus():
        ...
        yield assertReset()
        mode.next = HIGH
        print "Testing analyzer memory in recorder mode (mode = 1)"
        nWrites = 500
        # Check write access to meta (playback) memory
        #  from the DSP
        ...
        print "Finished writing to playback memory via DSP"
        # Read these data back via the DSP interface
        for a in memDict:
            yield rdRingdownMem(a, result)
            assert result == memDict[a]
        print "Finished reading from playback memory via DSP"
        # Read the data back via the FPGA interface
        for a in memDict:
            # bank.next = 1 if (a & 0x4000) else 0
            assert 0x1000 == (a & 0x3000)  # This is playback memory
            rec_addr.next = a & 0xFFF
            pb_wfm_sel.next = 1 if (a & 0x4000) else 0
            yield clk.negedge
            yield clk.negedge
            if pb_data_out != memDict[a]:
                print "Playback waveform %d, address %x (Dsp Address %x) should contain %x, but we read %x" % (
                    pb_wfm_sel, rec_addr, a, memDict[a], pb_data_out)
                assert False
        print "Finished reading from playback memory via FPGA"
        # Perform a number of writes via the FPGA port
        memDict = {}
        for iter in range(nWrites):
            b = randrange(2)
            dspBank = 0x4000 if b else 0x0
            ...
            rec_we.next = LOW
            yield clk.negedge
        print "Finished writing to record memory via FPGA"
        # Read these data back via the DSP interface
        for a in memDict:
            yield rdRingdownMem(a, result)
            assert result == memDict[a]
        print "Finished reading back record data via DSP"
        memDict = {}
        mode.next = LOW
        yield clk.negedge
        print "Testing analyzer memory in ringdown mode (mode = 0)"
        nWrites = 500
        ...
        for a in memDict:
            # In mode 0, the playback output should always be zero
            assert pb_data_out == 0
            bank.next = 1 if (a & 0x4000) else 0
