...
def bench():
    ...
    def stimulus():
        global filling
        paramVals = [
            (FPGA_RDMAN + RDMAN_PARAM0,
             0xFFFFFFFF), (FPGA_RDMAN + RDMAN_PARAM1, 0xEEEEEEEE),
            (FPGA_RDMAN + RDMAN_PARAM2,
             0xDDDDDDDD), (FPGA_RDMAN + RDMAN_PARAM3, 0xCCCCCCCC),
            (FPGA_RDMAN + RDMAN_PARAM4,
             0xBBBBBBBB), (FPGA_RDMAN + RDMAN_PARAM5, 0xAAAAAAAA),
            (FPGA_RDMAN + RDMAN_PARAM6,
             0x99999999), (FPGA_RDMAN + RDMAN_PARAM7, 0x88888888),
            (FPGA_RDMAN + RDMAN_PARAM8, 0x77777777),
            (FPGA_RDMAN + RDMAN_PARAM9, 0x66666666)]
        yield assertReset()
        ...
        # We should now be acquiring metadata each time the metadata strobe
        # occurs
        # for i in range(2):
        ...
        while True:
            ...
            if not result[RDMAN_STATUS_BUSY_B]:
                break
            yield delay(50 * PERIOD)
        # Examine the parameter table
        print "Parameters:"
        for addr in xrange(15):
            yield rdRingdownMem((0x3000 if bank_out else 0x7000) + addr, result)
            print "0x%x: 0x%x" % (addr, result)
        print "Ringdown buffer:"
        for addr in xrange(4096):
            yield rdRingdownMem((0x0000 if bank_out else 0x4000) + addr, result)
            print "0x%x: %d" % (addr, result)
