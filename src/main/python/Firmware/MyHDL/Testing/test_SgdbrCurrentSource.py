...
class TestSPI(TestCase):
    ...
    def test_spi(self):
        ...
        self.miso_data = 0x5A5A
        self.mosi_data = 0xDEAD
        ...
        def bench():
            ...
            def stimulus():
                ...
                # yield writeFPGA(FPGA_SGDBRCURRENTSOURCE + SGDBRCURRENTSOURCE_SCK_DIVISOR, 1)
                # yield writeFPGA(FPGA_SGDBRCURRENTSOURCE +
                # SGDBRCURRENTSOURCE_NUM_OF_CLOCK_PULSES,
                # self.num_of_clock_pulses)
        ...
        for trials in range(100):
            self.CPOL = randrange(2)
            self.CPHA = randrange(2)
            self.num_of_clock_pulses = 32
            self.miso_data = randrange(0, 1 << self.num_of_clock_pulses)
            self.mosi_data = randrange(0, 1 << self.num_of_clock_pulses)
            s = Simulation(bench())
            s.run(quiet=1)
