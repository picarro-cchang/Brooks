...
#
"""
File Name: AlarmSystemActual.py
Purpose:
...
class AlarmSystem(object):
    ...
    def __init__(self, configPath, legacyMode):
        ...
        self.alarmStatus = 0
        self.instr_configPath = None
        self.legacyMode = legacyMode

        self.alarmScriptCodeObj = None
        self.alarmParamsDict = {}
    ...
    def _LoadConfigFile(self):
        ...
        """
        if self.legacyMode:
            self.user_configPath = self.configPath
            self.user_cp = CustomConfigObj(self.configPath)
        else:
        ...
        self.NumAlarms = self.user_cp.getint("MainConfig", "numalarms")
        count = 0
        sectionList = self.user_cp.list_sections()
    ...
    def _DataListener(self, dataMgrObject):
        measData = MeasData()
        measData.ImportPickleDict(dataMgrObject)
        self._MeasDataListener(measData)

    def _MeasDataListener(self, source, reportDict, measGood):
        # check to make sure this a broadcast that I'm interested in
        if source in self.SrcDict:
            # iterate through all alarm object found in the list
            for alarmName in self.SrcDict[source]:
    ...
    def ALARMSYSTEM_start_inthread(self):
        """Called to start the alarm system."""
