    ...
    def _xticks(self, *args):
        if self._logscale[0]:
            ...
            if isinstance(xTimeAxis, tuple):
                ...
                utcTime = xTimeAxis[1]
            return self._timeticks(*args, **dict(utcTime=utcTime, offsetTime=offsetTime, format=self.xTickFormat))
        else:
    ...
    def _yticks(self, *args):
        if self._logscale[1]:
            ...
            if isinstance(yTimeAxis, tuple):
                ...
                utcTime = yTimeAxis[1]
            return self._timeticks(*args, **dict(utcTime=utcTime, offsetTime=offsetTime, format=self.yTickFormat))
        else:
