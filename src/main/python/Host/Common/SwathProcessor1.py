...
def angleRange(x, y, N, dist, u, v, dmax=100.0):
    ...
    while i <= 2 * N:
        if good[i]:
            while i <= 2 * N and good[i]:
                i += 1
            groups.append((da[start:i].min(), da[start:i].max()))
        while i <= 2 * N and not good[i]:
            i += 1
        start = i
    return groups
...
class PasquillGiffordApprox(object):
    def __init__(self):
        ...
        self.classConstants = {
            'A': PGTuple(1.92607854134, 7.06164265123),
...
def processGen(source, maxWindow, stabClass, minLeak, minAmpl, astdParams):
    ...
    for newdat in source:
        ...
        if len(fovBuff) == 2 * N + 1:
            ...
            if (fit > 0) and (mask < 1.0e-3) and isfinite(dstd) and isfinite(windN) and isfinite(windE) and (
                    instStatus == sis.INSTMGR_STATUS_GOOD) and (stabClass is not None):
