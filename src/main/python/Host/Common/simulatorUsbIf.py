...
class DataType(Union):
    _fields_ = [("asUint", c_uint), ("asInt", c_int), ("asFloat", c_float)]
...
class SimulatorUsb(Singleton):
    ...
    def connect(self):
        ...
        logging.debug("SimulatorUsb: connect called, VID = %d, PID = %d" % (
            self.vid,
            self.pid,
        ))
    ...
    def controlInTransaction(self, var, cmd, value=0, index=0):
        sv = sizeof(var)
        logging.debug("SimulatorUsb: controlInTransaction called, cmd = 0x%x, value = 0x%x, index = 0x%x, sizeof(var) = %d" %
                      (cmd, value, index, sv))

    def controlOutTransaction(self, msg, cmd, value=0, index=0):
        sm = sizeof(msg)
        logging.debug("SimulatorUsb: controlOutTransaction called, cmd = 0x%x, value = 0x%x, index = 0x%x, sizeof(msg) = %d" %
                      (cmd, value, index, sm))
    ...
    def loadHexFile(self, fp):
        ...
        def _loadHexFile():
            ...
            for r in regions:
                ...
                while n > block:
                    self.controlOutTransaction(create_string_buffer("".join(r.data[start:start + block]), block), 0xA0,
                                               addr + start)
                    n -= block
                    start += block
                if n > 0:
