...
from Host.ControllerBuildStation.ControllerBuildStationModels import DriverProxy, RDFreqConvProxy, SpectrumCollectorProxy, ControllerRpcHandler, waveforms, dasInfo
from Host.ControllerBuildStation.ControllerPanelsGui import CommandLogPanelGui, LaserPanelGui, PressurePanelGui
from Host.ControllerBuildStation.ControllerPanelsGui import WarmBoxPanelGui, HotBoxPanelGui, RingdownPanelGui
from Host.ControllerBuildStation.ControllerPanelsGui import WlmPanelGui, StatsPanelGui
    ...
    def onSelectGraphType(self, evt):
        ...
        if choice == 0:
            ...
            def lossVsWavenumber(data):
                if dataGood(data):
                    ...
                    wavenumber = data.laserTemperature
            ...
            self.ringdownGraph.SetGraphProperties(xlabel='Laser Temperature (degC)',
                                                  timeAxes=(False, False),
                                                  ylabel='Loss (ppm/cm)',
                                                  grid=True,
                                                  frameColour=wx.SystemSettings_GetColour(wx.SYS_COLOUR_3DFACE),
        ...
        if y == "Loss":
            ...
            for vLaser in range(interface.NUM_VIRTUAL_LASERS):
                self.ringdownGraph.AddSeriesAsPoints(waveforms["Ringdown"]["fineCurrent_%d" % (vLaser + 1, )],
                                                     colour=fillColours[vLaser],
                                                     fillcolour=fillColours[vLaser],
                                                     marker='square',
                                                     size=1,
                                                     width=1)
        elif y == "Tuner":
            for vLaser in range(interface.NUM_VIRTUAL_LASERS):
                self.ringdownGraph.AddSeriesAsPoints(waveforms["Ringdown"]["tuner_%d" % (vLaser + 1, )],
                                                     colour=fillColours[vLaser],
                                                     fillcolour=fillColours[vLaser],
                                                     marker='square',
                                                     size=1,
            ...
            for vLaser in range(interface.NUM_VIRTUAL_LASERS):
                self.ringdownGraph.AddSeriesAsPoints(waveforms["Ringdown"]["wavenumber_%d" % (vLaser + 1, )],
                                                     colour=fillColours[vLaser],
                                                     fillcolour=fillColours[vLaser],
                                                     marker='square',
                                                     size=1,
...
class PressurePanel(PressurePanelGui):
    def __init__(self, *a, **k):
        ...
        self.pressureGraph.SetGraphProperties(ylabel='Pressure (torr)',
                                              timeAxes=(True, False),
                                              frameColour=wx.SystemSettings_GetColour(wx.SYS_COLOUR_3DFACE),
                                              grid=True,
                                              backgroundColour=wx.SystemSettings_GetColour(wx.SYS_COLOUR_3DFACE))
...
class WarmBoxPanel(WarmBoxPanelGui):
    def __init__(self, *a, **k):
        ...
        self.temperatureGraph.SetGraphProperties(ylabel='Temperature (degC)',
                                                 ...
                                                 grid=True,
                                                 backgroundColour=wx.SystemSettings_GetColour(wx.SYS_COLOUR_3DFACE))
        self.tecGraph.SetGraphProperties(ylabel='TEC Current',
                                         grid=True,
                                         timeAxes=(True, False),
                                         frameColour=wx.SystemSettings_GetColour(wx.SYS_COLOUR_3DFACE),
                                         backgroundColour=wx.SystemSettings_GetColour(wx.SYS_COLOUR_3DFACE))
...
class HotBoxPanel(HotBoxPanelGui):
    def __init__(self, *a, **k):
        ...
        self.tecGraph.SetGraphProperties(ylabel='TEC Current',
                                         ...
                                         frameColour=wx.SystemSettings_GetColour(wx.SYS_COLOUR_3DFACE),
                                         backgroundColour=wx.SystemSettings_GetColour(wx.SYS_COLOUR_3DFACE))
        self.heaterGraph.SetGraphProperties(ylabel='Heater Current',
                                            grid=True,
                                            timeAxes=(True, False),
                                            frameColour=wx.SystemSettings_GetColour(wx.SYS_COLOUR_3DFACE),
                                            backgroundColour=wx.SystemSettings_GetColour(wx.SYS_COLOUR_3DFACE))
