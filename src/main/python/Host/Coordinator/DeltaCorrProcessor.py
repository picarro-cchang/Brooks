    ...
    def procData(self):
        ...
        if len(standardList) >= 2:
            for stdId in range(len(standardList) - 1):
                ...
                standardDeltaOffset = (float(standardList[stdId][8]) + float(standardList[stdId + 1][8])) / 2 - float(
                    standardList[stdId][12])
                standard12CO2Offset = (float(standardList[stdId][9]) + float(standardList[stdId + 1][9])) / 2 - float(
                    standardList[stdId][13])
