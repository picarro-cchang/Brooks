...
def RPC(request):
    RPC = CmdFIFO.CmdFIFOServerProxy("http://localhost:%d" % RPC_PORT_TEST_CMDFIFO,
                                     "Test_CmdFIFO",
...
class TestKill(object):
    def test_killServer(self):
        ...
        RPC = CmdFIFO.CmdFIFOServerProxy("http://localhost:%d" % RPC_PORT_TEST_CMDFIFO, "Test_CmdFIFO", IsDontCareConnection=False)
