...
class DasConfigure(object):
    ...
    def run(self):
        ...
        outputs.addOperation(
            Operation("ACTION_PULSE_GENERATOR", ["LOW_DURATION_REGISTER", "HIGH_DURATION_REGISTER", "LASER2_TEMPERATURE_REGISTER"],
                      "PULSE_GEN_ENV"))

        processors = OperationGroup(priority=7, period=100)
        processors.addOperation(Operation("ACTION_FILTER", ["LASER2_TEMPERATURE_REGISTER", "LASER2_TEC_PWM_REGISTER"],
                                          "FILTER_ENV"))

        streamer = OperationGroup(priority=11, period=100)
        streamer.addOperation(Operation("ACTION_STREAM_REGISTER_ASFLOAT", ["STREAM_Laser2Temp", "LASER2_TEMPERATURE_REGISTER"]))
        streamer.addOperation(Operation("ACTION_STREAM_REGISTER_ASFLOAT", ["STREAM_Laser2Tec", "LASER2_TEC_PWM_REGISTER"]))
