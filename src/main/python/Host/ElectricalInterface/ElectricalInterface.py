...
class EifAnalogOutput(EifSignal):
    def __init__(self, channel, eif, config, allowedMin=EIF_ANALOG_ALLOWED_MIN, allowedMax=EIF_ANALOG_ALLOWED_MAX):
        ...
        self.configlist = [
            'mode', 'source', 'slope', 'offset', 'min', 'max', 'bootmode', 'bootvalue', 'invalidvalue', 'currentvalue'
        ]
...
class EifMgr(object):
    def __init__(self, filename):
        ...
        self.measListener = Listener.Listener(None,
                                              ...
                                              self.measFilter,
                                              retry=True,
                                              name="Electrical inteface measurement listener",
                                              logFunc=Log)
    ...
    def loadConfig(self, filename):
        ...
        for channel in self.analog_outputs:
            ...
            self.analogOutput[channel] = EifAnalogOutput(channel, self, self._config, self.analog_allowedMin,
                                                         self.analog_allowedMax)
    ...
    def run(self):
        self.RpcServer = CmdFIFO.CmdFIFOServer(("", RPC_PORT_EIF_HANDLER),
                                               ServerName="ElectricalInterface",
                                               ServerDescription="EifMgr",
    ...
    def RPC_AO_Configure(self, channel, calSlope, calOffset, minOutput, maxOutput, bootmode, bootvalue, invalidvalue):
        ...
        if channel in self.analogOutput:
            return self.analogOutput[channel].configure(self._filename, calSlope, calOffset, minOutput, maxOutput, bootmode,
                                                        bootvalue, invalidvalue)
        else:
            return False
