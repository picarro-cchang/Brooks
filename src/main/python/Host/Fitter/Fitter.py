    ...
    def update(self, dataAnalysisResults):
        ...
        for analysis in analysisList:
            ...
            if analysis["name"] not in self.fitViewDict["analyses"]:
                ad = self.fitViewDict["analyses"][analysis["name"]] = dict(byFunction={},
                                                                           byProperty={},
                                                                           byIndex={},
                                                                           time=self.seqMgr.getSequence())
                m = analysis.model
...
class HistoryView(wx.SplitterWindow):
    ...
    def __init__(self,
        ...
        self.tree = HistoryViewTree(parent=self,
        ...
        self.graph.SetGraphProperties(ylabel='Sample Data',
                                      grid=True,
...
class HistoryViewFrame(wx.Frame):
    ...
    def __init__(self,
                 ...
                 parent=None,
                 id=-1,
                 title='Fit output history inspector',
                 pos=wx.DefaultPosition,
                 size=(600, 400),
...
class MyGrid(wx.grid.Grid):
    ...
    def OnRegress(self):
        ...
        sc = '+'
        if p[1] < 0: sc = '-'
        wx.MessageBox("Line of best fit: y = %.4g x %s %.4g\nBest line through origin: y = %.4g x" % (p[0], sc, abs(p[1]), m),
                      "Information")
...
class VariableViewTable(MyGrid):
    ...
    def __init__(self, parent, nColumns, id=-1, pos=wx.DefaultPosition, size=wx.DefaultSize):
...
class VariableView(wx.SplitterWindow):
    ...
    def __init__(self,
        ...
        self.parent = parent
        self.config = config
        self.tree = VariableViewTree(parent=self,
                                     rootObject=rootObject,
                                     rootLabel=rootLabel,
                                     rootIsNamespace=rootIsNamespace,
                                     static=static)
...
class VariableViewFrame(wx.Frame):
    ...
    def __init__(self,
                 ...
                 parent=None,
                 id=-1,
                 title='Fit output variable inspector',
                 pos=wx.DefaultPosition,
                 size=(600, 400),
                 style=wx.DEFAULT_FRAME_STYLE,
                 rootObject=None,
...
class FitViewer(wx.Frame):
    def __init__(self, configFile, useViewer, options):
        ...
        self.inputSelect = wx.RadioBox(panel,
                                       -1,
                                       "Select Input",
                                       choices=["Data Input", "HDF5 files", "Pickled files"],
                                       majorDimension=3,
        ...
        self.batchMode = wx.RadioBox(panel, -1, choices=["Single", "Continuous"], majorDimension=2, style=wx.RA_SPECIFY_COLS)
    ...
    def calculateBasicSeries(self):
        ...
        if analysis is not None:
            ...
            self.residualsPanel.modelLoss.Clear()
            self.residualsPanel.res.Clear()
            for x in analysis.xData:
                self.residualsPanel.dataFreq.Add(x)
    ...
    def calculateDetailedSeries(self):
        ...
        if analysis is not None:
            ...
            for x in xfine:
                self.residualsPanel.fineFreq.Add(x)
            for y in yfine:
    ...
    def doModelAnalysis(self):
        ...
        if analysis is not None:
            ...
            for i in range(analysis.nSteps()):
                self.analysisStage.Append("%d" % i)
            ...
            for i in sorted(model.basisFunctionByIndex.keys()):
                ...
                self.functionLabels.append("%d (%s)" % (
                    i,
                    func.name,
                ))
    ...
    def layoutFrame(self, parent):
        ...
        self.graphPanel1.SetGraphProperties(ylabel='Data and Fit', grid=True, backgroundColour=bg, frameColour=bg)
        self.graphPanel1.Update()
        self.graphPanel2 = GraphPanel(parent=panel, id=-1)
        self.graphPanel2.SetGraphProperties(ylabel='Residual', grid=True, backgroundColour=bg, frameColour=bg)
    ...
    def OnInputSelect(self, evt):
        ...
        self.fitViewManager.clear()
        for f in self.historyFrames:
            f.Refresh()
        for f in self.variableFrames:
            f.Refresh()
    ...
    def OnNewGraph(self, evt):
        historyFrame = HistoryViewFrame(config=self.config,
                                        root=self,
    ...
    def OnNewTable(self, evt):
        variableFrame = VariableViewFrame(config=self.config,
                                          root=self,
                                          rootObject=self.fitViewManager.getFitViewDict(),
                                          rootIsNamespace=False,
                                          rootLabel="fitter")
...
class ResidualsPanel(object):
    def __init__(self, config, parent, root):
        ...
        self.graphPanel2.SetGraphProperties(ylabel='Residuals', grid=True, backgroundColour=bg, frameColour=bg)
        #self.graphPanel2.Update()
        vbox = wx.BoxSizer(wx.VERTICAL)
        vbox.Add(self.graphPanel1, proportion=1, flag=wx.GROW)
        vbox.Add(self.graphPanel2, proportion=1, flag=wx.GROW)
    ...
    def setupFineDisplay(self):
        if self.mode != "fine":
            ...
            self.graphPanel1.AddSeriesAsPoints(self.dataSeries,
                                               fillcolour=self.root.getColorFromIni("FitViewer", "DataColor"),
                                               colour="black",
                                               marker="square",
                                               size=1,
                                               width=1)
            ...
            #                                  width=self.root.config.getfloat("FitViewer","LineWidth"))
            self.graphPanel1.AddSeriesAsLine(self.fineModelSeries, colour="black", width=1)
            self.graphPanel1.AddSeriesAsLine(self.finePartialModelSeries, colour="red", width=1)
            self.graphPanel2.RemoveAllSeries()
            self.graphPanel2.AddSeriesAsLine(self.resSeries,
                                             colour=self.root.getColorFromIni("FitViewer", "ResidualColor"),
                                             width=self.root.config.getfloat("FitViewer", "LineWidth"))
            self.graphPanel2.AddSeriesAsPoints(self.resSeries,
                                               fillcolour=self.root.getColorFromIni("FitViewer", "ResidualColor"),
                                               colour="black",
                                               marker="square",
                                               size=1,
                                               width=1)
            self.graphPanel2.AddSeriesAsLine(self.partialResSeries,
                                             colour=self.root.getColorFromIni("FitViewer", "PartialResidualColor"),
                                             width=self.root.config.getfloat("FitViewer", "LineWidth"))
            self.graphPanel2.AddSeriesAsPoints(self.partialResSeries,
                                               fillcolour=self.root.getColorFromIni("FitViewer", "PartialResidualColor"),
                                               colour="black",
                                               marker="square",
                                               size=1,
                                               width=1)
    ...
    def setupCoarseDisplay(self):
        if self.mode != "coarse":
            ...
            #                                  width=self.root.config.getfloat("FitViewer","LineWidth"))
            self.graphPanel1.AddSeriesAsLine(self.modelSeries, colour="blue", width=1)
            self.graphPanel2.RemoveAllSeries()
            self.graphPanel2.AddSeriesAsLine(self.resSeries,
                                             colour=self.root.getColorFromIni("FitViewer", "ResidualColor"),
                                             width=self.root.config.getfloat("FitViewer", "LineWidth"))
