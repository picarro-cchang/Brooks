...
Ts = array([0.314240376, 0.947788391, 1.597682640, 2.279507080, 3.020637030, 3.889724900])

Cs = array([1.011728050E+0, -7.519714700E-1, 1.255772700E-2, 1.002200820E-2, -2.420681350E-4, 5.008480610E-7])

Ss = array([1.393237000E+0, 2.311524060E-1, -1.553514660E-1, 6.218366240E-3, 9.190829860E-5, -6.275259580E-7])
...
def galatry0(x, y, z, strength=1.0, minimum_loss=0.001):
    ...
    if region1.any():
        ...
        c5r = z**3 / 240.0
        c = array([
            1.0, 0.0, 0.0, 1j * c3r, c4r, -1j * c5r, -1 * (-z**4 / 1440.0 + c3r**2 / 2.0), 1j *
            (z**5 / 10080.0 + c3r * c4r), -z**6 / 80640.0 + c3r * c5r + c4r**2 / 2.0
        ], 'D')
        xx = x[region1]
        vv = voigt0(xx, y)
        qq = xx + 1j * y
    ...
    def __init__(self, x_array, y_array, low_slope=None, high_slope=None, y2_array=None, tol=1.0e-7):
        ...
        if y2_array is not None:
            ...
            for j in range(1, len(x_array) - 1):
                ...
                misfit = max(
                    misfit,
                    abs((dxL * y2_array[j - 1] + 2 * (dxL + dxR) * y2_array[j] + dxR * y2_array[j + 1]) / 6.0 - (slopeR - slopeL)))
...
class Sinusoid(BasisFunctions):
    ...
    def __init__(self, params=None, **kwargs):
        ...
        if params is not None:
            ...
            self.initialParams = array([kwargs["amplitude"], kwargs["center"], kwargs["period"], kwargs["phase"]], float_)
...
class Spline(BasisFunctions):
    ...
    def __init__(self, splineLibrary, params=None, **kwargs):
        ...
        if params is not None:
            ...
            self.initialParams = array(
                [kwargs["freqShift"], kwargs["baselineShift"], kwargs["amplitude"], kwargs["squishParam"], kwargs["squishCenter"]],
                float_)
...
class BiSpline(BasisFunctions):
    ...
    def __init__(self, splineLibrary, params=None, **kwargs):
        ...
        if params is not None:
            ...
            self.initialParams = array([
                kwargs["freqShift"], kwargs["baselineShift"], kwargs["amplitude"], kwargs["squishParam"], kwargs["squishCenter"],
                kwargs["yEffective"], kwargs["yMultiplier"]
            ], float_)
    ...
    def call(self, a, x):
        return fitutils.bseval(self.splineA.x_vals, self.splineA.y_vals, self.splineA.y2_vals, self.splineB.x_vals,
                               self.splineB.y_vals, self.splineB.y2_vals, a, x)
...
class Galatry(BasisFunctions):
    ...
    def __init__(self, physicalConstants, spectralLibrary, params=None, **kwargs):
        ...
        if params is not None:
            ...
            self.initialParams = array(
                [kwargs["center"], kwargs["scaled_strength"], kwargs["scaled_y"], kwargs["scaled_z"], kwargs["v"]], float_)
...
class RdfData(object):
    ...
    def getTime(self):
        ...
        return unixTime(self.avgTimestamp)
...
##  14 June 2010  added modified sigma filter named "outlierFilter"
    ...
    def calcGroupStats(self):
        self.evaluateGroups([
            "waveNumber", "uncorrectedAbsorbance", "waveNumberSetpoint", "pztValue", "ratio1", "ratio2", "wlmAngle",
            "laserTemperature"
        ])
        self.groupStats = {}
            ...
            if i < 1000:
                ...
                def FP(basisFunc, extra={}):
                    ...
                    for j in range(nParams):
                        a.append(self.config.getfloat(section, "a%d" % j))
                ...
                if form == "sinusoid":
                    ...
                    ndx = form[8:].split("_")
                    m.addToModel(FP(BiSpline, dict(splineLibrary=splineLibrary, splineIndexA=int(ndx[0]),
    ...
    def doFit(self, seqIndex, fine=True):
        ...
        try:
            if fine:
                ...
                qparams, cov, infodict, msg, self.ier = leastsq(fitfunc,
                                                                normalize(p0),
                                                                full_output=1,
                                                                xtol=1e-4,
                                                                epsfcn=1e-6,
