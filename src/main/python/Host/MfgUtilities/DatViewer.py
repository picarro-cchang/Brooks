...
class DatViewer(HasTraits):
    ...
    traits_view = View(
        Group(Item("plot", style="custom", show_label=False), Item("dataSetName", editor=EnumEditor(name="dataSetNameList")),
              Item("varName", editor=EnumEditor(name="varNameList")), Item("transform")))
    ...
    def _transform_changed(self):
        if self.varName: self._varName_changed()
...
class h52Dat(HasTraits):
    ...
    def convert(self):
        ...
        try:
            ...
            if dat is not None: dat.close()
            if d is not None: d.Destroy()
            if ip is not None: ip.close()
