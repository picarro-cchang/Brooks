            ...
            a.append(
                HGroup(
                    ...
                    VGroup(Item("dataSetName", object="h%d" % i, editor=EnumEditor(name="dataSetNameList"), width=w),
                           ...
                           Item("mean", object="h%d" % i, width=w), Item("stdDev", object="h%d" % i))))
            self.cDict["h%d" % i] = fr
        ...
        for i, fr in enumerate(self.viewers):
            a.append(
                HGroup(
                    ...
                    VGroup(Item("dataSetName", object="h%d" % i, editor=EnumEditor(name="dataSetNameList"), width=w),
                           Item("varName", object="h%d" % i, editor=EnumEditor(name="varNameList"), width=w),
                           Item("autoscaleY", object="h%d" % i, width=w), Item("mean", object="h%d" % i, width=w),
                           Item("stdDev", object="h%d" % i))))
            self.cDict["h%d" % i] = fr
        tGroup = Group(*a, **dict(layout="split"))
        self.cDict["object"] = self
        self.traits_view = View(VGroup(tGroup, Item('correlate', show_label=False, label='Show correlation')),
                                buttons=NoButtons,
...
class NotebookHandler(Handler):
    ...
    def onConcatenate(self, info):
        ...
        if fd.ShowModal() == wx.ID_OK:
            ...
            try:
                d = wx.DirDialog(None,
                                 "Select directory tree with individual .h5 files",
...
class ViewNotebook(HasTraits):
    ...
    def __init__(self, *a, **k):
        ...
        self.traits_view = View(Item("dataFile", style="readonly", show_label=False),
                                ...
                                menubar=MenuBar(
                                    Menu(openAction, concatenateAction, convertAction, batchConvertAction, exitAction, name='File'),
                                    Menu(Menu(series1Action, series2Action, series3Action, name="Time Series Plot"),
                                         xyViewAction,
                                         name='New')),
