...
#
# FILE:
#   MakeWarmBoxCalFileD.py
#
# DESCRIPTION:
#   Create a warm box calibration file with a derivative spline from
#    a collection of WLM files
#
# SEE ALSO:
#   Specify any related information.
#
# HISTORY:
#   5-Jul-2010  sze  Initial version.
#
...
from Host.Tests.MidIR.WlmCalUtilities import bestFitCentered, WlmFile, AutoCal, BspInterp, WlmConverter
    ...
    def __init__(self, configFile, options):
        ...
        for key in self.config:
            if key.startswith("VIRTUAL"):
                ...
                if "REF" in self.config[key]:
                    virtualList[vLaserNum - 1]["REF"] = float(self.config[key]["REF"])
                else:
                    virtualList[vLaserNum - 1]["REF"] = None
    ...
    def run(self):
        ...
        for i, fp in enumerate(self.wlmFiles):
            ...
            if fp is not None:
                ...
                # Find frequency range required for this converter by finding min and
                #  max frequencies from all virtual lasers associated with this actual
                #  laser
                wMin, wMax = None, None
                for i, v in enumerate(self.virtualList):
                    ...
                    if v:
                        ...
                        if aLaserNum == laserNum:
                            if "WMIN" in v:
                                wMin = min(v["WMIN"], wMin) if wMin is not None else v["WMIN"]
                            if "WMAX" in v:
                                wMax = max(v["WMAX"], wMax) if wMax is not None else v["WMAX"]
                wlmConverter = WlmConverter()
                self.wlmConverters[laserNum] = wlmConverter
                print wMin, wMax
                wlmConverter.loadFromWlmFile(self.wlmFileList[laserNum - 1], dTheta=self.dTheta, wMin=wMin, wMax=wMax)
                wlmConverter.updateIni(self.op, laserNum)
                pylab.figure()
                pylab.plot(wlmFile.TLaser, wlmFile.WaveNumber, 'r.', wlmFile.TLaser, wlmFile.TtoW(wlmFile.TLaser))
                pylab.grid(True)
        ...
        for i, v in enumerate(self.virtualList):
            ...
            if v:
                aLaserNum = int(v["ACTUAL"])
                wlmConverter = self.wlmConverters[aLaserNum]
                refFreq = v["REF"] if v["REF"] is not None else 0.5 * (wlmConverter.wMin + wlmConverter.wMax)
                vLaserSec = {}
                vLaserSec["WLM_CONV"] = aLaserNum
                vLaserSec["WLM_OFFSET"] = 0.0
                vLaserSec["REF_FREQ"] = refFreq
                thToW = wlmConverter.thToW.coeffs
                vLaserSec["REF_ANGLE"] = (refFreq - thToW[1]) / thToW[0]
                self.op["VIRTUAL_LASER_%d" % vLaserNum] = vLaserSec
