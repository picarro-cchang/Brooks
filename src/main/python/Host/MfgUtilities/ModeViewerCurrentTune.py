...
class ScopePanel(wx.Panel):
    def __init__(self, *args, **kwds):
        ...
        self.graphPanel1.SetGraphProperties(xlabel='PZT Position',
...
class ModePanel(wx.Panel):
    def __init__(self, *args, **kwds):
        ...
        bg = wx.SystemSettings_GetColour(wx.SYS_COLOUR_3DFACE)
        self.graphPanel1 = GraphPanel(parent=self, id=-1)
        self.graphPanel1.SetGraphProperties(ylabel='Mode Amplitudes (%)', grid=True, backgroundColour=bg, frameColour=bg)
        vbox = wx.BoxSizer(wx.VERTICAL)
        vbox.Add(self.graphPanel1, proportion=1, flag=wx.GROW)
...
class ModeViewer(wx.Frame):
    ...
    def __init__(self, configFile):
        ...
        self.scopePanel = ScopePanel(self.notebook, config=configFile, root=self)
        self.notebook.AddPage(self.scopePanel, "Cavity Scan")
        self.modePanel = ModePanel(self.notebook, root=self)
    ...
    def onTimer(self, evt):
        ...
        ccorr = numpy.fft.fftshift(
            xcorr(numpy.concatenate((self.ref, self.ref[0] * numpy.ones(N))), numpy.concatenate((d, d[0] * numpy.ones(N)))))
        ...
        pkAreas = numpy.asarray(
            [self.modeDict[m]['area'] for m in ['Main1', 'Saddle', 'Horizontal', 'Polarization', 'Focus', 'Vertical', 'Main2']])
        ...
        if sel == 0:
            ...
            for x, y in enumerate(self.data):
                xi = (x - self.mainPos - self.mainSep) / self.mainSep
                w.Add(x, y)
...
_DEFAULT_CONFIG_NAME = "ModeViewerCurrentTune.ini"
