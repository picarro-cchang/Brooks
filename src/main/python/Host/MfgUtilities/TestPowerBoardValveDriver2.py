...
class TestPowerBoardValveDriver(object):
    ...
    def run(self):
        ...
        try:
            ...
            setPoints = arange(0.0, 50000.0, 2000.0)
            sweepMon = []

            self.sourcemeter.sendString("*RST")
            self.sourcemeter.sendString(":SOURCE:FUNC VOLT")
            self.sourcemeter.sendString(":SOURCE:VOLT:MODE FIXED")
            self.sourcemeter.sendString(':SENS:FUNC "CURR"')
            self.sourcemeter.sendString(":SOURCE:VOLT:RANG MIN")
            self.sourcemeter.sendString(":SOURCE:VOLT:LEV 0.0")
            self.sourcemeter.sendString(":SENS:CURR:PROT 1")
            self.sourcemeter.sendString(":SENS:CURR:RANG 0.1")
            self.sourcemeter.sendString(":OUTP ON")
        ...
        slopeOpt = 2.2e-6
        vt.setEntries([
            ("%s valve Current Slope" % valve, p[0], 0.95 * slopeOpt, 1.05 * slopeOpt, "%.3g"),
