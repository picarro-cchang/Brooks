...
from numpy import *
from pylab import *
from Host.MfgUtilities.TestDescriptions import *
import time
import types
import os
...
class OphirPowermeter(object):
    ...
    def sendCommand(self, cmd):
        ...
        if r[0] != "*":
            raise Exception, "Error %s in response to %s" % (
                r,
                cmd,
            )
        return r.strip("\r\n")
...
class TestParameters(object):
    ...
    def makeHTML(self):
        self.rstFile.close()
        os.system(r'"c:\Program Files\docutils\tools\rst2html.py" ' + self.baseDirectory + reportFilename + ' ' +
                  self.baseDirectory + htmlReportFilename)
