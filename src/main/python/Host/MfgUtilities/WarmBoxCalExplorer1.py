...
class ContourfPlot(HasTraits):
    ...
    def contourData(self, zz):
        self.lock.acquire()
        self.plot2dFigure.clf()
        self.axes = self.plot2dFigure.add_subplot(111)
        pl = self.axes.imshow(zz, interpolation='bilinear', origin='lower', aspect='auto')
...
class WarmBoxCalExplorer(HasTraits):
    ...
    def _calDirectory_fired(self):
        ...
        self.corrSurf.contourData(currentValues - originalValues)
        # self.corrSurf.contourData(diff(currentValues-originalValues,axis=0))


if __name__ == "__main__":
    warmBoxCalExplorer = WarmBoxCalExplorer()

    myView = View(Item("current", style="custom", show_label=False),
                  ...
                  resizable=True,
                  buttons=NoButtons,
                  title="Warm Box Calibration Viewer")
