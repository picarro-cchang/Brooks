...
RT_MANIFEST = 24
Controller = Target(
    description="Controller",  # used for the versioninfo resource
    script="Controller/Controller.py",  # what to build
    other_resources=[(RT_MANIFEST, 1, manifest_template % dict(prog="Controller"))],
...
QuickGui = Target(
    description="QuickGui",  # used for the versioninfo resource
    script="QuickGui/QuickGui.py",  # what to build
    other_resources=[(RT_MANIFEST, 1, manifest_template % dict(prog="QuickGui"))],
    ##    icon_resources = [(1, "icon.ico")],
    dest_base="QuickGui")

Fitter = Target(
    description="Fitter",  # used for the versioninfo resource
    script="pydCaller/Fitter.py",  # what to build
    other_resources=[(RT_MANIFEST, 1, manifest_template % dict(prog="Fitter"))],
    ##    icon_resources = [(1, "icon.ico")],
    dest_base="Fitter")

Coordinator = Target(
    description="Coordinator",  # used for the versioninfo resource
    script="Coordinator/Coordinator.py",  # what to build
    other_resources=[(RT_MANIFEST, 1, manifest_template % dict(prog="Coordinator"))],
    ##    icon_resources = [(1, "icon.ico")],
    dest_base="Coordinator")

deltaCorrProcessor = Target(
    description="DeltaCorrProcessor",  # used for the versioninfo resource
    script="Coordinator/DeltaCorrProcessor.py",  # what to build
    other_resources=[(RT_MANIFEST, 1, manifest_template % dict(prog="deltaCorrProcessor"))],
    ##    icon_resources = [(1, "icon.ico")],
    dest_base="DeltaCorrProcessor")

dilutionCorrProcessor = Target(
    description="DilutionCorrProcessor",  # used for the versioninfo resource
    script="Coordinator/DilutionCorrProcessor.py",  # what to build
    other_resources=[(RT_MANIFEST, 1, manifest_template % dict(prog="dilutionCorrProcessor"))],
    ##    icon_resources = [(1, "icon.ico")],
    dest_base="DilutionCorrProcessor")

supervisorLauncher = Target(
    description="SupervisorLauncher",  # used for the versioninfo resource
    script="Utilities/SupervisorLauncher/SupervisorLauncher.py",  # what to build
    other_resources=[(RT_MANIFEST, 1, manifest_template % dict(prog="supervisorLauncher"))],
    ##    icon_resources = [(1, "icon.ico")],
...
inclusionList = [
    ...
    'Host.autogen.*',
    'Host.Common.*',
    'Host.CommandInterface.*',
    'Host.DataManager.*',
    'Host.Driver.*',
    'Host.Fitter.*',
    'Host.PeriphIntrf.*',
    'Host.SampleManager.*',
    'Host.SpectrumCollector.*',
    'Host.ValveSequencer.*'
]
...
hex_images = hex_images + [
    "../Firmware/CypressUSB/analyzer/analyzerUsb.hex", "../Firmware/DSP/src/Debug/dspMain.hex",
...
data_files = [
    (".", [
        "pydCaller/Warning_16x16_32.ico", "pydCaller/Info_16x16_32.ico", "pydCaller/Critical_16x16_32.ico",
        "pydCaller/LEDgreen.ico", "pydCaller/LEDgreen2.ico", "pydCaller/LEDoff.ico", "pydCaller/LEDoff2.ico",
        "pydCaller/LEDred2.ico", "pydCaller/logo.png", "Fitter/fitutils.pyd", "Fitter/cluster_analyzer.pyd",
        "Utilities/SupervisorLauncher/Check.png", "Utilities/SupervisorLauncher/alarm.png",
        "Utilities/SupervisorLauncher/Cancel.ico", "Utilities/SupervisorLauncher/Controller_icon.ico",
        "Utilities/SupervisorLauncher/Diagnostics_icon.ico", "Utilities/SupervisorLauncher/EnviroSense.ico",
        "Utilities/SupervisorLauncher/Integration_icon.ico", "Utilities/SupervisorLauncher/Picarro_icon.ico",
        "Utilities/SupervisorLauncher/Utilities_icon.ico", "Utilities/KillHostSoftware.py", "PeriphIntrf/Serial2Socket.exe"
    ]),
...
# pydCaller
consoleList = [
    "pydCaller/AlarmSystem.py", "pydCaller/Archiver.py", "pydCaller/CommandInterface.py", "pydCaller/DataLogger.py",
    "pydCaller/DataManager.py", "pydCaller/DataManagerPublisher.py", "pydCaller/Driver.py", "pydCaller/ElectricalInterface.py",
    "pydCaller/EventManager.py", Fitter, "pydCaller/InstMgr.py", "pydCaller/MeasSystem.py", "pydCaller/PortListerner.py",
    "pydCaller/RDFrequencyConverter.py", "pydCaller/rdReprocessor.py", "pydCaller/RestartSupervisor.py",
    "pydCaller/RunSerial2Socket.py", "pydCaller/SampleManager.py", "pydCaller/SchemeProcessor.py", "pydCaller/SpectrumCollector.py",
    "pydCaller/Supervisor.py", "pydCaller/ValveSequencer.py", "ConfigMonitor/ConfigMonitor.py", "ControlBridge/ControlBridge.py",
    "Sensors/MobileKitMonitor.py", "PeakAnalyzer/RunPeakAnalyzer.py", "Utilities/FlowController/FlowController.py",
    "Utilities/IntegrationTool/IntegrationTool.py", "Utilities/RestartSurveyor/RestartSurveyor.py",
    "Utilities/IntegrationBackup/IntegrationBackup.py", "Utilities/Restart/RestoreStartup.py", "MfgUtilities/CalibrateSystem.py",
    "MfgUtilities/CalibrateFsr.py", "MfgUtilities/AdjustWlmOffset.py", "MfgUtilities/ExamineRawRD.py",
    "MfgUtilities/ExamineRDCount.py", "MfgUtilities/LaserLockPrbs.py", "MfgUtilities/LaserPidPrbs.py",
    "MfgUtilities/MakeWarmBoxCalFile.py", "MfgUtilities/MakeWarmBoxCal_NoWlm.py", "MfgUtilities/MakeWlmFile1.py",
    "MfgUtilities/WriteLaserEeprom.py", "MfgUtilities/MakeNoWlmFile.py", "MfgUtilities/WriteWlmEeprom.py",
    "MfgUtilities/DumpEeproms.py", "MfgUtilities/MakeCalFromEeproms.py", "MfgUtilities/FindWlmOffset.py",
    "MfgUtilities/SaveData.py", "MfgUtilities/SaveRaw.py", "MfgUtilities/TestClient.py", "MfgUtilities/ThresholdStats.py",
    "MfgUtilities/CheckLaserCal.py"
]

windowsList = [
    QuickGui, Coordinator, Controller, "pydCaller/StopSupervisor.py", "IPV/IPV.py", "IPV/IPVLicense.py",
    "Utilities/DiagDataCollector/DiagDataCollector.py", supervisorLauncher, "Utilities/SupervisorLauncher/HostStartup.py",
    "Utilities/CoordinatorLauncher/CoordinatorLauncher.py", "Utilities/FluxSwitcher/FluxScheduler.py",
    "Utilities/FluxSwitcher/FluxSwitcherGui.py", "Utilities/ValveDisplay/ValveDisplay.py",
    "Utilities/InstrEEPROMAccess/InstrEEPROMAccess.py", "Utilities/DataRecal/DataRecal.py", "Utilities/SetupTool/SetupTool.py",
    "Utilities/PicarroKML/PicarroKML.py", "Utilities/ReadGPSWS/ReadGPSWS.py", "Utilities/PeriphModeSwitcher/PeriphModeSwitcher.py",
    "Utilities/RecipeEditor/RecipeEditor.py", "Utilities/ConfigManager/ConfigManager.py"
]
...
ret = 0  #_createGitVersionPythonFile(verFilePath, buildTypeStr, branchName)
...
with open(os.path.join(os.path.dirname(__file__), 'Common', 'setup_version.py'), 'w') as fp:
    fp.writelines([
        "# autogenerated by PicarroExeSetup.py, %s\n" % time.asctime(), "\n", "def versionString():\n",
