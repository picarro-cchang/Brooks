"""
File Name: MobileKitArduino.py
Purpose: Polls arduino on mobile kit and sends it as tag-along data

File History:
    14-Apr-2014  anders/sze   Initial version
...
APP_NAME = "MobileKitArduino"

EventManagerProxy_Init(APP_NAME)


class MobileKitArduino(object):
    def __init__(self):
        self.ser = None
        self.arduinoCmds = [("B", "BACKUP_BATT_VOLT"), ("C", "CAR_BATT_VOLT"), ("F", "FAN1_SPEED"), ("G", "FAN2_SPEED"),
                            ("I", "IGNITION_STAT"), ("L", "MOBILE_FLOW"), ("M", "MOBILE_TEMPERATURE"), ("P", "PUMP_1"),
                            ("Q", "PUMP_2"), ("U", "UPS_STAT"), ("Z", "CHASSIS_VOLT")]
...
def main():
    mobileKitArduino = MobileKitArduino()
    ...
    rpcServer = CmdFIFO.CmdFIFOServer(("", RPC_PORT_MOBILE_KIT_ARDUINO),
                                      ServerName="MobileKitArduino",
                                      ServerDescription="MobileKitArduino",
                                      ServerVersion="1.0",
                                      threaded=True)
    try:
        while True:
        ...
        Log("Supervised MobileKitArduino died", Level=2)
    except:
        LogExc("CmdFIFO for MobileKitArduino stopped", Level=2)
    finally:
