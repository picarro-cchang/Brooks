...
Archiver = CmdFIFO.CmdFIFOServerProxy("http://localhost:%d" % RPC_PORT_ARCHIVER, APP_NAME, IsDontCareConnection=False)
...
class SpectrumCollector(object):
    ...
    def __init__(self, configPath):
        self.myProcess = psutil.Process(os.getpid())
        # Read from .ini file
        ...
        ### Try not writing out spectra to see if this reduces memory use
        ###self.enableSpectrumFiles = True
        ...
        self.diagPickle = file("c:/temp/RDFdata.pkl", "wb")
    ...
    def run(self):
        ...
        tincr = 5.0
        psFile = file(r"c:/temp/spectCollect.txt", "w", 0)
        tlast = tstart = time.time()
        mem = self.myProcess.get_memory_info()
        rssOrig = mem.rss
        print >> psFile, "%s: %s" % (tlast, mem)
        ...
        while not self._shutdownRequested:
            ...
            if tnow >= tlast + tincr:
                mem = self.myProcess.get_memory_info()
                tlast += tincr
                print >> psFile, "%s: %s Rate: %s" % (tnow, mem, (mem.rss - rssOrig) / (tnow - tstart))
    ...
    def finish(self):
        ...
        ts = self.rdfDict["rdData"]["timestamp"]
        latency = 0.0
        if len(ts) > 0:
            latency = 0.001 * (getTimestamp() - float(sum(ts)) / len(ts))
        self.rdfDict["controlData"] = {"RDDataSize": [self.numPts], "SpectrumQueueSize": [qsize], "Latency": [latency]}
