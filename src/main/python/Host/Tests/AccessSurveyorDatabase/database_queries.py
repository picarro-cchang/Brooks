...
def getSurveysForAnalyzer(analyzer, center, radius, num_points=20):
    ...
    outline = [coordinatePlusDistance(center, vectors[i].real, vectors[i].imag) for i in xrange(len(vectors))]
...
def get_backtraj_data(analyzer, epoch_time, duration):
    ...
    return dict(epoch_time=np.asarray(epoch_times),
                latitude=np.asarray(lats),
                longitude=np.asarray(lngs),
                car_vel_n=np.asarray(car_vel_norths),
                car_vel_e=np.asarray(car_vel_easts),
                wind_lat=np.asarray(wind_lats),
