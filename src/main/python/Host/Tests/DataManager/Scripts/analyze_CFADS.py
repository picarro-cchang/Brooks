...
if _PERSISTENT_["init"]:
    ...
    _PERSISTENT_["sync"] = Synchronizer("SYNC1", syncList, syncInterval=1000, syncLatency=5000, processInterval=500, maxDelay=20000)
    try:
...
try:
    ...
    h2o_precorr = 0.5 * (h2o_reported + _OLD_DATA_["h2o_reported"][-4].value
                         )  # average of current and last uncorrected h2o measurements

    try:
        ch4_conc_dry = _OLD_DATA_["ch4_conc"][-2].value / (
            1.0 + h2o_precorr * (_INSTR_["ch4_watercorrection_linear"] + h2o_precorr * _INSTR_["ch4_watercorrection_quadratic"]))
        _NEW_DATA_["ch4_conc_dry"] = ch4_conc_dry
        co2_conc_dry = _OLD_DATA_["co2_conc"][-1].value / (
            1.0 + h2o_precorr * (_INSTR_["co2_watercorrection_linear"] + h2o_precorr * _INSTR_["co2_watercorrection_quadratic"]))
        _NEW_DATA_["co2_conc_dry"] = co2_conc_dry
    except:
        _NEW_DATA_["ch4_conc_dry"] = 0.0
        _NEW_DATA_["co2_conc_dry"] = 0.0
    ...
    try:
        h2o_actual = _INSTR_["h2o_selfbroadening_linear"] * (h2o_reported +
                                                             _INSTR_["h2o_selfbroadening_quadratic"] * h2o_reported**2)
...
if _DATA_["SpectrumID"] != 10:
    ...
    t2 = "%02d%02d%02d.%03.0f" % (
        t[3],
        t[4],
        t[5],
        1000 * (_MEAS_TIME_ - int(_MEAS_TIME_)),
    )
