...
class DriveDataManager(object):
    def __init__(self):
        self.sensorCast = Broadcaster(port=BROADCAST_PORT_SENSORSTREAM, name="Sensor Broadcaster", logFunc=Log)

        self.fitterCastList = []
        for fitterIndex in range(interface.MAX_FITTERS):
            self.fitterCastList.append(
                Broadcaster(port=BROADCAST_PORT_FITTER_BASE + fitterIndex, name="Fitter Broadcaster", logFunc=Log))

        self.statusCast = Broadcaster(port=STATUS_PORT_INST_MANAGER, name="Status Broadcaster", logFunc=Log)
