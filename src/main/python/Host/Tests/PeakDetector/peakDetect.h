...
{
    PEAK_DETECT_CNTRL_StateType state; // Valve controller state
    float processedLoss;              // Processed loss
    unsigned int backgroundSamples;   // Number of samples for background calculation
    float background;                 // Background value
    float upperThreshold;             // Upper threshold
    float lowerThreshold1;            // Lower threshold 1
    float lowerThreshold2;            // Lower threshold 2
    unsigned int postPeakSamples;     // Required number of points after peak
    unsigned int activeSize;          // Size of active region to use for peak detection
    unsigned int triggerCondition;    // Specifies logical function for triggering
    unsigned int triggerDelay;        // Number of samples to delay when trigger conditions are met
    unsigned int resetDelay;          // Number of samples after entering triggered state before resetting to idle state
    unsigned int cancellingDelay;     // Number of samples after to remain in cancelling state before resetting to idle state
    unsigned int primingDuration;     // Number of samples after to remain in priming state before entering purging state
    unsigned int purgingDuration;     // Number of samples after to remain in purging state before entering injection pending state
    unsigned int samplesLeft;         // Number of samples left for lengthy operations
    unsigned int idleValveMaskAndValue;           // Valve mask and value for idle state
    unsigned int armedValveMaskAndValue;          // Valve mask and value for armed state
    unsigned int triggerPendingValveMaskAndValue; // Valve mask and value for trigger pending state
    unsigned int triggeredValveMaskAndValue;      // Valve mask and value for triggered state
    unsigned int inactiveValveMaskAndValue;       // Valve mask and value for triggered state
    unsigned int cancellingValveMaskAndValue;     // Valve mask and value for cancelling state
    unsigned int primingValveMaskAndValue;        // Valve mask and value for priming state
    unsigned int purgingValveMaskAndValue;        // Valve mask and value for purging state
    unsigned int injectionPendingValveMaskAndValue;  // Valve mask and value for injection pending state
    unsigned int solenoidValves;      // Solenoid valve mask
    float flow0Setpoint;              // Flow setpoint in state 0
    float flow1Setpoint;              // Flow setpoint in state 1
    float flow2Setpoint;              // Flow setpoint in state 2
    float flow3Setpoint;              // Flow setpoint in state 3
    float flowCntrlSetpoint;          // Flow control setpoint register

    unsigned int cond0, cond1, cond2, cond3;
...
extern PeakDetectCntrl peakDetectCntrl;
int peakDetectCntrlInit(void);
