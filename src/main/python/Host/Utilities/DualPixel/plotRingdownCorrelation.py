...
paramsTemplate = [
    {
        ...
        'children': [
            ...
            {
                'name': 'Save notes and parameters',
                'type': 'action'
            },
        ]
    },
...
class Form(QtGui.QMainWindow):
    ...
    def saveNotes(self):
        ...
        msg.append("Power_law = %.2f" % self.params.param('Basic parameters').getValues()['PowerLaw'][0])
        msg.append("Chan_1_IP_address = %s" % self.params.param('Basic parameters', 'Channel 1').getValues()['IP Address'][0])
        msg.append("Chan_1_delay = %.3f" % self.params.param('Basic parameters', 'Channel 1').getValues()['Delay'][0])
        msg.append("Chan_2_IP_address = %s" % self.params.param('Basic parameters', 'Channel 2').getValues()['IP Address'][0])
    ...
    def update(self):
        ...
        if len(x) == len(y) and len(x) > 1 and not isnan(x).all():
            ...
            if n >= 2:
                ...
                self.asymm.setText("<font color=red size=36><b>%6.2f</b></font>" % (asym1, ))
                self.rmsDev.setText("<font color=green size=36><b>%6.2f</b></font>" % (rmsDev1, ))
...
parser = OptionParser()
parser.add_option("-a", dest="ipAddr1", default="127.0.0.1", help="IP address of first analyzer (def: 127.0.0.1)")
parser.add_option("-b", dest="ipAddr2", default="127.0.0.1", help="IP address of second analyzer (def: 127.0.0.1)")
