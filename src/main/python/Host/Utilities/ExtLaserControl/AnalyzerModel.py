...
Driver = CmdFIFO.CmdFIFOServerProxy("http://localhost:%d" % RPC_PORT_DRIVER, "setup_extended_current", IsDontCareConnection=False)
...
class AnalyzerModel(object):
    ...
    def process_ringdowns(self):
        while not self.rd_queue.empty():
            ...
            if (datum.fineLaserCurrent <= lcm.upper_window and datum.fineLaserCurrent >= lcm.lower_window):
    ...
    def setup_current_gen(self, levels, alpha=0.5, actual_laser=1, bank=0, step_samples=500, lower_window=None, upper_window=None):
            ...
            if lcm is not None:
                ...
                if fsr_indices:
                    ...
                    for i, level in enumerate(levels):
                        if (in_window(level) or (i < len(levels) - 1 and in_window(levels[i + 1]))
                                or (i >= 1 and in_window(levels[i - 1]))):
                    ...
                    if lcm.update_waveform:
                        self.setup_current_gen(lcm.levels,
                                               lcm.slope_factor,
                                               laser_num,
                                               step_samples=lcm.time_between_steps,
