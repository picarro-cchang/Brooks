    ...
    def _saveSettings(self):
        ...
        if self._airType.currentIndex() == self.max_index:
            ...
            try:
                ...
                if float(self._rangeMin.text()) < float(self._rangeMax.text()) and float(self._rangeMin.text()) >= 0 and float(
                        self._rangeMax.text()) <= limitmax:

                    if CONFIG[self.section_name].get('SOURCE_MIN') == 'Disabled' or float(
                            CONFIG[self.section_name].get('SOURCE_MIN')) != float(self._rangeMin.text()):
                    ...
                    if CONFIG[self.section_name].get('SOURCE_MAX') == 'Disabled' or float(
                            CONFIG[self.section_name].get('SOURCE_MAX')) != float(self._rangeMax.text()):
                    ...
                    QMessageBox.warning(
                        self, 'Invalid Range Min and Max', 'Make sure: 0 <= Range Min < Range Max <=%s in %s.' %
                        (str(limitmax), self.unit_dict[str(self._airType.itemText(self._airType.currentIndex()))]))
            except:
        ...
        if isSaved:
            if CONFIG != ini_CONFIG:
                result = QMessageBox.information(
                    self, 'Message', "Write new values to configuration.  \
                \n If your analyzer is in measuring mode, you need to restart the analyzer to make the change take effect.")
                CONFIG.write()
            self.accept()
        else:
            QMessageBox.warning(self, 'Error', 'Save or Undo all channels setting before close')
