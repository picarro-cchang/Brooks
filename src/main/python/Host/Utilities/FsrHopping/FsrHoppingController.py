...
import time
from traitlets import (CaselessStrEnum, CBool, CFloat, CInt, Dict, Float, 
                       ForwardDeclaredInstance, Instance, Integer, List)
...
class LcmParameterOptimizer(PConfigurable):
    ...
    state = CaselessStrEnum(values=['middle', 'low', 'high'], 
    ...
    def process_ringdown_rate(self, ringdown_rate):
        ...
        if self.enabled:
            ...
            if self.state == states[-1]:
                ...
                curve = 0.5*(self.results_by_state['high'] -
                             2 * self.results_by_state['middle'] +
                             self.results_by_state['low'])
...
class LaserCurrentModel(PConfigurable):
    ...
    def setup_uniform_levels(self, reset=True):
        ...
        self.levels = levels
        self.sequence_id = 0
        setup_current_generator(self.actual_laser, slope_factor, time_between_steps,
