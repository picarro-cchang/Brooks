...
import time
from traitlets import (CaselessStrEnum, CBool, CFloat, CInt, Dict, Float, ForwardDeclaredInstance, Instance, Integer, List)
...
class ClusterAnalyzer(PConfigurable):
    """Perform clustering of data in the range [0,1] using kernel density estimation with a Gaussian kernel
        ...
        density estimate"""
    naverage = CFloat(1000.0, config=True)  # Effective number of values (exponential average) used to make histogram
...
class LcmParameterOptimizer(PConfigurable):
    enabled = CBool(True)
    laser_current_model = ForwardDeclaredInstance("LaserCurrentModel")
    var_name = CaselessStrEnum(values=['sawtooth_size', 'time_between_steps'],
                               default_value='time_between_steps')  # Variable to optimize over
    perturbations = Dict(default_value=dict(time_between_steps=25,
                                            sawtooth_size=0.025))  # Changes for the parameters that are to be adjusted
    state = CaselessStrEnum(values=['middle', 'low', 'high'], default_value='middle')
    results_by_state = Dict()

    def get_parameters(self):
        slope_factor = self.laser_current_model.slope_factor
        sawtooth_size = self.laser_current_model.sawtooth_size
        time_between_steps = self.laser_current_model.time_between_steps
        if self.var_name == 'slope_factor':
            if self.state == 'low':
                slope_factor -= self.perturbations['slope_factor']
            elif self.state == 'high':
                slope_factor += self.perturbations['slope_factor']
        if self.var_name == 'sawtooth_size':
            if self.state == 'low':
                sawtooth_size -= self.perturbations['sawtooth_size']
            elif self.state == 'high':
                sawtooth_size += self.perturbations['sawtooth_size']
        if self.var_name == 'time_between_steps':
            if self.state == 'low':
                time_between_steps -= self.perturbations['time_between_steps']
            elif self.state == 'high':
                time_between_steps += self.perturbations['time_between_steps']
        return slope_factor, sawtooth_size, time_between_steps

    def update_parameters(self, shift):
        if self.var_name == 'slope_factor':
            self.laser_current_model.slope_factor += shift * self.perturbations['slope_factor']
        elif self.var_name == 'sawtooth_size':
            self.laser_current_model.sawtooth_size += shift * self.perturbations['sawtooth_size']
            self.laser_current_model.sawtooth_size = max(0.15, self.laser_current_model.sawtooth_size)
        elif self.var_name == 'time_between_steps':
            self.laser_current_model.time_between_steps += shift * self.perturbations['time_between_steps']
        else:
            raise ValueError('Unknown parameter name: %s' % self.var_name)

    def reset_optimizer(self):
        states = self.traits()['state'].values
        self.state = states[0]

    def process_ringdown_rate(self, ringdown_rate):
        states = self.traits()['state'].values
        if self.enabled:
            var_names = self.traits()['var_name'].values
            self.results_by_state[self.state] = ringdown_rate
            if self.state == states[-1]:
                # Calculate update for variable based on ringdown rates
                slope = 0.5 * (self.results_by_state['high'] - self.results_by_state['low'])
                curve = 0.5 * (self.results_by_state['high'] - 2 * self.results_by_state['middle'] + self.results_by_state['low'])
                if curve < 0:  # We have an interior maximum
                    shift = max(min(-0.5 * slope / curve, 0.5), -0.5)
                elif slope > 0:  # Move in uphill direction
                    shift = 0.5
                else:
                    shift = -0.5
                self.update_parameters(shift)
                self.state = states[0]
                # We need to update self.var_name
                if self.var_name == var_names[-1]:
                    self.var_name = var_names[0]
                else:
                    self.var_name = var_names[var_names.index(self.var_name) + 1]
            else:
...
class LaserCurrentModel(PConfigurable):
    """Model controlling the laser current waveform generator for a single laser. The generated waveform is defined
    via a list of ``levels`` which specify the laser current values (in digitizer units) at the knots. The output
    current changes from the value at one knot to the next via a *transition template*. The time spent
    transitioning from one knot to the next is specified by ``time_between_steps``, which is measured in multiples
    of 10 microseconds.

    The transition template is parameterized by a parameter called the ``slope_factor``, which lies in the range 0.0 to 0.5.
    When the ``slope_factor`` is 0.5, the template specifies a linear transition from the initial value to the final value.
    Other values of the ``slope factor`` give a piecewise linear transition with three segments, with the slope around the
    knot levels becoming flatter as the factor is decreased. When the ``mode`` is set to ``sawtooth``, the ``slope factor``
    is fixed at 0.5, and the output waveform transitions linearly between the knots which define the teeth of the waveform.
    When the ``mode`` is set to ``staircase``, the knots define the levels at the midpoint of the tread of the staircase,
    while the ``slope factor`` adjusts the slopes and relative time spent on the treads and risers of the staircase.

    The ``window`` consists of two numbers, a ``lower_window`` and a ``upper_window`` which specify the range of currents
    between which a ringdown is allowed to occur.

    The class has two important methods called ``setup_uniform_levels`` and ``setup_custom_levels`` which are respectively
    used to generate a waveform with equally-spaced knots and a waveform with custom-generated knots.
    """
    actual_laser = CInt(-1)
    ...
    lcm_parameter_optimizer = Instance(LcmParameterOptimizer)
    reset_flag = CBool(False)
    sawtooth_size = CFloat(0.25, config=True)
    sequence_id = CInt(0)  # Set to zero when using uniformly-spaced knots, increments when using custom-spaced knots
    slope_factor = CFloat(0.5, config=True)
    time_between_steps = CInt(600, config=True)
    optimize_levels = CBool(False, config=True)
    optimize_parameters = CBool(True, config=True)
    upper = Float()
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.lcm_parameter_optimizer = LcmParameterOptimizer()
        self.lcm_parameter_optimizer.laser_current_model = self
    ...
    def _time_between_steps_changed(self):
        self.lcm_parameter_optimizer.reset_optimizer()

    def _slope_factor_changed(self):
        self.lcm_parameter_optimizer.reset_optimizer()

    def _sawtooth_size_changed(self):
        self.lcm_parameter_optimizer.reset_optimizer()

    def _optimize_parameters_changed(self):
        self.lcm_parameter_optimizer.enabled = self.optimize_parameters

    def setup_uniform_levels(self, reset=True):
        ...
        If reset is True, the sawtooth size and slope factor are reset to fail-safe values and the 
        density estimation calculation is reset.
        """

        slope_factor, sawtooth_size, time_between_steps = self.lcm_parameter_optimizer.get_parameters()
        ...
        if reset:
            sawtooth_size = 0.25
            self.sawtooth_size = sawtooth_size
            slope_factor = 0.5
        ...
        self.levels = levels
        self.sequence_id = 0
        setup_current_generator(self.actual_laser, slope_factor, time_between_steps, self.lower_window, self.upper_window,
        ...
        for i, current in enumerate(mode_currents):
            ...
            if self.mode == 'sawtooth':
                ...
                for j in range(dwell):
                    fac = np.exp(-(dwell - 1))
                    size = sawtooth_size * fac + 0.35 * (1 - fac)
                    levels.append(int(current - size * median_slope))
                    levels.append(int(current + size * median_slope))
            elif self.mode == 'staircase':
                levels.append(int(current))
...
class FsrHoppingController(PConfigurable):
    """Collects ringdown data to find fine laser currents at which ringdowns occur and supervises
    a number of LaserCurrentModel instances to generate laser current waveforms. Also provides an
    RPC server to control the operation of the laser current models.
    """
    ...
    rd_rates = List(default_value=[[] for _ in range(interface.MAX_LASERS)])
    timestamps_at_rd = List(List(Integer), [[] for _ in range(interface.MAX_LASERS)])
    ...
    def main_loop(self):
        """Collect fine laser current values at the ringdowns and place them in the lists
        ``self.currents_at_rd[actual_laser-1]``, one for each actual laser. When the list for an
        actual laser reaches the length ``lcm.batch_size``, we process it using ``lcm.cluster_currents``
        to find the positions of the cavity resonances.

        If ``lcm.optimize_levels`` is set for the laser, we use the estimated mode currents to set up a
        customized collection of knots for the waveform generator so as to better target the resonances.
        If ``lcm.optimize_levels`` is ot set, the waveform generator is set up with uniformly spaced knots.
        ...
        while self.rpc_thread.isAlive():
            try:
                ...
                self.timestamps_at_rd[actual_laser - 1].append(datum.timestamp)
                lcm = self.lcm[actual_laser - 1]
                if len(self.currents_at_rd[actual_laser - 1]) == lcm.batch_size:
                    # Calculate the average ringdown rate over the batch size
                    timestamps = np.asarray(self.timestamps_at_rd[actual_laser - 1], dtype=np.int64)
                    dt = np.diff(timestamps)
                    good = (dt <= 500)  # Discard intervals longer than 500ms
                    rd_rate = 0.0
                    if np.any(good):
                        rd_rate = 1000.0 / np.mean(dt[good])
                        print "Ringdown rate:", lcm.slope_factor, lcm.sawtooth_size, lcm.time_between_steps, rd_rate
                        lcm.lcm_parameter_optimizer.process_ringdown_rate(rd_rate)
                    if lcm.optimize_levels:
                        ...
                        if lcm.sequence_id == 0:
                            ...
                            lcm.setup_uniform_levels(reset=False)
                        else:
                            lcm.setup_uniform_levels(reset=True)
                    self.currents_at_rd[actual_laser - 1] = []
                    self.timestamps_at_rd[actual_laser - 1] = []
            except Queue.Empty:
                for lcm in self.lcm:
                    if lcm is not None:
    ...
    def get_optimize_parameters_flags(self):
        """Get a list of four booleans indicating if the parameters of the
        current waveform generator are being dithered to optimize the ringdown 
        rate """
        return self.get_lcm_param('optimize_parameters')
    ...
    def set_optimize_parameters_flags(self, flag, lasers=None):
        """Turn on or off the optimization of waveform generator parameters
        for the specified laser or list of lasers. If the flag is False
        fo some laser, the parameter values remain fixed. If True, the 
        parameters are dithered to optimize the ringdown rate."""
        self.set_lcm_param('optimize_parameters', flag, lasers)
    ...
    def register_rpc(self):
        ...
        self.rpc_server.register_function(self.get_optimize_parameters_flags)
        ...
        self.rpc_server.register_function(self.set_optimize_parameters_flags)
