...
import calendar
import time
from Host.Utilities.GlobalHawkComms.StreamDisplayPanelGui import StreamDisplayPanelGui
...
def makeEnv(dataList):
    ...
    for i in range(14):
        env['Data%d' % (i + 1, )] = safeFloat(dataList[3 + i])
    return env
...
class StreamDisplayPanel(StreamDisplayPanelGui):
    def setSource(self, source, nCols, maxPoints):
        ...
        self.graph_panel_stream_display.SetGraphProperties(xlabel='Time',
                                                           timeAxes=(True, False),
                                                           ylabel='Value',
                                                           grid=True,
        ...
        self.graph_panel_stream_display.AddSeriesAsPoints(self.waveform,
                                                          colour='blue',
                                                          width=1,
                                                          fillcolour='blue',
                                                          marker='square',
