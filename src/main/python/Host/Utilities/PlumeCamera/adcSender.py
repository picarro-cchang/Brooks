...
class MCC_Board(object):
    ...
    def adcGetBlock(self):
        self.measComp.cbGetStatus(self.boardNum, byref(self.status), byref(self.curCount), byref(self.curIndex), mc.AIFUNCTION)
    ...
    def start(self):
        ...
        def _start(self):
            ...
            self.measComp.cbAInScan(self.boardNum, self.adcLowChan, self.adcHighChan, self.bufferLength, byref(self.actualRate),
                                    self.adcGain, self.memHandle, self.adcOptions)
            return False
