...
#
# FILE:
#   StartupCalibration.py
#
# DESCRIPTION:
#   Utility to calibrate the warm box on system startup
#
# SEE ALSO:
#   Specify any related information.
#
# HISTORY:
#  15-Jun-2017  sze  Initial release
#
...
from Host.Utilities.SelfCalibration.Listener import Listener
from Host.Common.SharedTypes import (BROADCAST_PORT_RDRESULTS, BROADCAST_PORT_SENSORSTREAM, RPC_PORT_DRIVER,
                                     RPC_PORT_FREQ_CONVERTER, RPC_PORT_INSTR_MANAGER, RPC_PORT_STARTUP_CALIBRATOR)
from Host.Common.timestamp import getTimestamp, unixTime
from Host.Utilities.SelfCalibration import hapi
from Host.Utilities.SelfCalibration.WlmCalUtilities import (AutoCal, WlmSat, bestFitCentered, bspEval, bspUpdate, parametricEllipse,
                                                            penalty)

APP_NAME = "StartupCalibration"
...
InstMgr = CmdFIFO.CmdFIFOServerProxy("http://localhost:%d" % RPC_PORT_INSTR_MANAGER, APP_NAME, IsDontCareConnection=False)
...
class Environment(FixedTraits):
    ...
    FILENAME = Unicode("StartupCalibration")
...
class StartupCalibration(object):
    def __init__(self, configFile, options):
        ...
        self.rpcServer = CmdFIFO.CmdFIFOServer(("", RPC_PORT_STARTUP_CALIBRATOR),
                                               ServerName="Startup Calibrator",
                                               ServerDescription="Startup Calibrator",
                                               threaded=True)
    ...
    def associateRingdownsByAngle(self, thetaValues):
        ...
        On the other hand if no key lies within the specified tolerance, a new list is created at rd[idx_high][idx].
    ...
    def bin_fsr(self, thetaCal, lossData, binMethod, figBase=None):
    ...
    def collectPztFsrInfo(self):
        ...
        try:
            ...
            Driver.wrDasReg(interface.PZT_OFFSET_VIRTUAL_LASER1, 0)
    ...
    def fit_fsr_spectrum(self, dbaseName, species, wmin, wmax, p_torr, temp_C, fsr_spectrum, cavity_fsr=0.0, figBase=None):
        ...
        if cavity_fsr != 0.0:
            ...
            # Go through the most promising candidates and do a LM search for
            # each
            res_candidates = []
            pfit_candidates = []
            pcov_candidates = []
            ...
            while True:
                ...
                sdev_nu0 = np.sqrt(pcov[0][0])
                logging.info("Decim %d: %.5f +/- %.5f" % (decim, nu0, sdev_nu0))
                nuGrid = nu0 + np.arange(len(coarseData)) * decim * cavity_fsr
                if decim == 1:
                    self.qualityMetrics["Frequency offset error laser %d" % self.laserNum] = (sdev_nu0, sdev_nu0 < 2.0e-4)
                    ...
                    self.qualityMetrics["Spectrum residual laser %d" % self.laserNum] = (fracMisfit, fracMisfit < 1.0e-4)
                    plt.figure()
            ...
            while True:
                ...
                sdev_nu0 = np.sqrt(pcov[0][0])
                sdev_fsr = np.sqrt(pcov[1][1])
                logging.info("Decim %d: %.5f +/- %.5f FSR: %.7f +/- %.7f" % (decim, nu0, sdev_nu0, fsr, sdev_fsr))
    ...
    def makeWarmBoxCalibrationFile(self):
        ...
        with openFile(self.h5Filename, "r") as self.h5:
            for self.laserNum in self.actualLasers:
                ...
                if ("-f" in self.options) or (set(self.species.keys()) - set(hapi.getTableNamesFromStorage(dbname))):
                ...
                slope_grid, offset_grid = np.meshgrid(np.arange(-1.4, -2.4, -0.1), np.arange(-3, 4, 1.0))
                ...
                peak_pos, loss_mean, peak_std, loss_std = self.bin_fsr(thetaCal,
                                                                       data["uncorrectedAbsorbance"],
                                                                       self.binMethod,
                                                                       figBase=self.figBase + "_laser%d" % (self.laserNum, ))
                logging.info("Starting fit_fsr_spectrum")
                nu0, fsr, nu_err, fsr_err = self.fit_fsr_spectrum(dbname,
                                                                  self.species,
                                                                  self.wavenumMin,
                                                                  self.wavenumMax,
                                                                  self.cavityPressure,
    ...
    def measureDarkCurrents(self):
        ...
        ok = (5000 < table.attrs.etalon1_mean < 7000) and table.attrs.etalon1_std <= 10
        self.qualityMetrics["Etalon 1 dark current statistics"] = ((table.attrs.etalon1_mean, table.attrs.etalon1_std), ok)
        if not ok:
            logging.warning("Etalon 1 dark current out of range")
        ok = (5000 < table.attrs.etalon2_mean < 7000) and table.attrs.etalon2_std <= 10
        self.qualityMetrics["Etalon 2 dark current statistics"] = ((table.attrs.etalon2_mean, table.attrs.etalon2_std), ok)
        if not ok:
            logging.warning("Etalon 2 dark current out of range")
        ok = (5000 < table.attrs.reference1_mean < 7000) and table.attrs.reference1_std <= 10
        self.qualityMetrics["Reference 1 dark current statistics"] = ((table.attrs.reference1_mean, table.attrs.reference1_std), ok)
        if not ok:
            logging.warning("Reference 1 dark current out of range")
        ok = (5000 < table.attrs.reference2_mean < 7000) and table.attrs.reference2_std <= 10
        self.qualityMetrics["Reference 2 dark current statistics"] = ((table.attrs.reference2_mean, table.attrs.reference2_std), ok)
        if not ok:
    ...
    def run(self):
            ...
            try:
                ...
                regVault = Driver.saveRegValues(
                    ...
                    ["LASER%d_TEMP_CNTRL_SWEEP_INCR_REGISTER" % (a + 1, ) for a in range(nl)] + [
                        ...
                        "VALVE_CNTRL_STATE_REGISTER", "VALVE_CNTRL_CAVITY_PRESSURE_SETPOINT_REGISTER",
                        "VALVE_CNTRL_USER_INLET_VALVE_REGISTER", "VALVE_CNTRL_USER_OUTLET_VALVE_REGISTER", "VIRTUAL_LASER_REGISTER",
                        ("FPGA_TWGEN", "TWGEN_SLOPE_UP"), ("FPGA_TWGEN", "TWGEN_SLOPE_DOWN"), ("FPGA_TWGEN", "TWGEN_CS"),
                        ("FPGA_RDMAN", "RDMAN_OPTIONS"), ("FPGA_INJECT", "INJECT_CONTROL"),
                ...
                # TODO: Wait until the laser current loop is enabled
        ...
        ok = True
        for metric in self.qualityMetrics:
            ok = ok and self.qualityMetrics[metric][1]
            if not self.qualityMetrics[metric][1]:
                logging.warning("Quality metric %s failed: %s" % (metric, self.qualityMetrics[metric][0]))
        if ok:
            warmBoxFactory, warmBoxActive, hotBoxFactory, hotBoxActive = RdFreqConverter.getCalFileNames()
            nowString = time.strftime("%Y%m%d_%H%M%S", time.localtime())
            if os.path.isfile(warmBoxActive):
                shutil.move(warmBoxActive, warmBoxActive.replace(".ini", "_%s.ini" % (nowString, )))
            if os.path.isfile(warmBoxFactory):
                shutil.move(warmBoxFactory, warmBoxFactory.replace(".ini", "_%s.ini" % (nowString, )))
            config = ConfigObj(self.wbCalFilename)
            ...
            with file(warmBoxActive, "wb") as filePtr:
                filePtr.write(calStr)
            logging.info("Warm box calibration successful - Copying warm box calibration file")
        self.sensorListener.stop()
        self.rindownListener.stop()
        InstMgr.INSTMGR_StartRpc()

    def scanPzt(self):
        self.scanPztValue += 2000
        Driver.wrDasReg(interface.PZT_OFFSET_VIRTUAL_LASER1, self.scanPztValue)
...
HELP_STRING = """
StartupCalibration.py [-a] [-f] [-c<FILENAME>] [-h|--help]
...
-c                   specify a config file:  default = "./StartupCalibration.ini"
"""
...
if __name__ == "__main__":
    ...
    try:
        m = StartupCalibration(configFile, options)
        m.run()
    except:
        logging.error(traceback.format_exc())

# What is required for proper measurement?
# 1. Cavity temperature is not too low (condensation?)
# 2. Etalon temperature is above minimum for proper reading - should keep etalon
#     temperature approx constant during calibration

# Warnings of improper operation
# 1. RD rate < 70 rd/s on either laser
# 2. PZT sensitivity worse than 40000 digU
# 3. WLM ratios do not fit ellipse well, or ratios are out of range

# QA metrics for warm box cal file
# 1. Fractional misfit of spectrum < 1e-4 for each laser
# 2. Error in each frequency offset < 2e-4 wavenumbers
# 3. Std dev of dark currents < 10.0 digU
# If these are met, we copy the warm box cal file and rename the old files
# (including active file)

# Added getCalFileNames RPC call to RDFrequencyConverter
