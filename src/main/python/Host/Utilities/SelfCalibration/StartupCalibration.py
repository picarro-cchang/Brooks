...
from Host.Common.SharedTypes import (BROADCAST_PORT_RDRESULTS,
                                     BROADCAST_PORT_SENSORSTREAM,
                                     RPC_PORT_DRIVER, RPC_PORT_FREQ_CONVERTER,
                                     RPC_PORT_INSTR_MANAGER,
                                     RPC_PORT_STARTUP_CALIBRATOR)
from Host.Common.timestamp import getTimestamp, unixTime
from Host.Utilities.SelfCalibration import hapi
from Host.Utilities.SelfCalibration.WlmCalUtilities import (AutoCal, WlmSat,
                                                            bestFitCentered,
                                                            bspEval, bspUpdate,
                                                            parametricEllipse,
                                                            penalty)
...
InstMgr = CmdFIFO.CmdFIFOServerProxy("http://localhost:%d" % RPC_PORT_INSTR_MANAGER,
                                     APP_NAME, IsDontCareConnection=False)
...
class StartupCalibration(object):
    ...
    def bin_fsr(self, thetaCal, lossData, binMethod, figBase=None):
        ...
        logging.info(
            "Starting kernel density estimation for locating cavity FSRs")
        ...
        enough = (peak_count >= 0.05 * np.mean(peak_count)
                  ) & ~np.isnan(peak_pos)
        ...
        if cavity_fsr != 0.0:
            ...
            while True:
                ...
                logging.info("Decim %d: %.5f +/- %.5f FSR: %.7f +/- %.7f" %
                             (decim, nu0, sdev_nu0, fsr, sdev_fsr))
        ...
        with openFile(self.h5Filename, "r") as self.h5:
            for self.laserNum in self.actualLasers:
                ...
                slope_grid, offset_grid = np.meshgrid(
                    np.arange(-1.4, -2.4, -0.1), np.arange(-3, 4, 1.0))
                ...
                peak_pos, loss_mean, peak_std, loss_std = self.bin_fsr(
                    thetaCal, data["uncorrectedAbsorbance"],
                    self.binMethod,
                    figBase=self.figBase + "_laser%d" % (self.laserNum,))
                logging.info("Starting fit_fsr_spectrum")
                nu0, fsr, nu_err, fsr_err = self.fit_fsr_spectrum(
                    dbname, self.species, self.wavenumMin, self.wavenumMax, self.cavityPressure,
