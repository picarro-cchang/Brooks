...
def penalty(k, x):
    """Compute the action of (D_k)^t D_k on a vector x, where D_k is the matrix representation
        of the k'th finite-difference operator"""
    return diff(concatenate((zeros(k), diff(x, k), zeros(k))), k)
        ...
        for k in range(len(ix)):
            ...
            if i >= 0 and i < self.N:
                self.coeffs[i:i + 4] = self.coeffs[i:i + 4] + relax * \
                    W[k, :] * (y[k] - dot(W[k, :], self.coeffs[i:i + 4]))
...
def fitEllipse(X, Y):
    """Compute parameters of the best-fit ellipse to the 2D points (x,y). The result is
        returned as (x0,y0,r1,r2,phi) where (x0,y0) is the center of the ellipse,
        and r1 and r2 are the semi-principal axes lengths. The first principal axis makes an
...
class WlmSat(object):
    # Class to compensate for wavelength monitor photodiode saturation effects. An object of this class is
    #  associated with each actual laser, and it is used to modify the raw wavelength monitor angle based
    #  on the fine laser current. It does so by evaluating a B-spline at the wavlength monitor angle and
    # using the result as a factor to apply to the fine laser current in order
    # to obtain an angle correction
    ...
    def loadFromIni(self, ini, vLaserNum):
        ...
        The INI file must have valid WLMSAT_PARAMS and WLMSAT_COEFFS sections for the specified
        vLaserNum. If all sections are all missing, this returns None. Otherwise, an exception
        is raised."""

        paramSec = "WLMSAT_PARAMS_%d" % vLaserNum
        coeffSec = "WLMSAT_COEFFS_%d" % vLaserNum
        ...
        self.ratio1Ampl = float(ini[paramSec]["RATIO1_AMPL"])
        self.ratio2Ampl = float(ini[paramSec]["RATIO2_AMPL"])
        self.tempCen = float(ini[paramSec]["TEMP_CEN"])
        self.tempScale = float(ini[paramSec]["TEMP_SCALE"])
        self.t2aCoeffs = fetchCoeffs(ini[paramSec], "T2A_")
        self.coeffs = []
        for i in range(self.nCoeffs):
            self.coeffs.append(float(ini[coeffSec]["COEFF%d" % i]))
        self.coeffs = array(self.coeffs)
        return self

    def updateIni(self, ini, aLaserNum):
        """Update (and possibly create) sections and keys in "ini" to describe the current
        AutoCal object, using the specified "aLaserNum" to indicate which actual laser
        (1-origin) is involved"""
        paramSec = "WLMSAT_PARAMS_%d" % aLaserNum
        coeffSec = "WLMSAT_COEFFS_%d" % aLaserNum
        if paramSec not in ini:
            ini[paramSec] = {}
        if coeffSec not in ini:
            ini[coeffSec] = {}
        sec = ini[paramSec]
        sec["ANGLE_BASE"] = self.thetaBase
        sec["ANGLE_INCREMENT"] = self.dTheta
        sec["COARSE_CURRENT"] = self.coarseCurrent
        sec["NCOEFFS"] = self.nCoeffs
        sec["RATIO1_AMPL"] = self.ratio1Ampl
        sec["RATIO2_AMPL"] = self.ratio2Ampl
        sec["TEMP_CEN"] = self.tempCen
        sec["TEMP_SCALE"] = self.tempScale
        sec["T2A_0"], sec["T2A_1"], sec["T2A_2"], sec["T2A_3"] = self.t2aCoeffs
        for i in range(len(self.coeffs)):
            ini[coeffSec]["COEFF%d" % i] = self.coeffs[i]
...
class AutoCal(object):
    ...
    def loadFromWlmFile(self, wlmFile, dTheta=0.05, wMin=None, wMax=None):
        ...
        with self.lock:
            ...
            self.wlmTempSensitivity = -0.185 * \
                (mean(wlmFile.WaveNumber[window]) / 6158.0)  # radians/degC
            self.autocalStatus = 0
        return self
    ...
    def loadFromEepromDict(self, eepromDict, dTheta, wMin, wMax):
        ...
        with self.lock:
            ...
            # Temperature sensitivity of etalon
            self.wlmTempSensitivity = -0.185 * \
                (mean(waveNumber[window]) / 6158.0)  # radians/degC
            self.autocalStatus = 0
        return self
    ...
    def loadFromIni(self, ini, vLaserNum):
        ...
        with self.lock:
            ...
            self.nCoeffs = int(ini[paramSec]["NCOEFFS"])
            self.calPressure = float(ini[paramSec].get("CAL_PRESSURE", 760.0))
            self.pressureC0 = float(ini[paramSec].get("PRESSURE_C0", 0.0))
            self.pressureC1 = float(ini[paramSec].get("PRESSURE_C1", 0.0))
            self.pressureC2 = float(ini[paramSec].get("PRESSURE_C2", 0.0))
            self.pressureC3 = float(ini[paramSec].get("PRESSURE_C3", 0.0))
            self.coeffs = []
            for i in range(self.nCoeffs):
    ...
    def updateIni(self, ini, vLaserNum):
        ...
        with self.lock:
            ...
            ini[paramSec]["CAL_PRESSURE"] = self.calPressure
            ini[paramSec]["PRESSURE_C0"] = self.pressureC0
            ini[paramSec]["PRESSURE_C1"] = self.pressureC1
            ini[paramSec]["PRESSURE_C2"] = self.pressureC2
            ini[paramSec]["PRESSURE_C3"] = self.pressureC3
    ...
    def wlmAngleAndLaserTemp2ThetaCal(self, wlmAngle, laserTemp):
        """Convert the wlmAngle (which typically lies between -pi and pi) to unwrapped angles thetaCal
    ...
    def wlmAngleAndLaserTemp2ThetaCalAndThetaRaw(self, wlmAngle, laserTemp, etalonTemperature, ambientPressure):
        """Convert the wlmAngle (which typically lies between -pi and pi) to unwrapped angles thetaCal and thetaRaw
        using the laser temperature, etalon temperature and ambient pressure and the information in the AutoCal
        object.
        """
        with self.lock:
            approxWaveNum = self.laserTemp2WaveNumber(laserTemp)
