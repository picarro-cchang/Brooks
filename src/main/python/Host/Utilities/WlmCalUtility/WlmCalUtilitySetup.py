...
WlmCalUtility = Target(
    description="WlmCalUtility",  # used for the versioninfo resource
    script="WlmCalUtility.py",  # what to build
    other_resources=[(RT_MANIFEST, 1, manifest_template % dict(prog="WlmCalUtility"))],
...
inclusionList = [
    "scipy.interpolate", "scipy.misc", "sip", "matplotlib.backends", "matplotlib.backends.backend_wxagg",
    "matplotlib.backends.backend_qt4agg", "matplotlib.figure", "pylab", "numpy", "configobj", "encodings.*", "tables.*"
]
