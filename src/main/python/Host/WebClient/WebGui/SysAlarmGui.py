...
class SysAlarmViewListCtrl(wx.ListCtrl):
    ...
    """
    def __init__(self, parent, id, attrib, DataSource=None, pos=wx.DefaultPosition, size=wx.DefaultSize, numAlarms=2):
...
class SysAlarmInterface(object):
    ...
    def getStatus(self, index):
        if index == 0:
            ...
            good = pressureLocked and cavityTempLocked and warmboxTempLocked and measActive and (not warmingUp) and (
                not systemError)
            if not good:
                descr = "System Status:"
                if warmingUp:
