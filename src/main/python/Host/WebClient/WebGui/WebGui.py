...
class ShutdownDialog(wx.Dialog):
    def __init__(self, mainForm, *args, **kwds):
        ...
        self.selectShutdownType = wx.RadioBox(
            ...
            choices=["Turn Off Analyzer in Current State", "Stop Analyzer Software Only", "Restart Analyzer Software"],
            majorDimension=3,
...
class DataStore(object):
    ...
    def getMeasMode(self):
        return self.dataStruct.keys()[0]

    def getSources(self):
        return sorted(self.dataStruct[self.measMode].keys())

    def getKeys(self, source):
        return self.dataStruct[self.measMode][source].keys()
...
class QuickGui(wx.Frame):
    def __init__(self, configFile):
        wx.Frame.__init__(self,
                          ...
                          size=(1200, 700),
                          style=wx.CLOSE_BOX | wx.CAPTION | wx.MINIMIZE_BOX | wx.MAXIMIZE_BOX | wx.RESIZE_BORDER | wx.SYSTEM_MENU
                          | wx.TAB_TRAVERSAL)
        ...
        self.restartCmd = self.config.get(
            'Web', 'restartCmd',
    ...
    def setLineMarkerColor(self, lineMarkerColor=None, colorTime=None):
        ...
        for idx in range(self.numGraphs):
            if colorTime == None:
                cTime = timestamp.unixTime(self.series[idx].x.GetLatest())
            else:
                cTime = colorTime
    ...
    def layoutFrame(self):
        ...
        for idx in range(self.numGraphs):
            ...
            try:
                self.series.append(GraphPanel.Series(self.seqPoints))
            except:
    ...
    def dataKeyUpdateAction(self, idx):
        ...
        ds = self.dataStore
        self.graphPanel[idx].RemoveAllSeries()
        self.series[idx] = GraphPanel.Series(self.seqPoints)
    ...
    def OnTimer(self, evt):
        ...
        for idx in range(self.numGraphs):
            ...
            # JSON-RPC
            self.jsonRpcService.plotData(wfm=self.series[idx],
        ...
        for idx in range(self.numGraphs):
            ...
            if self.dataStore != None and self.source[idx] != None and self.dataKey[idx] != None:
                timeSeq = self.series[idx].x
                v = self.series[idx].y.GetLatest()
