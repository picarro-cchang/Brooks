...
from Host.WebClient.WebGui.PulseAnalyzerGui import PulseAnalyzerGui
from Host.WebClient.WebGui.UserCalGui import UserCalGui
from Host.WebClient.WebGui.SysAlarmGui import *
...
class ShutdownDialog(wx.Dialog):
    def __init__(self, mainForm, *args, **kwds):
        ...
        self.selectShutdownType = wx.RadioBox(self,
                                              -1,
                                              "Select shutdown method",
                                              choices=[
                                                  "Turn Off Analyzer and Prepare For Shipping",
                                                  "Turn Off Analyzer in Current State", "Stop Analyzer Software Only"
                                              ],
                                              majorDimension=3,
...
class AlarmDialog(wx.Dialog):
    ...
    def __init__(self,
        ...
        self.mode = wx.ComboBox(parent=self,
                                id=-1,
                                size=(125, -1),
...
class AlarmViewListCtrl(wx.ListCtrl):
    ...
    """
    def __init__(self, parent, id, attrib, DataSource=None, pos=wx.DefaultPosition, size=wx.DefaultSize, numAlarms=4):
        ...
        self.IconAlarmClear = myIL.Add(wx.Bitmap(thisDir + '/LEDoff2.ico', wx.BITMAP_TYPE_ICO))
        self.IconAlarmSet = myIL.Add(wx.Bitmap(thisDir + '/LEDred2.ico', wx.BITMAP_TYPE_ICO))
    ...
    def OnLeftDown(self, evt):
        ...
        if self._DataSource.alarmData:
            ...
            clear1 = "%.2f" % clear1
            clear2 = "%.2f" % clear2
            d = dict(name=name, mode=mode, enabled=enabled, alarm1=alarm1, clear1=clear1, alarm2=alarm2, clear2=clear2)
            dialog = AlarmDialog(self.mainForm, d, None, -1, "Setting alarm %d" % (item + 1, ))
            retCode = dialog.ShowModal()
            dialog.Destroy()
            if retCode == wx.ID_OK:
                self._DataSource.setAlarm(item + 1, d["enabled"], d["mode"], float(d["alarm1"]), float(d["clear1"]),
...
class AlarmInterface(object):
    ...
    def __init__(self, config):
        ...
        self.listener = Listener.Listener(self.queue, SharedTypes.STATUS_PORT_ALARM_SYSTEM, AppStatus.STREAM_Status, retry=True)
            ...
            for opt in config.list_options(secname):
                if opt.startswith(keyPrefix.lower()):
                    ...
                    for sp, default in zip(substPrefixList, defaultSubst):
                        try:
                            repl.append(getInnerStr(config.get(secname, "%s%d" % (
                                sp.lower(),
                                index,
                            ))))
                        except KeyError:
...
class DataStore(object):
    ...
    def getQueuedData(self, tstart, tstop):
        dataStruct = jsonRpcTools.getDataStruct(tstart, tstop)
        self.measMode = dataStruct.keys()[0]
        sources = dataStruct[self.measMode].keys()
        for source in sources:
            allKeys = dataStruct[self.measMode][source].keys()
            ...
            if source not in self.sourceDict:
                ...
                for k in allKeys:
                    d[k] = GraphPanel.Series(self.seqPoints)
            else:
                d = self.sourceDict[source]
    ...
    def getTime(self, source):
        return self.sourceDict[source]['timestamp'].y
    ...
    def getDataSeries(self, source, key):
        return self.sourceDict[source][key]
...
class QuickGui(wx.Frame):
    ...
    def setLineMarkerColor(self, lineMarkerColor=None, colorTime=None):
        ...
        for idx in range(self.numGraphs):
            if colorTime == None:
                cTime = timestamp.unixTime(ds.getTime(self.source[idx]).GetLatest())
            else:
                cTime = colorTime
    ...
    def layoutFrame(self):
        ...
        self.titleLabel = wx.StaticText(parent=self.mainPanel,
                                        id=-1,
        ...
        for idx in range(self.numGraphs):
            ...
            gp.SetGraphProperties(ylabel='Y',
                                  timeAxes=((self.config.getfloat("Graph",
                                                                  "TimeOffset_hr"), self.config.getboolean("Graph",
                                                                                                           "UseUTC")), False),
            ...
            try:
                self.series.append(ds.getDataSeries(self.source[idx], self.dataKey[idx]))
            except:
        ...
        self.instStatusPanel = InstStatusPanel(font=self.getFontFromIni('InstStatPanel'), parent=self.measPanel, id=-1, size=size)
        ...
        self.userLogTextCtrl = wx.TextCtrl(parent=self.measPanel,
                                           id=-1,
                                           size=(-1, 120),
    ...
    def OnResetBuffers(self, evt):
        for s in self.getSourcesbyMode():
            for k in self.dataStore.getKeys(s):
                self.dataStore.getDataSeries(s, k).Clear()
    ...
    def dataKeyUpdateAction(self, idx):
        ...
        ds = self.dataStore
        self.graphPanel[idx].RemoveAllSeries()
        self.series[idx] = ds.getDataSeries(self.source[idx], self.dataKey[idx])
        ...
        if selection != None:
            ...
            if selection.strip():
                try:
                    ...
                    selSequence = ds.getDataSeries(self.source[idx], selKey).y
                ...
                selection = None
        self.graphPanel[idx].AddSeriesAsLine(self.series[idx], selection, statsFlag=True, width=self.defaultLineWidth)
        self.graphPanel[idx].AddSeriesAsPoints(self.series[idx],
                                               selection,
    ...
    def OnTimer(self, evt):
        ...
        for idx in range(self.numGraphs):
            ...
            # JSON-RPC
            jsonRpcTools.plotData(wfm=self.series[idx],
                                  data=self.dataKey[idx],
                                  mode=measMode,
                                  source=self.source[idx],
        ...
        for idx in range(self.numGraphs):
            ...
            if self.dataStore != None and self.source[idx] != None and self.dataKey[idx] != None:
                ...
                v = self.dataStore.getDataSeries(self.source[idx], self.dataKey[idx]).y.GetLatest()
        ...
        if self.showInstStat:
            try:
                cavityTemp = self.dataStore.getDataSeries(self.instStatSource, self.instStatCavityTempKey).y.GetLatest()
                if cavityTemp != 0.0:
            ...
            try:
                warmBoxTemp = self.dataStore.getDataSeries(self.instStatSource, self.instStatWarmBoxTempKey).y.GetLatest()
                if warmBoxTemp != 0.0:
            ...
            try:
                cavityPressure = self.dataStore.getDataSeries(self.instStatSource, self.instStatCavityPressureKey).y.GetLatest()
    ...
    def OnUserCal(self, evt):
        ...
        if getUserCals:
            ...
            for idx in range(numConcs):
                if dlg.textCtrlList[2 * idx].GetValue() != userCalList[2 * idx][2] or dlg.textCtrlList[
    ...
    def OnPulseAnalyzerParam(self, evt):
        ...
        pulseConfigList = []
        for conc in concList:
            concThresList.append(
                (conc, "%s threshold" % conc, str(self.dataManagerRpc.PulseAnalyzer_GetParam(self.pulseSource, "threshold", conc))))
        concThresList.sort()
        pulseConfigList.append(
            ("waitTime", "Wait Time (seconds)", str(self.dataManagerRpc.PulseAnalyzer_GetParam(self.pulseSource, "waitTime"))))
        pulseConfigList.append(("triggerTime", "Trigger Time (seconds)",
                                str(self.dataManagerRpc.PulseAnalyzer_GetParam(self.pulseSource, "triggerTime"))))
        pulseConfigList.append(
            ("bufSize", "Buffer Size", str(self.dataManagerRpc.PulseAnalyzer_GetParam(self.pulseSource, "bufSize"))))
        ...
        if getParamVals:
            ...
            for idx in range(numThres):
                if dlg.textCtrlList[idx].GetValue() != concThresList[idx][2]:
                    self.dataManagerRpc.PulseAnalyzer_SetParam(self.pulseSource, "threshold",
                                                               float(dlg.textCtrlList[idx].GetValue()), concThresList[idx][0])
            for idx in range(numConfigParam):
                if dlg.textCtrlList[idx + numThres].GetValue() != pulseConfigList[idx][2]:
                    ...
                    if pulseConfigName in ["bufSize"]:
                        self.dataManagerRpc.PulseAnalyzer_SetParam(self.pulseSource, pulseConfigName,
                                                                   int(dlg.textCtrlList[idx + numThres].GetValue()))
                    else:
                        self.dataManagerRpc.PulseAnalyzer_SetParam(self.pulseSource, pulseConfigName,
                                                                   float(dlg.textCtrlList[idx + numThres].GetValue()))
        dlg.Destroy()
