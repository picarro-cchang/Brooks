...
###################### GPS ############################################
    ...
    def test_normal_GPS_uncertainty(self):
        status = self._get_peripheral_status(0, {"species": 25, "ValveMask": 0, "GPS_UNC_LAT": 0.0, "GPS_UNC_LONG": 0.0})
    ...
    def test_moderate_GPS_uncertainty(self):
        status = self._get_peripheral_status(0, {"species": 25, "ValveMask": 0, "GPS_UNC_LAT": 20.0, "GPS_UNC_LONG": 20.0})
        self.assertTrue((status & self._get_alarm_mask("ModerateGpsUncertainty")) > 0)
        self.assertTrue((status & self._get_alarm_mask("LargeGpsUncertainty")) == 0)

    def test_large_GPS_uncertainty(self):
        status = self._get_peripheral_status(0, {"species": 25, "ValveMask": 0, "GPS_UNC_LAT": 100.0, "GPS_UNC_LONG": 100.0})
        self.assertTrue((status & self._get_alarm_mask("ModerateGpsUncertainty")) > 0)
        self.assertTrue((status & self._get_alarm_mask("LargeGpsUncertainty")) > 0)
