...
###################### flow rate #####################################################
    ...
    def test_normal_intake_flow_rate(self):
        for _ in range(10):
            status = self._get_analyzer_status(0, {"species": 25, "ValveMask": 0, "MOBILE_FLOW": 4.0, "interval": 10.0})
        self.assertTrue((status & self._get_alarm_mask("IntakeFlowRate")) == 0)
        self.assertTrue((status & self._get_alarm_mask("IntakeFlowDisconnected")) == 0)

    def test_intake_flow_disconnected(self):
        status = self._get_analyzer_status(0, {"species": 25, "ValveMask": 0, "MOBILE_FLOW": -9999.0})
        self.assertTrue((status & self._get_alarm_mask("IntakeFlowDisconnected")) > 0)
...
###################### wlm ###############################################        
    ...
    def test_large_wlm_shift_and_adjust(self):
        status = self._get_analyzer_status(0, {"species": 25, "ValveMask": 0, "ch4_high_shift": 0.01, "ch4_high_adjust": -0.01})
    ...
    def test_uncorrelated_wlm_shift_and_adjust(self):
        status = self._get_analyzer_status(0, {"species": 25, "ValveMask": 0, "ch4_high_shift": 1e-3, "ch4_high_adjust": 0})
        self.assertTrue((status & self._get_alarm_mask("WlmShiftAdjustCorrelation")) > 0)
        self.assertTrue((status & self._get_alarm_mask("WlmShift")) == 0)
        self.assertTrue((status & self._get_alarm_mask("WlmAdjust")) == 0)
...
###################### measurement rate and results ############################################
    ...
    def test_slow_data_rate(self):
        for _ in range(10):
            status = self._get_analyzer_status(0, {"species": 25, "ValveMask": 0, "CH4": 2.0, "interval": 10.0, "SpectrumID": 25})
        self.assertTrue((status & self._get_alarm_mask("FastMethaneInterval")) > 0)
        self.assertTrue((status & self._get_alarm_mask("InvalidData")) > 0)

    def test_slow_data_rate_at_high_ch4_concentration(self):
        for _ in range(10):
            status = self._get_analyzer_status(0, {"species": 25, "ValveMask": 0, "CH4": 10.0, "interval": 10.0, "SpectrumID": 25})
        self.assertTrue((status & self._get_alarm_mask("FastMethaneInterval")) == 0)
        self.assertTrue((status & self._get_alarm_mask("InvalidData")) == 0)

    def test_slow_isotopic_data_rate(self):
        for _ in range(10):
            status = self._get_analyzer_status(0, {"species": 150, "ValveMask": 0, "CH4": 2.0, "interval": 10.0, "SpectrumID": 150})
        self.assertTrue((status & self._get_alarm_mask("IsotopicMethaneInterval")) > 0)
        self.assertTrue((status & self._get_alarm_mask("InvalidData")) > 0)

    def test_normal_isotopic_data_rate(self):
        for _ in range(30):
            status = self._get_analyzer_status(0, {"species": 150, "ValveMask": 0, "CH4": 2.0, "interval": 1.0, "SpectrumID": 150})
        self.assertTrue((status & self._get_alarm_mask("IsotopicMethaneInterval")) == 0)
        self.assertTrue((status & self._get_alarm_mask("InvalidData")) == 0)

    def test_negative_ch4_concentration(self):
        status = self._get_analyzer_status(0, {"species": 25, "ValveMask": 0, "CH4": -1.0})
        self.assertTrue((status & self._get_alarm_mask("CH4Concentration")) > 0)
        self.assertTrue((status & self._get_alarm_mask("InvalidData")) > 0)

    def test_normal_ch4_concentration(self):
        status = self._get_analyzer_status(0, {"species": 25, "ValveMask": 0, "CH4": 2.0})
        self.assertTrue((status & self._get_alarm_mask("CH4Concentration")) == 0)
        self.assertTrue((status & self._get_alarm_mask("InvalidData")) == 0)
...
###################### GPS ############################################
    ...
    def test_normal_GPS_uncertainty(self):
        ...
        analyzerStatus, peripheralStatus  = self._get_both_status(0, {"species": 25, "ValveMask": 0, "GPS_UNC_LAT": 0.0, "GPS_UNC_LONG": 0.0, "GPS_FIT": 6})
    ...
    def test_moderate_GPS_uncertainty(self):
        status = self._get_peripheral_status(0, {"species": 25, "ValveMask": 0, "GPS_UNC_LAT": 20.0, "GPS_UNC_LONG": 20.0, "GPS_FIT": 6})
    ...
    def test_large_GPS_uncertainty(self):
        status = self._get_peripheral_status(0, {"species": 25, "ValveMask": 0, "GPS_UNC_LAT": 100.0, "GPS_UNC_LONG": 100.0, "GPS_FIT": 6})
        self.assertTrue((status & self._get_alarm_mask("ModerateGpsUncertainty")) > 0)
        self.assertTrue((status & self._get_alarm_mask("LargeGpsUncertainty")) > 0)
        
    def test_large_GPS_uncertainty_without_inertial_mode(self):
        status = self._get_peripheral_status(0, {"species": 25, "ValveMask": 0, "GPS_UNC_LAT": 100.0, "GPS_UNC_LONG": 100.0, "GPS_FIT": 1})
        self.assertTrue((status & self._get_alarm_mask("ModerateGpsUncertainty")) == 0)
        self.assertTrue((status & self._get_alarm_mask("LargeGpsUncertainty")) == 0)
        
    def test_iGPS_configured_error(self):
        self.tester.DriverProxy.EEPROM["HardwareCapabilities"]["iGPS"] = False
        analyzerStatus, peripheralStatus  = self._get_both_status(0, {"species": 25, "ValveMask": 0, "GPS_FIT": 6})
        self.assertTrue((peripheralStatus & self._get_alarm_mask("iGPSConfigureError")) > 0)
        self.assertTrue((analyzerStatus & self._get_alarm_mask("InvalidData")) > 0)

    def test_car_speed(self):
        # large car speed
        status = self._get_peripheral_status(0, {"species": 25, "ValveMask": 0, "CAR_VEL_N": 50.0, "CAR_VEL_E": 50.0})
        self.assertTrue((status & self._get_alarm_mask("LargeCarSpeed")) > 0)
        # normal car speed
        status = self._get_peripheral_status(0, {"species": 25, "ValveMask": 0, "CAR_VEL_N": 5.0, "CAR_VEL_E": 5.0})
        self.assertTrue((status & self._get_alarm_mask("LargeCarSpeed")) == 0)
...
###################### Anemometer ############################################
    ...
    def test_anemometer_disconnected(self):
        ...
        status = self._get_peripheral_status(0, {"species": 25, "ValveMask": 0, "WS_STATUS": 1})
        analyzerStatus, peripheralStatus = self._get_both_status(0, {"species": 25, "ValveMask": 0, "WS_STATUS": 1})
